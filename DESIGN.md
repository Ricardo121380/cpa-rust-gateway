...
| 表头 `th` | `11px / 600 / +0.04em / uppercase` | `--ink-3` |
| 统计标签 `.stat-label` | `11px / 600 / +0.05em / uppercase` | `--ink-3` |
...
| 侧栏**激活**导航项(13px/600,坐在 `tint 16%` 芯片上) | **3.25:1** | **3.91:1** |

**地板是 4.5:1**(≤14px 正文按 WCAG AA 正常文本判定)。前三行通过。
第四行**不通过**,它是这个世界当前唯一的对比度缺口 —— 见 §10.4。
...
`tint` 文字压在 `tint 16%` 芯片上,实测 3.25:1(亮)/ 3.91:1(暗),低于 4.5:1。
它目前靠**芯片底 + 600 字重 + 位置**三重冗余来读,不是只靠颜色,因此不是"只用颜色传信息"的违规。
**已决定**:记录为公开缺口而不是悄悄放行。修法方向是把芯片提到 ~22% 或把激活文字换成 `--ink`,两者都会改变已批准的观感,需要一次显式决定。
