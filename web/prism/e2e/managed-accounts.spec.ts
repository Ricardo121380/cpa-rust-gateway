...
test("an unbound account is discoverable and can be disabled without resubmitting its secret", async ({page}) => {
  ...
  await page.getByRole("button", {name: "添加 API Key 账号", exact: true}).click();
  const dialog = page.getByRole("dialog", {name: "添加 API Key 账号"});
...
for (const width of [1440, 390]) {
  test(`managed account layout at ${width}px`, async ({page}) => {
    ...
    await expect(page.getByRole("button", {name: "添加 API Key 账号", exact: true})).toBeVisible();
