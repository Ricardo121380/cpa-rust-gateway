...
export async function navigate(page: Page, label: string): Promise<void> {
  const destinations: Record<string, [string, string?]> = {
    ...
    "模型目录": ["模型管理", "上游模型"], "计费与价格": ["用量与费用", "计费与价格"],
...
export async function selectVersion(page: Page, id: string): Promise<void> {
  ...
  if (await button.isEnabled()) {
    const draftChoice=(await button.textContent())?.trim()==="编辑草稿";
    await button.click();
    if(draftChoice)await page.getByRole("dialog",{name:"接续配置草稿"}).getByRole("button",{name:/^(接续草稿|切换待应用选择)$/u}).click();
  }
  ...
  if (previousPath !== "/versions") {
    await page.evaluate(async destination=>{const {router}=await import("/src/App.tsx");await router.navigate(destination);},previousPath);
  }
}
