...
test("provider edit saves one pending draft with stable footer and no early publication",async({page})=>{
 ...
 await page.locator(".provider-card",{hasText:"中转站 A"}).locator(".row-menu summary").click();await page.locator(".provider-card",{hasText:"中转站 A"}).getByRole("button",{name:"编辑",exact:true}).click();
...
test("pending provider creation blocks close and local action without replaying navigation",async({page})=>{
 ...
 await page.keyboard.press("Escape");await page.locator(".provider-card",{hasText:"中转站 A"}).locator(".row-menu summary").click();await page.locator(".provider-card",{hasText:"中转站 A"}).getByRole("button",{name:"编辑",exact:true}).click();
