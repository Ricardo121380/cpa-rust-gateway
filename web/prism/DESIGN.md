...
> 2026-09-13 当前交付：已部署57e32d1。视觉继续使用V6；日常结构为8个工作区，14个旧路由保留。提供商卡片包含多模型入口，模型按原始ID与来源连接组织，上游目录可批量接入；配置版本/审计等收于设置的高级维护。手机来源面板366px（390视口左右12px），三尺寸实际EgoLite验证通过。详见 [交付与截图](../../docs/reports/prism-model-workspace-delivery-20260913.md)。以下早期V4描述为历史基准，不能覆盖后续V6与本轮工作区实现。
...
## 26. D6 骨架层:枚举词汇的英文,以及一句诚实的覆盖声明(2026-08-21)

### 26.1 先量,再决定做多少

计划把 D6 估成 3.5 天。实测:**9,776 个用户可见中文字符、约 1,461 个字符串片段、32 个文件**。

更要命的是形状 —— 长句在 JSX 里被 `<strong>` / `<span className="mono">` 切碎了:

```tsx
<strong>空只意味着</strong>「该来源不存在」—— 它不等于健康、可用、已测试或可用于生产。
```

所以 D6 不是"抽出来翻译",是**连结构一起搬**。而这批文案的精度本身就是批 A–D 的产出:
「空不等于健康、可用、新鲜、已测试或可用于生产」这种句子,**英文译松一分就变成一个更弱的断言**,
比没有英文更糟。据此把范围收敛到骨架层,并把边界写在界面上。

### 26.2 枚举词汇的英文放在枚举旁边,不放进 i18n pack

pack 是**扁平键空间**,而这些词汇**故意重叠**:

| 值 | 在哪个词汇表 | 含义 |
|---|---|---|
| `disabled` | 认证轴 | 运维显式停用的凭据 |
| `disabled` | 价格证据 | 价格策略未启用,没有做过比较 |
| `disabled` | egress 域 | 出口被管理性停用 |

`active` / `expired` / `fresh` / `available` 同样各自碰撞。**一张扁平表会把整个模块存在的意义合并掉** ——
`authStatusMeta("cooling")` 必须是「未知」这条纪律(§22.2)在英文侧同样成立。

所以 `StateMeta` 多了一个 `en?: { label, detail }`,与它的枚举同住。分工写清楚:
**页面骨架的文案归 pack;枚举词汇是数据,数据跟着它的枚举走。**

代价是"英文只在 en.ts 一处"这条约定被打破,换来的是零漂移(中英同一个字面量里)
与零调用点改动。

### 26.3 一个组件,四十个调用点

`StateChip` 本来就收到 `raw`(枚举原值)与 `meta`。把 `localizeMeta(meta, lang)` 放进
`StateChip` 内部,**全站所有徽章一次性本地化** —— 不用给四十个调用点加 `lang` 参数。
glyph 与 tone 不过 `localizeMeta`:它们不随语言变。

缺 `en` 时**回落到中文,而不是空字符串**。一个显眼的未翻译标签是诚实的;
一个空白标签读起来像渲染故障,还把"缺翻译"这件事藏起来了。

### 26.4 覆盖率靠测试保证,不靠自觉

pack 的完整性由类型强制(漏一个键就是类型错误 —— 本轮就真的报了一次)。
枚举词汇住在模块里,拿不到这个保证,所以补一条单测:遍历十个闭集词汇表的每个成员,
断言都有 `en`。新增状态忘了翻译会在这里失败,而不是把中文徽章发进英文界面。

另有一条断言钉住"不合并":`authStatusMeta("disabled").en.detail` 必须**不等于**
`priceEvidenceMeta("disabled").en.detail`。

### 26.5 语言开关此前是过度承诺

原文案:「界面文案立即切换」。对骨架是真的,对**每一页正文都是假的**。
现在明说英文覆盖到哪 —— 导航、设置、解锁、版本选择,加上所有状态词汇 ——
以及**为什么正文没译**。已知缺口和意外,差别就在这一句。

### 26.6 验证边界

真网关上验到:导航整条切英文(零中文残留)、覆盖声明在屏、旧的过度承诺文案已不在包里、
页面正文如声明所述仍是中文。
...
## 31. 生产资源显示名（2026-09-10）

用户要求移除列表中到处出现的 P12 历史前缀与长哈希。`resourceNames.ts` 和
`ResourceIdentity.tsx` 提供统一展示：优先使用已有业务名称，旧阶段 ID 推导为可读名称，
并附稳定短编号区分同名对象。账号详情的“技术标识”可展开与复制完整 ID。
原始 ID 仍用于 API、URL、查询、缓存、操作和审计，公开模型的 exact ID／协议名称不变。
同一账号的不同绑定不合并；保留历史请求、账本和生产配置。

名称层已接入账号、上游及绑定、目录、路由／候选、访问控制、出口、诊断、请求／账本、
用量分组、配置及审计列表。选择框保留原 ID 作为 value；确认、编辑字段与审计技术事实
仍可查看完整原值。长名称可换行；前缀清理不通过重命名数据库或删除历史数据实现。
K3 的 V6 结构设计仍在 OpenDesign 中执行，另行记录其来源与正式实现证据。


## 32. V6：K3 max 结构与正式资源展示（2026-09-10）

本轮用户明确选择 OpenDesign Pi 的 Kimi for Coding K3，并指定 max。实际 MCP 参数为
`cc-switch-kimi-for-coding/k3:max`；原型、规格、MCP 回读哈希与适配决定见
[设计交接](../../docs/handoffs/prism-opendesign-v6.md)。§31 的设计进行中状态已结束。

V6 采用 K3 的紧凑 chrome、分层指标、细线分节、总览分栏与分组表格。按照用户对内容与
导航一致性的要求，正式应用保留 V5 的一个共享磨砂工作区，而不采用原型的纯实底大板。
三面 GlassSurface／PrismLens、手机 12px 边距、520px 详情、居中确认、焦点与辅助回退保留。
`src/app/v6.css` 替代 V5 样式入口，消除旧背景的硬斜切光带与重复卡片边框。

名称层覆盖主要资源列表、引用与上下文。业务名称优先，历史阶段 ID 和长哈希用可读名称与
8 位编号展示；完整 ID 留在技术详情、复制、API、URL 与原始审计中。模型 exact ID 不改。
账号按上游分组但不合并绑定，统计明确为绑定数量；原有维护与诊断动作保留。
配置的技术信息合并折叠，手机列表纵向排列。全部 14 栏目与 `#/unlock`、总览 `#/` 保留。

最终本地类型、262 单测、128 E2E、111 操作／四文件双构建、3 Rust 嵌入测试通过；真实
网关三尺寸浅深色 90 视图及 10 阶段写入链路通过。生产状态与原始证据索引以
[交付报告](../../docs/reports/prism-v6-refinement.md) 为准。原型合成数据不构成接口或业务授权。

## 33. 面向 CPA 用户任务的导航（2026-09-11）

用户已授权功能重构方案，八个一级工作区替代固定十四项主导航：仪表盘、账号管理、AI 提供商、
模型管理、API 密钥、请求日志、用量与费用、设置。模型目录进入模型管理，计费价格进入用量与费用，
出口、诊断、配置版本及审计备份进入设置子页。十四个既有 hash 路由与 query 保留，搜索仍能找到。
主导航在子页上高亮所属工作区，次级导航保持键盘可达及窄屏换行。沿用 V6 token 和玻璃材料。
这次调整不意味着旧页面的功能缺口已补齐；完整账号接入和保存生效进度以功能重构进度报告为准。


## 34. 全部账号：人类身份与统一分类（2026-09-11）

用户要求取代 §31 的账号短编号和 §32 的账号上游分组。全部账号采用 API、Codex / ChatGPT、
Claude、Kimi、Kiro、Grok 六类，Grok 下面 Web / Console / Build。共用 AccountList，手机
同一列表 DOM 纵向重排；沿用 V6 的共享磨砂工作区与 panel tint，不为 Grok 另建玻璃卡片。

身份优先邮箱、电话、用户名；Autoreg 仅为来源，机器 ID 保留在技术详情。缺少身份不生成
假名称。接口连接显示协议用途，主机与配置启用状态进入右侧详情；原生 Grok 则说明渠道池
关系，不套用普通绑定计数。分组计数与搜索只覆盖已加载结果，分页入口保持可见。
...
EgoLite 真实网关复验发现共用详情与 Codex 授权标题仍使用内部 ID，已修复：目录的身份
投影和真实渠道名传入详情，授权两个入口沿用同一身份；其他详情入口可使用已有元数据邮箱。
标题统一为“账号详情”/“重新授权”，原 ID 只留在技术详情与请求参数。实际浏览器与回归证据见
[EgoLite 验收](../../docs/reports/prism-egolite-20260912.md)，替代 §34 中待完成人工检查的状态。

## 36. 身份、授权与运行连接（2026-09-12）

用户认可全部账号的分类与样式，要求补上 SSO 邮箱、解释重复 Codex，并统一运行状态。
沿用 V6 材质和列表；本批没有切换设计方向或调用新的外部设计模型。

同渠道下相同邮箱/电话只占一条目录身份，标明授权数量，“管理授权”打开共用详情面板，
每份授权保留独立详情、重新授权与启停目标。缺失身份或仅同用户名不归并，不删除底层记录。
Autoreg 只在对应授权中标记来源。520px 桌面详情、手机 12px 边距与单个遮罩保持。

运行状态使用实际运行配置提供的姓名/邮箱、渠道及协议/主机；不再展示旧阶段标签或短编号。
每条运行连接仍独立，认证、调度、停用分开显示；完整技术 ID 可展开，精确操作目标不变。
搜索支持人类身份/渠道/接口，旧 provider query 继续生效，可显式清除限定。

SSO 导入自动尝试服务端资料读取；已有缺邮箱账号有“读取身份”入口。读取失败明确提示，
不伪造姓名、不把保存凭据成功说成邮箱已获取。线上两条 Console 仍受 403 限制，本地交互
验收与线上资料获取分别记录，详见
[身份一致性报告](../../docs/reports/prism-account-identity-continuity-20260912.md)。本批尚未发布。

## 37. 全站资源名称与精确引用（2026-09-12）

本节按用户最新要求替代 §31、§32、§34–36 中“八位编号”和“展开显示完整技术 ID”的规则。
普通列表、详情、操作标题、不可编辑字段及选择器使用身份、实际渠道、接口用途与可读资源名称，
不再附加生成编号，也不将历史测试主键作为 tooltip。沿用现有 V6 材质与导航，不切换风格。

账号的真实邮箱/电话/用户名原样保留；缺失身份明确提示。公开模型 exact name、别名、
请求追踪号与修订值不按测试前缀清洗。`ResourceIdentity` 只处理显示，`ResourceIdInput`
保留隐藏的原表单值，`ResourcePicker` 通过已有接口读取有界资源列表，提交原 ID。
历史请求筛选支持当前配置之外的资源：“指定历史资源…”收取精确引用后恢复人类标签。

详情中的“关联信息”使用实际姓名与渠道；“复制内部引用”明确复制原主键。
API、URL、缓存、操作目标、原始审计/导出及机器 JSON 中的引用不重写。
生产主键不为界面观感做批量迁移，同邮箱也不作为删除不同授权的充分依据。

此次已在生产主机内验证同 subject、同 token、同连接/出口的两份 Codex grant，
通过配置生命周期只保留原主授权及绑定；正常的多授权详情功能仍保留。
...
## 38. CPA 日常流程对齐与上线（2026-09-13）

生产版本 `1b81ce7`、schema 26。本节替代旧节中“读取身份”和“保存后需重启”的交互说明。
身份由授权/导入/凭据更新取得；列表不提供“读取身份”按钮。账号、提供商、模型与客户端密钥
按业务任务维护，普通操作准备、校验、保存并应用；显式进入已有草稿时只保存。高级资源工作台
仍在原有链接下，八个一级工作区与全部旧路由保留。

V6 的材质、GlassSurface/PrismLens、统一账号样式和可读名称保留，未调用新的外部设计生成器。
系统信息、设置四项服务概况、首页接入步骤与费用信息按实际网关契约表达。新请求因应用失败
暂停时给出明确恢复入口；关闭密钥时退出动画副本立即清除秘密。

EgoLite 验证本地实际网关的空安装、导入、启停、更新凭据、模型/别名/密钥、受控请求与账本，
14 个入口三种尺寸与深色/辅助偏好、会话失效和错误恢复；签名 ARM64 生产副本、schema 升降级、
现有域名四资产与管理读回通过。原管理员及生产历史保留。详见
[完整交付与范围](../../docs/reports/prism-cpa-alignment-delivery-20260913.md)。

## 39. V2 文本安全间距（2026-09-23）

当前 V2 各 surface 自己管理内边距，不再通过旧 rail-underlap 选择器重置任意
section 子节点的 padding。圆角运行面板的工具栏/底栏水平 18px，手机账号卡片
容器 16px、指标单元 14px；提供商行与选中边线保留 12px 间距。

账号表格长内容允许换行，手机值列最小宽度为零。弹窗正文、标签与操作区应在
容器内排布，保留下拉箭头空间及可见焦点。420px 及以下的授权渠道选择改为单列，
长名称完整显示；正文滚动，页脚取消操作保持可见。桌面仍为双列。

本次仅在本地真实嵌入网关验收，未上线；证据与验证边界见
[文字边距与弹窗精修](../../docs/reports/prism-v2-spacing-polish-20260923.md)。
