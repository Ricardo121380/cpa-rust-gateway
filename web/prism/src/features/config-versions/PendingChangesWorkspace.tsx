...
export function PendingChangesWorkspace({target,active,onClose,onAdopt}:Readonly<{target:ConfigVersionSummary;active?:ConfigVersionSummary;onClose:()=>void;onAdopt:(target:ConfigVersionSummary)=>void}>) {
 ...
 return <InlineWorkspace title="待应用变更" description="核对这一批配置的净变化；全局操作单独列出。" dirty={false} busy={lifecycle.active} onClose={onClose}
  ...
  </div><aside className="changes-workspace-aside"><section className="changes-impact"><h4>应用影响</h4><p>{active?complete?`已完整读取 ${rows.length} 项净变化。`:"差异仍未完整读取，暂不能应用。":"首次服务配置，需校验后应用。"}</p><p>应用后使用新的服务配置；历史请求与账本保留。</p></section>
  <section className="changes-independent" aria-label="已独立保存的全局操作"><h4>已独立保存的全局操作</h4><p className="stat-sub">目录按自己的生效时间参与计价；应用、放弃或回滚配置不会删除这些目录，也不会改写历史账本。</p>
