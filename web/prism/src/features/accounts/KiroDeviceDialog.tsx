...
export function KiroDeviceDialog({providerId,endpointId,onClose,onComplete,credentialId}:{providerId?:string;endpointId?:string;credentialId?:string;onClose:()=>void;onComplete:(notice?:string)=>void}) {
 ...
 return <Sheet title={credentialId?"重新授权 Kiro 账号":"授权 Kiro 账号"} description={credentialId?"仅更新这个已有账号的授权；当前连接会保留。":"使用 AWS Builder ID 设备授权；组织设置只在需要时展开。"} onEscape={close} onBeforeDismiss={dismissAuthorization} busy={busy} blockNavigation={awaitingConsent} footer={footer}>{completed?<p role="status">账号已保存{current.current?.endpointId?"并连接接口":""}。</p>:session?<><p role={unresolvedResult?"alert":"status"}>{unresolvedResult?"授权结果暂时无法确认；不会重复提交授权请求。请稍后在账号管理中核对。":completionError!==undefined?"账号已保存，配置尚未应用。":pollInFlight&&awaitingConsent?"正在检查 AWS 授权…":session.state==="pending"?"等待 AWS 官方授权":session.state==="completed"?"账号已保存":session.state==="denied"?"授权被拒绝":session.state==="expired"?"授权已过期":"授权未完成"}</p>{awaitingConsent?<><p>在 AWS 官方页面完成登录并输入验证码。</p><strong className="mono">{session.user_code}</strong>{url?<p><a href={url} target="_blank" rel="noopener noreferrer">打开 AWS 授权页</a></p>:null}{session.expires_at_ms?<p>有效期至 {new Date(session.expires_at_ms).toLocaleTimeString()}。完成后会自动检查结果。</p>:null}</>:null}{notice?<p role="status">{notice}</p>:null}</>:<form id={formId} className="sheet-form" onSubmit={event=>{event.preventDefault();start.mutate();}}><p>AWS Builder ID 使用默认入口。组织 Identity Center 设置仅在需要时填写。</p><details open={advanced} onToggle={event=>setAdvanced((event.currentTarget as HTMLDetailsElement).open)}><summary>组织账号高级选项</summary><label>授权地区<input required value={region} onChange={e=>setRegion(e.target.value)} disabled={busy}/></label><label>启动地址<input required type="url" value={startUrl} onChange={e=>setStartUrl(e.target.value)} disabled={busy}/></label></details></form>}
