...
export function KiroDeviceDialog({providerId,endpointId,onClose,onComplete}:{providerId:string;endpointId:string;onClose:()=>void;onComplete:(notice?:string)=>void}) {
 ...
 const start=useMutation({mutationFn:async()=>{const task=await beginConfigurationTask("授权 Kiro 账号");const id=`kiro-${crypto.randomUUID()}`;current.current={task,id};setWorkingId(task.version.id);return task.read<Session>("startKiroEnrollment",{path:{upstream_id:providerId},headers:{"If-Match":task.version.revision},body:{id,region,start_url:startUrl}});},onSuccess:setSession});
 const poll=useMutation({mutationFn:async()=>{const entry=current.current;if(!entry||!session)throw new Error("请先开始授权");const {task,id}=entry;const result=await task.mutate<Session>("pollKiroEnrollment",{path:{upstream_id:providerId},body:{id,session_id:session.session_id}});if(result.state==="pending"){setNotice(`等待官方授权，请至少 ${Math.ceil(result.interval_ms/1000)} 秒后再检查。`);return;}
 ...
 const cancel=useMutation({mutationFn:async()=>{const entry=current.current;if(entry&&session)await entry.task.read("cancelKiroEnrollment",{path:{upstream_id:providerId},headers:{"If-Match":entry.task.version.revision},body:{id:entry.id,session_id:session.session_id}});},onSuccess:onClose});
