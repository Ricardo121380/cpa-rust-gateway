...
function ManagedAccounts() {
  ...
  const actions = (row: ManagedCredential) => <div className="page-actions">
              ...
              {directory.find(item=>item.id===row.credential.id&&!item.native)?.operations.includes("reauthorize") ? <button className="secondary" onClick={() => {setAuthorizations(undefined);if(row.category==="claude")setClaudeOauth(row);else setOauth(row.credential.id);}}>重新授权</button> : null}
