...
export function AuthorizationCodeDialog({providerId,providerName,endpointId,onClose,onComplete,channel="codex",credentialId}:Readonly<{
  ...
  const complete=useMutation({gcTime:0,mutationFn:async()=>{
    ...
    if(endpointId){
      ...
      if(!bindings.some(row=>row.credential_id===account.id))await task.mutate("createEndpointCredentialBinding",{path:{endpoint_id:endpointId},body:{credential_id:account.id,enabled:true,priority:0,weight:1,concurrency:1}});
    }
    return task.finish();
