...
const formats: Record<string, string> = {
  ...
  grok_build_json: "Grok Build 凭据 JSON", sso: "SSO 凭据",
};
...
export function AddAccountDialog({onClose, onCreated}: Readonly<{onClose: () => void; onCreated: () => void}>) {
  ...
  const create = useMutation({
    ...
    mutationFn: ({provider, name, material}: {provider: string; name: string; material: string}) =>
      call("importChannelAccount", {path: {upstream_id: provider}, body: {id: name, channel: channelId, secret: material}}, {versionScoped: true, mutating: true}),
    onSuccess: onCreated,
  ...
  return <Sheet title="添加账号" onEscape={() => !create.isPending && onClose()}>
    {channels.isError || providers.isError ? <p role="alert">{asAppError(channels.error ?? providers.error).message}</p> : channels.isPending || providers.isPending ? <p>读取接入方式…</p> : <>
      ...
      {channel?.import_available ? matches.length === 0 ? <p>先添加此渠道的 <Link to="/upstreams" onClick={onClose}>AI 提供商</Link>。</p> :
        <form className="sheet-form" onSubmit={submit} autoComplete="off" key={channelId}>
          <label>提供商<select name="provider" required>{matches.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
          <label>账号名称<input name="name" required maxLength={128} placeholder="例如：团队账号" /></label>
