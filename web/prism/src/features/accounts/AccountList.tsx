...
export type AccountListRow = Readonly<{key:string;name:string|undefined;source:string|undefined;provider:string;authentication:string;plan?:string|null;planSource?:string|null;status:ReactNode;connection:ReactNode;actions:ReactNode;selected?:boolean;onSelect?:()=>void}>;
export function AccountList({rows}:Readonly<{rows:readonly AccountListRow[]}>) {
  ...
  return <div className="account-list-wrap"><table className="account-list"><thead><tr><th>账号身份</th><th>渠道</th><th>授权状态</th><th>接口连接</th><th><span className="sr-only">操作</span></th></tr></thead>
    <tbody>{rows.map((row)=><tr key={row.key} data-account-key={row.key}>
      ...
      <td data-label="授权状态">{row.status}</td><td data-label="接口连接">{row.connection}</td><td className="account-list-actions">{row.actions}</td>
    </tr>)}</tbody>
  </table></div>;
}
