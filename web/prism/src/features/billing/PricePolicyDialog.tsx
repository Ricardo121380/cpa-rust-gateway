...
export function PricePolicyDialog({action,onClose,onDone,onReview}:Readonly<{
  ...
  return <Sheet title={receipt?"价格策略结果":action.kind==="bind"?"绑定路由价格目录":"清除路由价格策略"}
    ...
    {receipt?<><p className="operation-receipt" role={receipt.kind==="unconfirmed"?"alert":"status"}>{receipt.message}</p><p>目录：<strong className="mono">{receipt.target}</strong></p><p>工作配置 ID：<code className="mono">{receipt.workingVersion.id}</code></p>{review.isError?<p role="alert">重读失败：{asAppError(review.error).message}。可重试或按此 ID 稍后核对。</p>:null}</>
