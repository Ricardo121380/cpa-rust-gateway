...
## 2026-09-15: recently observed key requests, public prices and Kiro device enrollment

- ClientKey `last_request_at_ms` is nullable and refers to a logical key ID across configuration forks. It is the maximum observed request start among durable terminal records; in-flight/legacy unknown times are not fabricated. A schema28 expression index supports one indexed batch read without a global event cap; down28 only drops this index and does not delete history.
- `POST /admin/billing/price-source/refresh` reads only fixed `https://models.dev/api.json`, with 16MiB/20s limits and the bounded blocking reader. Requested exact model IDs are filtered locally; no account secrets or private model list is sent upstream. Provider sources remain separate, missing rates null, context tiers not flattened. No catalog write occurs until ordinary import/difference confirmation. USD conversion only scales dollars into micro-units; the UI requires acknowledgement and preserves six independent rates.
- Kiro start/poll/cancel uses the [AWS OIDC device API](https://docs.aws.amazon.com/singlesignon/latest/OIDCAPIReference/API_StartDeviceAuthorization.html), [token API](https://docs.aws.amazon.com/singlesignon/latest/OIDCAPIReference/API_CreateToken.html), and [official CLI scope/start URL reference](https://github.com/aws/amazon-q-developer-cli/blob/main/crates/chat-cli/src/auth/consts.rs). 128 expiring sessions, server-only client/device/token secrets, region/start URL allowlist, session/context ownership, polling interval and slow-down, CAS persistence and replay rejection. No placeholder account. No unsupported AWS ID-token email is invented. Real official login remains a separate human verification step.
- Existing runtime refuses nonempty active Access Group limits; this release does not add a quota engine or silently split such groups. Ordinary per-key model/status/expiry editing remains supported. The shared-quota UI experiment was withdrawn after real-gateway validation exposed this boundary.
