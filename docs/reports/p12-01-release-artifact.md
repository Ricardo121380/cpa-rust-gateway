...
than a multi-architecture index, so the arm64 job pins `sha256:9b672946…` explicitly.

Building with the wrong base digest does **not** simply fail. A local arm64 build using the amd64
digest succeeded and produced an image whose config falsely claims `architecture=arm64`, which the
architecture assertion alone cannot detect. Only the `base.name` comparison rejects it, so that
check is load-bearing rather than redundant.

The first remote run
([30530890273](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/30530890273)) failed
both jobs identically with `base name (${RELEASE_BASE_IMAGE}) should not be blank`: the
`ARG RELEASE_BASE_IMAGE` had been placed after the first `FROM`, making it stage-scoped, and a
`FROM` only sees ARGs declared before the first `FROM`
([Dockerfile reference](https://docs.docker.com/reference/dockerfile/#understand-how-arg-and-from-interact)).
The arm64 job reached that step having already built its native binary and SBOM successfully, so the
defect was in the Dockerfile rather than in arm runner availability. The ARG is now global and
re-declared inside the stage so the label can still use it, and the workflow checker statically
rejects any `FROM`-referenced ARG not declared before the first `FROM`.
...
rejection of targets outside the table. The workflow checker was exercised against four injected
regressions — a base digest the verifier does not enforce, an arm64 target moved onto an x86_64
runner, an added `setup-qemu-action` step, and the stage-scoped ARG defect above — and rejected each.

The arm64 path was then verified end to end on a local arm64 Docker host: `docker buildx build
--platform linux/arm64` with the pinned arm64 digest produced a real `architecture=arm64` non-root
OCI archive whose `base.name` carries the pinned digest; the production verifier accepted it and
rejected the wrong-base variant; and `docker load` followed by
`docker run --network none --read-only --user 65532:65532 … --help` executed the image. A real
`aarch64-unknown-linux-gnu` CycloneDX SBOM (205 components, correct target triple) was generated by
the production generator and passed the full verifier end to end.

### Remote dual-target acceptance

| Field | Result |
|---|---|
| Accepted revision | `428be9396881ebf7468f946643b09fa1a54c7ca6` |
| Release workflow | [run 30533211028](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/30533211028), `SUCCESS`, both jobs |
| aarch64 job | `SUCCESS`, 12m20s on `ubuntu-24.04-arm`; Rekor index `2289216552` |
| x86_64 job | `SUCCESS`, 14m25s on `ubuntu-24.04`; Rekor index `2289229433` |
| Signing identity | `…/release-artifact.yml@refs/heads/codex/p12-deployment`, GitHub Actions OIDC keyless, identical for both targets |
| Deployment boundary | No registry push, GitHub Release/tag, server login, listener, systemd, Caddy/Cloudflare or deployment change |

Both private artifacts were downloaded and re-verified locally with
`--require-signature --require-receipt`, each against its own target, and both passed.

| Target | Binary | Bytes | Binary SHA-256 | Manifest SHA-256 |
|---|---|---:|---|---|
| `aarch64-unknown-linux-gnu` | `gateway-aarch64-unknown-linux-gnu` | 7108288 | `e8d7b534e44a794615b0dd4946d1af795a6c2b66f54e3f59338466c41b8b3f62` | `0fee08110b73322acd0424ed4f14b73fddc273c6511ba9fc4c4588443ee3f8ad` |
| `x86_64-unknown-linux-gnu` | `gateway-x86_64-unknown-linux-gnu` | 8044696 | `d6645801e51bfde6b8f6685ecc6b455a09b7083cf1d435579bfe3733d7324494` | `d931efa0fd25131292b070c3aa1a2055187a3ef683499923ab312bafd0f68ca4` |

`file` reports the aarch64 payload as `ELF 64-bit LSB pie executable, ARM aarch64`, so the
production host can now execute a signed release binary. Each image carries only its own
architecture and its own pinned base digest (`arm64` / `sha256:9b672946…` and `amd64` /
`sha256:63a496b5…`). In the aarch64 job the container smoke test ran the real binary under
`--network none --read-only --user 65532:65532` and it printed its actual `gateway serve` /
`gateway admin` usage, so the signed arm64 binary is proven executable rather than merely built.
