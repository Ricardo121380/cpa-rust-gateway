...
`web/prism/src/app/AppShell.tsx:175-177` 已渲染模型工作区切换，`app/navigation.ts` 已把 `/models` 和 `/catalog` 归为同一工作区。上表 G02 的页面级检查遗漏了外壳实现，“尚无统一工作区入口”的推断撤回。仍需单独验证切换后的筛选与来源连续性，不增加重复导航，也不把 G02 记为本批新实现。G03 保留：原提供商开放入口确实未带来源。
