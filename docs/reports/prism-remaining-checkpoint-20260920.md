...
## Provider top-level maintenance checkpoint
...
## Inline egress checkpoint — 2026-09-21

Policy, proxy pool/node and binding editors now use InlineWorkspace with shared departure ownership and fixed actions. Each accepted write is bound to the captured draft revision, cannot be replayed from a failed editor and retains a saved-not-applied receipt. Known deletion references block confirmation. Node addresses are cleared from the form on accepted submit and scrubbed from the retained mutation input after settlement. Policy edit preserves the original scheme list and names.

Fresh evidence: 14 compatible/egress/modal browser cases, 390 units, TypeScript and four-file deterministic/SPA gates passed. Actual disposable gateway/EgoLite: edited local policy; created pool, sealed node and endpoint/account pool binding; read all receipts with context still draft. No actual proxy dial or Provider inference. Structural viewport checks at 1440×900, 1280×720 and 390×844: one inline editor, no dialogs, no document horizontal overflow. This does not establish final visual/accessibility acceptance. Whole-app remaining UI refinement, integration gates and production deployment remain open.
