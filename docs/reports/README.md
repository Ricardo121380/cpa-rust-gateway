...
- [P13-11E4 Provider-specific egress management projection](p13-11e4-provider-egress-management-projection.md) — `LOCAL_PASS_PENDING_PHASE_GATE`; atomic exact source-domain snapshot, retained cursor, protected GET and OpenAPI/Prism/client passed focused local gates; current source is Build/Console-only, production Web/clearance may be empty, generic compatible remains separate; independent final review passed with no remaining finding, aggregate Full/formal Gate pending
