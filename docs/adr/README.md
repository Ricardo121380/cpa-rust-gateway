...
- [ADR-0098 Provider-specific egress management projection](ADR-0098-provider-specific-egress-management-projection.md) — `P13-11E4 READY_FOR_FORMAL_DELIVERY_GATE`（atomic GET-only exact egress/session/clearance snapshot；Config/runtime 双 revision 与 retained cursor；protected HTTP/OpenAPI/Prism/client 已完成；aggregate Full 43/43；当前只投影 Build/Console，production Web/clearance 可空，generic compatible 独立；formal Gate 未运行，无 recovery/Provider/网络）
