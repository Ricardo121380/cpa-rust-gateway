...
P13-11A 已建立 [BC-SEC-005 Generic compatible-endpoint egress profile](BC-SEC-005-generic-compatible-endpoint-egress.md)（`LOCAL_PASS_PENDING_PHASE_GATE`；generic `base_url + api_key`/imported credential foundation；不调用 Provider/网络）。
P13-11B 已完成 [BC-SEC-006 Compatible-endpoint runtime egress composition](BC-SEC-006-compatible-endpoint-runtime-composition.md)（`LOCAL_PASS_PENDING_PHASE_GATE`；active Config Version、exact pool/policy lineage、单一 Upstream transport registry、Direct/local-DNS SOCKS5、RAII lease；不调用 Provider/网络）。
P13-11C 已完成本地实现 [BC-SEC-007 Compatible serving transport handoff](BC-SEC-007-compatible-serving-transport-handoff.md)（`LOCAL_PASS_PENDING_PHASE_GATE`；复用 serving exact Credential lease；模式超时不变；精确 egress lease/failure scope；不调用 Provider/网络）。
