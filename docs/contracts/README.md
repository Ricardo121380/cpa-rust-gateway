...
P13-10A 已建立 [BC-RESP-004 Public OpenAI Responses WebSocket](BC-RESP-004-public-responses-websocket.md)（`LOCAL_PASS_PENDING_PHASE_GATE`；复用 Canonical/Client Key/lease/stored continuity，text-only `response.create`、显式 capability、有界背压/关闭/取消；不等同 Realtime API）。
