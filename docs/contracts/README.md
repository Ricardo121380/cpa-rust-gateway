...
P13-11D 已建立 [BC-MGMT-018 Compatible proxy-pool persistence and protected management](BC-MGMT-018-compatible-proxy-pool-management.md)（`IN_PROGRESS`；D1 先实现 Config-Version-owned Store/AEAD foundation；D2 才改变 management OpenAPI/Prism，D3 才接入 active runtime；当前不调用 Provider/代理/DNS）。
