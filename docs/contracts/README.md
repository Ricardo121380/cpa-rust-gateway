...
P13-11E 已建立 [BC-SEC-008 Provider-specific egress, health, and recovery isolation](BC-SEC-008-provider-specific-egress-health-recovery.md)（`E0 PLANNED_REVIEWED`；冻结 generic endpoint、Grok Build/Console/Web、Official/Kiro/其他 Provider 的状态域和 no-fallback/recovery 边界；未开始代码或网络验证）。
