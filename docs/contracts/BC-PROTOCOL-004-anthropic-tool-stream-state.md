...
2. Anthropic wire blocks never overlap: an emitted `content_block_start` receives only its own
   deltas and exactly one `content_block_stop` before the next wire block starts. A later logical
   Tool/Text block is buffered while an earlier block remains open, then flushes its retained
   per-Tool fragment sequence under its already assigned index. This changes delivery timing, not
   the final Canonical Tool semantic projection.
3. A Tool start closes the active text block in the logical projection before allocating the Tool
   block. A Tool block starts on the wire with `input: {}` and has exactly one
   `content_block_stop` after its completed input is accepted.
4. Each non-empty `input_json_delta` uses the declared Tool's own index. Empty decoded fragments
...
5. If any argument delta was received, its accumulated normalized JSON text must equal the final
   ...
   visible.
6. Empty/whitespace-only input and a whitespace-wrapped empty object normalize to `{}`. A Tool
...
7. The completed input must parse as a JSON object. Array, scalar, malformed, duplicate,
   ...
   unfinished.
8. A completed response containing Tool output uses `stop_reason: "tool_use"` in both Anthropic
   response forms. P5-06 owns all broader Stop Reason, Thinking, cache-usage, and model-rewrite
   mapping.
9. This contract does not prove required-property validation because the response codec has no
...
- A fixed-seed 128-case property suite splits Unicode-safe decoded strings at arbitrary scalar
  ...
  reassembly to the non-streaming projection. It also asserts that each Anthropic SSE delta and
  stop belongs to the one currently active content block.
