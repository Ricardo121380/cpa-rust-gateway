...
3. Account and lineage are short opaque ASCII references, not email/account values. A strict
   direct-import session must expire after observation time and within 90 days. The governed
   migration adapter may reduce a valid longer source expiry to exactly the 90-day local maximum
   and must report that reduction as a value-free count. It has at most 32 scoped Cookies.
...
| Expiry | A credential is unusable when `now_ms >= expires_at_ms`. Expired, non-positive, or overflowed imports fail before state creation. Strict direct imports also reject overlong sessions; the governed migration adapter may persist only `min(source_expiry, observed_at + 90 days)` and reports each reduction. |
...
- `overlong_web_expiry_is_reduced_to_the_local_window_before_persistence`
