...
| Domain | Exact Statsig signature cache plus injected P2 signer/redirect admission |
...
| I/O boundary | This module has no HTTP client, socket, TLS, browser, Cookie, proxy/TUN, server, or production-config action. A static resolver in local tests is not a live DNS lookup. |
| Ownership | P9-06 owns quota, P9-07 owns HTTP failure attribution, and P9-09 owns any authorized real signer/Web traffic. |
