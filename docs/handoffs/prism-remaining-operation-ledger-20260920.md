...
| 全局 / 复杂页内操作 | 新增 OperationBoundary / InlineWorkspace；计费采用，复核修正中 | 关闭、同页 query、Back、忙碌、会话和错误恢复 |
| 全局 / 待应用批次 | 精确草稿身份、显式恢复入口与策略→密钥→发布链路已实现 | 人类可读合并差异、其他草稿采用/刷新恢复边界仍待补 |
...
| 提供商 / 顶层编辑与删除 | UpstreamsPage 仍有兼容表单路径 | 稳定底栏、精确基线、结果不确定及重读完整迁移 |
...
| 价格 / 导入与恢复 | 全局保存与配置发布分离；真实本地导入已验 | C2C 本轮修正复核、真实恢复/生效时间与历史保留 |
| 价格 / 编辑与完整预览 | 页内、每页50、全量 backing model、错误定位 | 1/50/51/512、513拒绝、1024差异、冻结内容回读已验；最终视觉待定 |
...
| 设置 / 配置版本与差异 | 既有发布/回滚回执；创建补 busy | 复杂差异页内、待应用摘要与明确校验应用 |
...
设计依赖：指定的 OpenDesign Pi/K3 max 实际解析为 high，已向用户说明并等待选择；功能改动继续。已返回的原型仅为待审探索，不能覆盖真实授权、协议、价格和配置语义。
