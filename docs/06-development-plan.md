...
| 当前任务 | `P13-07B` 已完成本地实现与 review：将同一 RouteCredentialScheduler、credential pools、Health/Quota registry 接入现有管理 Route Explain，增加可选的显式 provider scope；多 Provider 未指定 scope 时 fail-closed，expiry/known-vs-unknown quota 与 foreign Provider 均保持安全投影。下一片为 `P13-07C` serving-path lease revalidation；继续保留 max_attempts、首败、Config Version 与无跨 Provider fallback 边界，不启动 P13-08/11/12。 |
...
| P13-07 | Cost-aware、Fill-first、Least-loaded 路由：在 Provider scope 内依据额度、并发、健康和 capability 选择；保留 max_attempts、首败边界和 Config Version 回滚 | P13-05、P13-06 | P13-07A deterministic Provider-scoped selector 与 P13-07B 只读 scheduler/Route Explain composition 已本地通过并完成 review；P13-07C 再做 serving-path lease revalidation，之后才评估 bounded staging canary；不跨 Provider fallback | IN_PROGRESS |
