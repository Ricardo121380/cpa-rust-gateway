...
| 当前任务 | `P13-12 closeout + P13-15A rollout`：P13-12 完成真实 Build entitlement/backfill、Claude Code contract sync 与正式 Gate；P13-15A 本地实现已把 Client-Key 可见模型改为 exact upstream Candidate model ID，并让请求只能进入同一 exact-model candidate，等待 revision-bound ARM64 production rollout。Pi/Grok Build 固定使用 OpenAI Responses 与 `/v1` base URL，不再把 Messages 诊断路径或 `grok-cpar-build` 人工别名当作最终配置。后续 P13-15B-E 再完成所有渠道的真实 discovery、持久化 freshness/removal、Credential-scoped materialization 与真实隔离 E2E。 |
...
> 当前版本置顶变更：`v1.312`（2026-09-01）见下方第一行；历史版本保持原样。
