...
5. **模型与思考强度。** 可用时优先使用 `luna` 的 `minimal`；只有需要汇总少量固定输出时升为
   `low`。平台没有可调用的低成本模型时，不得把高级模型伪称为 Luna 子代理；主会话改用自身的
   `minimal`/`low` 通道完成，或向用户报告该 fallback 不可用。
...
| P4-06 | 实现 Route Explain 和 Candidate 排除原因查询 | P3-06,P4-04,P4-05 | 固定输入决策快照 | LOCAL_PASS_PENDING_CI |
