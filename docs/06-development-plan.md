...
> **2026-09-26 当前状态与后续计划索引：** 最新发布、数据与登录态验收事实以[当前状态清单](reports/cpar-current-status-20260926.md)为准，避免在此重复维护已经过期的生产版本。用户新确认的[可靠性与 CPA 行为对齐 M0–M4 计划](handoffs/cpar-reliability-alignment-plan-20260926.md)、[决策记录](handoffs/cpar-reliability-alignment-decisions-20260926.md)和[验收清单](handoffs/cpar-reliability-alignment-acceptance-20260926.md)已落盘，功能实施尚未启动。金额硬预算及独立客户端 RPM 新增限额延期；新计划单独获准最多12次受控上游推理尝试，旧额度不重置。以下旧阶段状态保留历史口径，不据此宣称当前全部能力已完成。

> **2026-09-20 当前执行索引：** 用户已通过讨论确认 [CPAR 剩余开发与前端精修计划](handoffs/prism-remaining-development-plan-20260920.md)。本轮收口现有 Prism Goal，旧 P13 未完成能力另行核实登记，不自动并入。下方 `v1.316 / 2026-09-03` 元数据与变更记录保留当时事实，不代表最新生产或前端进度。旧“发现模型后自动加入授权路由”行为已被后续“完整目录＋管理员手动开放＋密钥显式授权”决策替代；当前实现与证据见新计划及其审计引用。此次仅同步文档索引，不标记代码、验收或发布完成。
...
| 当前阶段 | `P1` 至 `P6`、P9、P10、P11 与 CPAR 侧 P12 已完成（P12 为 `DONE_WITH_BOUNDARY`）；P13-04、P13-05、P13-06A/B/C、P13-07A/B/C/D、P13-08、P13-09A/B/C、P13-10A、P13-11A/B/C/D、P13-11E E0-E3 与独立 P13-11E4 均已通过对应正式 Delivery Gate 并以 `DONE_WITH_BOUNDARY` 收口。P13-11E4 由 `phase-p13-provider-egress-status-complete` / exact `ce98faa9306d076f5af53b9eef0c818abb1cb9c8` / [Gate 32110872875](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/32110872875) 正式验收；P13-11 umbrella 因所有已授权切片均已验收而收口为 `DONE_WITH_BOUNDARY`，E5 真实网络仍作为独立 `DEFERRED_UNAUTHORIZED` 边界，不占用活动任务状态。P13 umbrella 因 P13-12、P13-15 与新增 P13-16 正在实施而继续 `IN_PROGRESS`。P7 Kiro OAuth、P8 Official API-key E2E 与 Grok Web 外部 egress/真实网络边界继续延后。Autoreg 的注册、首次登录/授权、refresh grant 撤销后的交互 reauth、账号修复和 replenishment 不属于 CPAR/P12；CPAR 对已导入且支持 exact-channel refresh 的 OAuth 日常自动续期，以及 entitlement observation、credential pool Health/Quota/Circuit、lease、cooldown、failure feedback、调度与上游模型目录仍属于 CPAR，按 `CR-P12-AUTOREG-SEPARATION-001/002`、`CR-P13-RUNTIME-OAUTH-REFRESH-001`、`CR-P13-ACCOUNT-TIER-001` 与 `CR-P13-UPSTREAM-MODEL-CATALOG-001` 解释。 |
| 当前任务 | `P13-15 remaining sources and P13-15E isolation/formal Gate`：C/D exact commit `058805e556d5b22a00bd56b846f75ed8b81696fd` 已部署 Oracle Singapore；Build catalog 自动把 `grok-4.6` 加入授权 `/v1/models`，唯一一次真实 non-streaming Responses 返回 `200/completed`，schema 21、SQLite 与 rollback invariants 均通过。下一步补 Grok Web/Console、Official、Kiro、generic compatible 等 remaining source owner 与 P13-15E 多 target/多 Credential 隔离矩阵，再做 Claude Code 接线和正式 Delivery Gate。两个生产 Codex Credential 仍为 `free` 且 refresh grant 已失效；Luna 的 production acceptance 等 operator 对目标 Go 账号重新 OAuth，不得用套餐推断或前端常量代替。 |
| 本次计划变更（2026-09-03，P13-15C/D Oracle production acceptance） | exact implementation `058805e556d5b22a00bd56b846f75ed8b81696fd` 的 release workflow [33651468181](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/33651468181) 两个 Linux arch job 成功，签名与 OCI metadata 校验后部署 ARM64 SHA-256 `69acbe01c8aa4934867f473e6eeaa063185eeae364865d8730fd88f8ab6344af`。切换前完成 SQLite/旧 binary/release pointer 备份；仅重启 CPAR。schema 21、quick check、foreign keys、loopback/public health 均通过；授权 `/v1/models` 返回四个 exact ID 并包含动态物化的 `grok-4.6`；protected catalog 为 1 Fresh/3 Missing，且唯一一次真实 non-streaming `grok-4.6` Responses 返回 `200/completed`。未输出凭据或模型内容，未修改 Autoreg/Caddy/Cloudflare/DNS/frontend/其他服务；rollback 未触发。remaining channel sources、target Go/Codex evidence、P13-15E isolation、Claude Code 与 formal Gate 仍待完成。 | `P13-15C/D PRODUCTION_PASS_FOR_GROK_BUILD`; P13-15 `IN_PROGRESS` |
| 本次计划变更（2026-09-02，P13-15C/D durable catalog + exact Credential materialization） | 新增 SQLite migration 0021，成功目录按 Config Version/Endpoint/Credential 单调版本化，使用 Fresh 6h、refresh due 24h、Expired 72h 与连续三次成功遗漏 + 24h 删除隔离；失败只记录 safe class 并保留最后成功。Build/Codex worker 在 listener 后立即运行并每小时检查，仅到 refresh due 才发送；目录从 active Config Snapshot 原子物化 exact upstream ID，Candidate 只允许其目录列出的 Credential，Expired/ambiguity fail closed。HTTP 鉴权固定的 immutable snapshot 一直携带到 scheduler selection，不能跨 publication 混用。`GET /admin/catalog/status` 增加 optional version/refresh/model-count/failure fields，OpenAPI 变化已要求 Claude Code sync。Gateway `120/120`（最终以本次报告为准）、HTTP/router/Build/Codex focused suites 通过；Oracle deploy、production `grok-4.6`、remaining B channels、P13-15E 与 formal Gate 待完成。 | `P13-15C/D LOCAL_PASS_PENDING_PHASE_GATE`; P13-15 `IN_PROGRESS` |
| 本次计划变更（2026-09-02，P13-16A production refresh + 真实模型缺口确认） | commits `e0b2d82b13a9363fbad3e71ec994f2f8e1ea58fc` / `388e156cdf8c4c0693ecaed02ddd772f89e03962` 已推送 GitHub，最终 Linux aarch64 artifact SHA-256 `4691d6b34dd09f8eff101f7e65f3ad5094bc54b8e1ae39faee8b0da6b095b5fc` 已安装到 Oracle Singapore。启动验收中 Build `claimed=2/succeeded=1/reauth_required=1`，成功记录 revision 升为 `1`；公网 Pi Client Key 对 `grok-4.5` 的 non-streaming Responses 返回 `200/completed/CPAR_OK`。两个旧 Codex grant 刷新失败，生产 metadata 均为 `free`，因此不得声称已有 Go 或已经可路由 Luna；失败已增加指数退避，有效 grant 仍由 CPAR 自动刷新，revoked grant 需 operator/Autoreg 交互 reauth。公开 catalog 实测仍为 `gpt-5.6-terra`/`grok-4.20-0309`/`grok-4.5`，证明 P13-15C/D 不能被误报为完成。 | `P13-16A LOCAL_PASS_PENDING_PHASE_GATE`（Build production pass；Codex reauth required）；`P13-15C/D IN_PROGRESS`; P13 `IN_PROGRESS` |
| 本次计划变更（2026-09-02，P13-15B Build/Codex source + P13-16A runtime OAuth refresh） | 用户纠正：所有已保存到 CPAR 且 Provider 明确支持 refresh 的 OAuth 应由 CPAR 日常自动续期；Autoreg 只负责注册、首次授权、补池与 revoked grant 的交互 reauth。新增 [CR-P13-RUNTIME-OAUTH-REFRESH-001](change-requests/CR-P13-RUNTIME-OAUTH-REFRESH-001.md) 与 P13-16。实现启动前 catch-up、listener 后每分钟 worker、Build-only claim、exact official Codex binding、encrypted revision CAS、atomic runtime material replacement 与 in-flight lease pinning；API key/SSO cookie 不伪 refresh。P13-15B 新增 Build/Codex exact-Credential catalog source，真实目录证明 `grok-4.6` 与 `gpt-5.6-luna` 等遗漏项；模型 ID 原样保留且不按套餐硬编码。Gateway `118/118`、相关 crates 全量、strict Clippy、fmt/diff 全通过；Oracle deployment、P13-15C/D 与其他渠道仍待完成，Prism/OpenAPI 未改。 | `P13-15B PARTIAL_LOCAL_PASS`; `P13-16A LOCAL_PASS_PENDING_PRODUCTION`; P13 `IN_PROGRESS` |
| 本次计划变更（2026-09-02，P13-15A 生产验证与 Responses 纠正） | revision `ac23f387bb71d2494d6b2d999096e2bd1c9c5d1e` 已安装为校验过的 Linux aarch64 release。修复 native Grok durable revision 0 到 runtime revision 1 的 one-based 投影，避免 Console 空状态连带拒绝 Build runtime；同协议 OpenAI Responses 允许 typed `prompt_cache_key`/retention 进入 Build adapter，由 adapter 执行 Client-Key/模型隔离 HMAC，跨协议仍 fail closed。生产 exact catalog 为 `gpt-5.6-terra`、`grok-4.20-0309`、`grok-4.5`，旧 alias 缺席；真实 Build JSON Responses、Pi streaming Responses、Provider egress 与 SQLite/health 均通过。Pi 与 CC Switch 保存 exact `grok-4.5`、`openai-responses`、`/v1`。P13-15A 达到 production live pass，但 B-D 动态 discovery/freshness/materialization 与 E 剩余隔离/formal Gate 未完成。 | `P13-15A PRODUCTION_LIVE_PASS`; next `P13-15B`; P13-15 `IN_PROGRESS` |
| 本次计划变更（2026-09-01，全渠道 upstream model catalog） | 用户明确纠正所有反代渠道都应透传上游模型列表，不应由 CPAR 自定义公开模型名。批准 [CR-P13-UPSTREAM-MODEL-CATALOG-001](change-requests/CR-P13-UPSTREAM-MODEL-CATALOG-001.md) 并新增 P13-15：exact upstream ID、Provider/channel/Endpoint/Credential discovery 隔离、Client-Key 授权 union、同模型 exact candidate lease、staleness/removal 与 collision fail-closed。P13-15A 的 Snapshot/list/request boundary 已本地通过；它只投影 compiler-retained Candidate 原值，尚不能代替 B-E 的真实 discovery 与 Credential-scoped materialization，因此总项保持 `IN_PROGRESS`。 | `P13-15A LOCAL_PASS_PENDING_PRODUCTION`; P13-15 B-E planned |
| 本次计划变更（2026-09-01，P13-12 account entitlement 启动） | 用户明确纠正 Grok `free/supergrok/heavy` 只属于 Build，Web 与 Console 必须单独建模；并要求 Autoreg 不占用 CPAR 开发阶段。批准 [CR-P13-ACCOUNT-TIER-001](change-requests/CR-P13-ACCOUNT-TIER-001.md)：原 P13-12 Autoreg handoff 行移出 CPAR phase sequence，P13-12 改为账号 entitlement。管理投影源不可用/快照失效的遗留映射已先修为 safe `503 management_runtime_unavailable` 并有 HTTP 回归。当前实现不改 `web/prism/**`；authoritative OpenAPI 变化后必须在 cross-boundary log 留给 Claude Code。 | `P13-12 LOCAL_PASS_PENDING_PHASE_GATE`（生产回填先于 Gate） |
| 本次计划变更（2026-08-19，公开主线与部署产品化审计） | 完成后端完成度、GitHub 推送/CI 触发与全部 `59` 个本地/远端分支的只读审计：已批准后端到 P13-11E4 均已推送并正式 Gate，但停留在 `codex/p13-11-egress`/当前 integration line，默认 `main` 尚未合并；49 个历史分支已是 main 祖先，P13 后续分支除旧 P13-08 外均由 `claude/route-candidates` 覆盖，禁止逐分支盲目 merge。新增独立中英文 README、后端完成度/分支审计、双语多架构部署指南、Linux amd64/arm64 source-build Dockerfile 与 host-network Compose；明确当前无公开 GHCR/GitHub Release，macOS 仅开发、Windows 经 WSL2、musl/ARMv7/Kubernetes 未验收。GitHub 当前无 open PR、ruleset 或 classic main protection；先等待 Claude Code 完成 `web/prism/**`，再配置 main protection，并在唯一 integration PR 上执行 reconciliation、Full review 和最终 Gate。本变更不启动 E5、不调用 Provider/代理/DNS/服务器/生产 | backend through P13-11E4 `DONE_WITH_BOUNDARY`; public-main/release integration `IN_PROGRESS`; E5 `DEFERRED_UNAUTHORIZED` |
| 本次计划变更（2026-08-18，P13-11E4 状态一致性与前端交接） | 将 P13 总览与优先级中的 E4 旧状态从本地 `LOCAL_PASS_PENDING_PHASE_GATE` 同步为 formal Gate 后的 `DONE_WITH_BOUNDARY`；历史 v1.304 本地收口记录保持原样，不回写当时事实。用户批准执行既有 Claude Code action-required handoff：前端只消费 generated `listProviderEgressStatus`，按 `egress`/`session`/`clearance` 分区显示，传递 exact `X-Config-Version`、保留 opaque cursor 并在 `409` 后重启读取；不得合成 overall health、增加 operator action 或把 empty Web/clearance 解释为 healthy。前端完成证据尚待 Claude Code 实现与本地 Prism gate；E5 仍未授权 | `P13-11E4 DONE_WITH_BOUNDARY`; frontend handoff `IN_PROGRESS`; E5 `DEFERRED_UNAUTHORIZED` |
| 本次计划变更（2026-08-18，P13-11E4 正式收口） | annotated tag `phase-p13-provider-egress-status-complete` 精确固定于 pushed closeout commit `ce98faa9306d076f5af53b9eef0c818abb1cb9c8`；唯一正式 Delivery Gate run [32110872875](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/32110872875) 的 Authorize（4s）、Fast（7m02s）、Full supply-chain（1m00s）与 Required（3s）全部成功。结合本地 Full `43/43`、focused evidence 与独立最终 review，P13-11E4 以 `DONE_WITH_BOUNDARY` 收口；P13-11/P13 umbrella 继续 `IN_PROGRESS`。唯一非阻断 annotation 是 pinned `actions/checkout` 与 `actions/cache` 仍声明 Node.js 20、GitHub runner 强制使用 Node.js 24；未导致失败或降低 Gate 强度。当前 source 仅 Build/Console，production Web/clearance 可空，synthetic clearance 仅 serializer evidence，generic compatible 独立；无 Provider/代理/DNS/server/staging/production/Autoreg/真实 Health 证据，E5 仍 `DEFERRED_UNAUTHORIZED`，四个既有 helper 未触碰 | `P13-11E4 DONE_WITH_BOUNDARY`；P13-11 `IN_PROGRESS` |
| 本次计划变更（2026-08-18，P13-11E4 本地实现收口） | 完成 atomic egress/session/clearance runtime snapshot、monotonic runtime revision、Config-lineaged retained page/cursor、strict Provider/channel/domain source shape、protected `GET /admin/operations/provider-egress-status`、authoritative OpenAPI/Prism/generated client 和 Claude Code action-required handoff。定向证据：gateway-router `170/170`、gateway-control `87/87`、gateway bin `114/114`、E4 HTTP `4/4`、management OpenAPI `12/12`、existing management runtime `3/3`；provider-grok 全量套件通过（authorized/live tests 按边界 intentionally ignored，E3 Web 历史证据 `11/11`）；gateway-control/gateway-router/gateway-http-actix/gateway/provider-grok strict Clippy、fmt、source policy、secret、Prism 与 Full `43/43` 通过；独立最终 review 无剩余 P0-P3 finding。aggregate receipt/review 已冻结，formal Gate 尚未运行。只投影已组合 Build/Console；production Web/clearance 可空，synthetic HTTP clearance 不是真实 Web 证据，generic compatible 继续独立。无 Provider/代理/DNS/server/production/Autoreg/真实 Health 证据；E5 `DEFERRED_UNAUTHORIZED` | `P13-11E4 READY_FOR_FORMAL_DELIVERY_GATE`；P13-11 `IN_PROGRESS` |
| 本次计划变更（2026-08-18，P13-11E4 启动） | 用户明确批准启动 E4。新增 [E4 CR](change-requests/CR-P13-11E4-PROVIDER-EGRESS-STATUS-001.md)、[ADR-0098](adr/ADR-0098-provider-specific-egress-management-projection.md)、[BC-MGMT-019](contracts/BC-MGMT-019-provider-specific-egress-runtime-status.md) 与 [E4 report](reports/p13-11e4-provider-egress-management-projection.md)，冻结 egress/session/clearance 三个独立 source-domain row、Config/runtime 双 revision、固定 observation snapshot、稳定 keyset cursor、Build/Console-first source、empty Web/clearance 与 generic-source isolation。当前 governance slice 不改 OpenAPI/Prism/cross-boundary log；真正增加 GET 时必须 authoritative OpenAPI -> Prism sync/generated client -> Claude Code handoff。无 recovery、Provider/代理/DNS/server/staging/production；E5 仍未授权。 |
| 本次计划变更（2026-08-18，P13-11E 正式收口） | 用户明确批准后，annotated tag `phase-p13-provider-egress-complete` 精确固定于 pushed closeout commit `ba2261a5414fe73d147a102a266abd3e9a7fbb5b`；唯一正式 Delivery Gate run [32044424886](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/32044424886) 的 Authorize（3s）、Fast（6m41s）、Full supply-chain（1m03s）与 Required（3s）全部成功。结合本地 Full `43/43`、phase review、ADR-0097、BC-SEC-008 与 E0-E3 reports，P13-11E E0-E3 以 `DONE_WITH_BOUNDARY` 收口；P13-11 umbrella 仍 `IN_PROGRESS`。唯一非阻断 annotation 是 pinned `actions/checkout` 目标 Node.js 20 而 GitHub 强制使用 Node.js 24，未导致失败或降低 Gate 强度。Build/Console 仍为 typed Direct，Web 仍 transport-free；E4 `DEFERRED_OPTIONAL`、E5 `DEFERRED_UNAUTHORIZED`，没有 Provider/代理/DNS/server/staging/production 证据，无 OpenAPI/Prism/frontend 变化。 |
| 本次计划变更（2026-08-17，P13-11E aggregate closeout candidate） | E0-E3 目标回溯 review 无剩余 P1/P2 blocker；[aggregate Full receipt](reports/evidence/p13-11e-aggregate-full-20260817.md) 在 exact implementation baseline `7c182af13912a72be304448308dba7fda963ec82` 上记录本地 `43/43` PASS，[phase review](reports/evidence/p13-11e-phase-review-20260817.md) 将状态冻结为 `READY_FOR_FORMAL_DELIVERY_GATE`。Build/Console 当前只证明 typed Direct Provider-local seam，Web 仍为 transport-free seam，不声称 native proxy pool、真实账号、Statsig/clearance/FlareSolverr、DNS、公网或生产可用。E4 明确为 `DEFERRED_OPTIONAL`，E5 未授权；本批无 OpenAPI/Prism/frontend 变化，不新增 Claude Code handoff。正式 tag/Gate 尚未创建，四个既有 helper 未触碰。 |
| 本次计划变更（2026-08-17，P13-11E E3） | 完成 Grok Web transport-free sticky egress/session/clearance seam：exact `grok_web_sso` lease、named target、session 与 clearance identity 在任何计数前 fail-closed 校验；新增 generation-owned atomic clearance singleflight，防 live-owner overwrite、stale/same-deadline ABA 与跨 lineage completion；Statsig/clearance/inference 使用 bounded ledger，unknown/confirmed/challenge `403` 分离，challenge 仅供下一逻辑 attempt 恢复且不在当前 attempt 重发。Web `11/11`、clearance state `13/13`、provider-grok/router regressions、strict Clippy/fmt/docs/diff 通过；没有 Autoreg、Provider/代理/DNS/Store/FlareSolverr/server/staging/production，也无 OpenAPI/Prism/frontend 变化。下一步为 E0-E3 aggregate review；E4/E5 不自动启动。 |
| 本次计划变更（2026-08-17，P13-11E E0 计划/CR/review） | 用户批准启动 E0。新增 [CR-P13-11-PROVIDER-SPECIFIC-EGRESS-001](change-requests/CR-P13-11-PROVIDER-SPECIFIC-EGRESS-001.md)、[ADR-0097](adr/ADR-0097-provider-specific-egress-health-recovery.md)、[BC-SEC-008](contracts/BC-SEC-008-provider-specific-egress-health-recovery.md) 与 [P13-11E planning report](reports/p13-11e-provider-specific-egress-planning.md)，冻结 generic compatible、Grok Build、Grok Console、Grok Web、Official、Kiro/Claude/other Provider 的 egress/session/clearance/credential/quota 状态域、failure ownership、bounded recovery 和 no-fallback。E0 不改 Rust、OpenAPI、Prism、前端或运行时，不调用 Provider/代理/DNS；下一片是 E1 synthetic typed state。 |
| 本次计划变更（2026-08-17，P13-11E E2） | 完成 Grok Build/Console native adapter seam：复用现有 CPAR native account-pool、exact `CredentialLease`、Build/Console adapter transport 和 Router lease 生命周期；新增 provider-local egress attempt ledger，Build 在 egress admission/request construction 后记一次 inference，Console 将 DPoP/bootstrap 计入 auxiliary、401 仅标记 exact session `challenge_required` 并停止 E2 内部 refresh/second inference。新增 `p13_11e_native_egress` `4/4`，provider-grok 全量、gateway-router `158/158`、gateway `109/109`、strict Clippy/fmt/diff 通过；无 Autoreg、Provider/代理/DNS/Store/server/staging/production、无 OpenAPI/Prism/frontend 变化。E2 状态为 `LOCAL_PASS_PENDING_PHASE_GATE`，下一片 E3 Grok Web fake-only seam；真实网络仍需 E5 新 CR。 |
| 本次计划变更（2026-08-17，P13-11D closeout candidate） | D1/D2/D3 与 aggregate 目标回溯 review 无剩余 P1/P2 blocker；[phase review](reports/evidence/p13-11d-phase-review-20260817.md) 与 [aggregate Full receipt](reports/evidence/p13-11d-aggregate-full-20260817.md) 已冻结边界。新 tag `phase-p13-egress-management-complete` 仅指向包含 review 的 exact pushed closeout commit，并只触发一次正式 Delivery Gate；旧 `phase-p13-egress-complete` 保持不变。用户已批准继续正式 Gate；Gate 成功前仍不得启动下一任务或宣称 D 完成。 |
| 本次计划变更（2026-08-17，P13-11D 正式收口） | 新 annotated tag `phase-p13-egress-management-complete` 固定到 exact commit `1beb230248fb75ced146b87c547eb020ee9cd010`；唯一正式 Delivery Gate run [31996324578](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/31996324578) 的 Authorize（2s）、Fast（7m03s）、Full supply-chain（1m22s）和 Required（3s）全部成功。P13-11D 及 D aggregate 统一为 `DONE_WITH_BOUNDARY`；旧 `phase-p13-egress-complete` 仍固定于 `a716eaaa9d31c26b6d09489f3f7fdbb9b0e1ebeb`。不声称真实代理/Provider/DNS/生产验证；P13-11 umbrella 因后续 Provider-specific egress backlog 继续 `IN_PROGRESS`。 |
| 本次计划变更（2026-08-17，P13-11D3） | 完成 active runtime composition：新增 exact Config-Version/Upstream/pool/node AAD open+revalidation，将持久化 fixed/pool node 与 binding policy 编译进 P13-11B/C 已有 registry/lease/serving path；enabled empty pool、foreign/native owner、orphaned exact binding、wrong AAD、unsafe plaintext、unknown target 与 drift 在 publication 前 fail closed。Gateway 109、Control 77、Router 151、Upstream 37，strict Clippy/fmt/docs/secret/diff review 通过；清理仅限本仓库 `target/`，四个既有 untracked helper 保持未触碰。D3 不改变 management OpenAPI/Prism，不新增 Claude Code 接口任务；未调用 Provider/代理/DNS/服务器/生产。 |
| 本次计划变更（2026-08-17，P13-11D2） | D2 已在 D1 基础上接通 Config-Version/Management Key/CSRF/If-Match/revision/audit 保护的 pool、node、exact binding-profile 管理接口；proxy endpoint 仅 write-only，立即校验并 AEAD 保存，更新省略值时保留既有密文，成功 revisioned 响应带 `Cache-Control: no-store`。`docs/openapi/management-v1.json` 已新增 15 个 P13-11D2 operation，Prism contract/client 已同步，`p10_01`/`p10_04` 回归、Prism check、generated-client freshness 与 management SPA check 通过；cross-boundary log 已提醒 Claude Code。D2 仍是 `LOCAL_PASS_PENDING_PHASE_GATE`，未运行正式 Gate；下一片为 D3 active runtime composition。 |
...
| 本次计划变更（2026-08-15，续） | 完成 `P13-07B` 本地实现与 review：`RouteCredentialScheduler` 暴露只读 provider-scoped explain seam，管理 Route Explain 复用 serving 使用的同一 scheduler/pools/Health/Quota；expiry 以 `expires_at_ms <= observed_at` fail closed，精确 quota 缺失保持 Unknown；显式 `provider_id` 可限定范围，多 Provider 未指定范围时不隐式选择，foreign Provider 映射为 `provider_mismatch`。OpenAPI 与 Prism contract/generated client 已同步，`docs/cross-boundary-log.md` 已通知 Claude Code 增加 Provider 选择器和安全 reason 展示。该切片不获取 lease、不发送 Provider 请求、不改变公开 serving 请求路径、不改 production 或运行正式 UI；下一片为 P13-07C serving-path lease revalidation。 |
| 本次计划变更（2026-08-15，再续） | 完成 `P13-07C` serving-path lease revalidation：P13-07A advisory ordering 已接入真实 request-time AttemptOrchestrator；同一 RouteCredentialScheduler/EndpointCredentialPools/Health/Quota registry 在原子 lease 前重新校验 Provider/channel/candidate、hard eligibility、binding、expiry、Health、Quota 与 capacity。独立 review 找到并修复 quota-recovery fallback 未携带 Provider scope 的阻断项，新增 foreign hard-ineligible recovery-due 零 lease/零 driver 与 same-Provider recovery 正向回归。Router 121、Gateway 100、HTTP 116 passed（4 ignored），strict Clippy/fmt/source/crate-boundary/diff 通过；无公开协议/OpenAPI/Prism/frontend 变化，不调用 Provider、不改 production/server/traffic。P13-07 仍 `IN_PROGRESS`，下一步先冻结 versioned cost signal 边界。 |
| 本次计划变更（2026-08-15，P13-07D） | 启动 `P13-07D`：P13-05 catalog 是六维每百万 token rate，而请求开始时没有最终 Usage，故禁止把单维 rate、总和、历史值、body bytes 或 max output 伪装为 request cost。新增 Config-Version-owned exact catalog binding 与闭合 `rate_dominance_v1`；serving/Explain 只在一个 known vector 同时达到六维最小时认定 dominance，交叉 rate 保持 incomparable，missing entry 保持 unpriced，显式全零才是 known zero。catalog 只在 composition 编译为共享 immutable map，hot path 不读 Store/调用 Provider。该管理合同变化先改 authoritative OpenAPI、sync Prism generated contract，并在 cross-boundary log 通知 Claude Code；不实现正式 UI、不做 production/staging canary。 |
| 本次计划变更（2026-08-15，P13-07 正式收口） | `phase-p13-routing-complete` 在 exact commit `0c338ee8eef76e470c55515a24728324684365c5` 上触发唯一一次正式 Delivery Gate；run [31875826495](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/31875826495) 的 Authorize（3s）、Fast（5m57s）、Full supply-chain（1m16s）与 Required（2s）全部 PASS。结合本地 43/43 Full preflight 与独立 phase review，P13-07A/B/C/D 及 P13-07 aggregate 以 `DONE_WITH_BOUNDARY` 收口；P13 umbrella 继续 `IN_PROGRESS`，P13-08 仅列为下一候选且未启动。没有 Provider 请求、staging/production/server mutation，也没有重复运行第二次昂贵 Gate。 |
| 本次计划变更（2026-08-16，P13-09 正式收口） | `phase-p13-responses-complete` 在 exact commit `d419c4678bd2ff563046849cef800c1985d48688` 上触发 P13-09 唯一一次正式 Delivery Gate；run [31922870604](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/31922870604) 的 Authorize（2s）、Fast（6m10s）、Full supply-chain（59s）与 Required（3s）全部 PASS。结合本地 43/43 Full preflight 与 phase review，P13-09A/B/C 及 aggregate 以 `DONE_WITH_BOUNDARY` 收口；P13 umbrella 继续 `IN_PROGRESS`，P13-08 仍单独等待 phase boundary，P13-10 未启动。没有 Provider 请求、management OpenAPI/Prism 变化、staging/production/server mutation，也没有重复运行第二次昂贵 Gate。 |
| 本次计划变更（2026-08-16，P13-10A） | 用户明确要求 WebSocket 后启动公共 Responses WebSocket 第一片：`GET /v1/responses` 在 upgrade 前复用 exact Client Key auth，接收 text-only flat `response.create` 并通过既有 Responses decoder/Canonical executor/lease/usage/stored continuity 输出同一 lifecycle JSON events。新增显式 `responses_websocket` capability；downstream WebSocket 不要求 Provider-native upstream WebSocket。固定 4-MiB frame/message、64 fragments、4096 events、8-MiB turn capture/output、one-active+one-pending、write/liveness/turn/session deadlines与 RFC 6455 close reasons；disconnect 必须取消 source/释放 lease。`response.append`、Realtime、Chat/Messages WS、browser Origin、binary/media 与 upstream-native WS 延后。只跑本地适用 Gate/review，不调用 Provider、不改 production/staging/server；public client surface 已在 cross-boundary log 提醒 Claude Code，management OpenAPI/Prism 不变。 |
| 本次计划变更（2026-08-16，P13-10A 本地收口） | 完成 strict `response.create` decoder、authenticated no-Origin RFC 6455 upgrade、JSON lifecycle projection、same-session/durable exact continuity、one-active+one-pending、fragment/UTF-8/size/close/backpressure/deadline/cancel 和 response-ID collision fail-closed；runtime candidate 必须显式 `responses_websocket`，HTTP JSON/SSE 不变。protocol 30、catalog 15、router 138、gateway 106、HTTP 65 及 workspace Fast/supply-chain/strict Clippy/source/crate/docs/secret review 通过；Fast 首轮仅发现并修正 WebSocket runtime/test dependency 的 crate-boundary allow-list，随后 boundary/tail 全部通过。未调用 Provider、不改 management OpenAPI/Prism/production/staging/server，也未运行正式 GitHub Delivery Gate。 |
| 本次计划变更（2026-08-16，P13-10 closeout candidate） | exact implementation commit `fd1e21fc398c732763108bbe12036fa3999818f0` 上运行 `CHECK_REPORT_PATH=/tmp/cpar-p13-10-phase-preflight-20260816.md ./scripts/check.sh full`，Darwin arm64 从 `04:26:59Z` 至 `04:28:58Z` 的 43/43 steps 全部 PASS；phase review 无剩余 P1/P2 blocker。只允许把 annotated `phase-p13-websocket-complete` 固定到包含两份 receipt 的 exact pushed commit，并运行一次正式 Delivery Gate；不调用 Provider、不改 management OpenAPI/Prism、server/production/staging 或四个 untracked helper，P13-11 仍阻塞。 |
| 本次计划变更（2026-08-16，P13-10 正式收口） | `phase-p13-websocket-complete` 在 exact commit `dc48ec40e4fb38961925f203bf3cd0f7434a34a0` 上触发 P13-10 唯一一次正式 Delivery Gate；run [31926927914](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/31926927914) 的 Authorize（4s）、Fast（5m51s）、Full supply-chain（1m33s）与 Required（3s）全部 PASS。结合本地 43/43 Full 与 phase review，P13-10A 及 aggregate 以 `DONE_WITH_BOUNDARY` 收口；P13 umbrella 继续 `IN_PROGRESS`，P13-08 仍单独等待 phase boundary，P13-11 未启动。没有 Provider 请求、management OpenAPI/Prism 变化、staging/production/server mutation，也没有重复运行第二个昂贵 Gate。 |
| 本次计划变更（2026-08-16，P13-08 正式收口） | 从 pure Channel Pin implementation `b6a7085ed69144f395d3b5579541844fabf424e9` 建立独立 retrospective Gate line；首次 Full 发现并以一行 `read_context` corrective commit `04a8d315750ae2eb08a3f7d6f4fd72efbb101144` 修复 billing catalog GET 回归，未把 P13-09A 算入标签。最终本地 Full 43/43 PASS；`phase-p13-channel-pin-complete` 固定到 exact commit `7e14a2733c461d04198a6413efda420a03545eea`，唯一正式 run [31928169486](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/31928169486) 的 Authorize（5s）、Fast（6m55s）、Full supply-chain（1m9s）与 Required（2s）全部 PASS。P13-08 以 `DONE_WITH_BOUNDARY` 收口；未调用 Provider、未改 production/staging/server、未重复运行第二个 Gate。 |
...
| 已批准变更（续 15） | `CR-P12-10H-003`：用户要求把“调用测试”提升为直接 CPAR HTTP E2E；新增 value-free curl harness，使用 root-only endpoint/client-key/model 输入，先做认证 `/v1/models` 与 Grok route preflight，再最多发送 100 次 Chat/Responses/Messages JSON/SSE 请求，首败停止。当前无可见 Grok route 时必须零推理发送并记录 `BLOCKED_NO_GROK_ROUTE`；不得把其它渠道结果计为 Grok。 |
| 已批准变更（续 16） | `CR-P12-10H-004`：native Grok route 绑定与请求/选择/回退/凭证解包逻辑必须以冻结 grok2api `v3.0.10` 源码为行为基线；CPAR 只将这些语义映射到现有 account pool、Health/Quota、Canonical 和 config-version 边界，不新增独立 selector 或隐式 fallback。Console 迁移 envelope 仅按源项目规则提取 `sso_token`，未知字段和无效 `probe_model` fail closed；先完成本地 parity review，再进入隔离 staging HTTP E2E。生产图、旧 CPA、CC Switch、Caddy/DNS 与 grok2api 服务均不变。 |
| 已批准变更（续 17） | `CR-P12-10H-006`：用户要求后续 CPAR 渠道验收必须走真实公共数据面；每个 live channel/provider matrix 必须通过 CPAR base URL 与 client key 的 `curl`（或等价真实 HTTP 客户端）完成认证 `/v1/models`、目标路由预检以及已支持协议的 JSON/SSE 请求。native provider probe、loopback synthetic 和 fixture 只能作为补充证据，不能替代 public HTTP E2E；预检失败必须零推理发送、首败停止、无跨渠道归因。 |
| 已批准变更（P12-10H） | `CR-P12-10H-007`：在隔离 staging 绑定 native Console route 后，按 `CR-P12-10H-006` 用 CPAR base URL + client key 重新执行公共 HTTP matrix；最多 100 次、每个协议/模式首败停止，不重试或跨渠道 fallback。只记录固定类别和计数。将公共 Responses output-limit 扩展窄化映射到 Console 请求；未知扩展仍拒绝。生产图、Caddy/DNS、CC Switch、grok2api 和公开流量不变。 |
| 已批准变更（P12-10H） | `CR-P12-10H-008`：按用户要求在隔离 staging 导入 grok2api 中当前满足既有安全迁移条件的全部 Console/Build 账号，使用 CPAR pool 轮转，每个 eligible credential 最多一次真实公共请求，不重试、不跨 Provider fallback；首轮 activation-only `RouteNotFound` 不计为 upstream attempt，完成重启后才计入最终 sweep。源侧缺失模型、过期或 `reauthRequired` 继续 fail closed，不猜测、不强制导入。 |
| 已批准变更（P12-10I） | `CR-P12-10I-001`：用户要求把雅加达 `grok-register` 的自动 reauth 能力复用到 CPAR。以该项目的 refresh-first、交互式 OAuth/浏览器 SSO fallback、串行批处理、逐账号结果和立即导入行为为 clean-room 参考；CPAR 先交付 native reauth state、单账号 CAS 加密替换、最多 200 条/批且并发固定为 1 的 coordinator，以及 Device Code/外部浏览器 bridge 的内存契约。不得把 grok2api 作为最终上游，不保存密码/SSO/Bearer，不并发打开授权窗口，不在本 Task 触发真实 OAuth、生产图或服务器变更。 |
| 已批准变更（P12-10I） | `CR-P12-10I-002`：用户要求找少量账号完成 OAuth/SSO 后测试 CPAR 的 Console 与 Build。限定最多 2 个 Build OAuth 账号、2 个 Console SSO 账号，所有授权窗口和刷新操作固定串行；Build 只允许 refresh-token/device OAuth，Console 只允许独立 SSO/browser bridge，不混用凭证。成功账号仅进入 Oracle Singapore 独立 loopback staging，使用 CPAR base URL + client key 做有界 JSON/SSE 请求；单账号首败停止，不重试、不跨 Provider fallback。仅允许更新所选账号的 OAuth/SSO 状态，不写入旧 CPA、grok2api、CC Switch 或生产 CPAR；所有凭证只在远端内存/临时受控管道中传递，不进入本机、日志、receipt 或 Git。 |
| 已批准变更（P12-10I） | `CR-P12-10I-003`：记录 P12-10I-02 的受控真实结果：Build 2/2 refresh 失败即停止；Console 2 个 SSO 导入后仅做 `/v1/models` 与第一条 Responses JSON，`EgressRejected/egress` 首败后不发送 SSE；允许 staging 回滚/清理与生产不变性复核。该 CR 不授权重试失败 OAuth、修复 egress 或进入 P12-10I-03。 |
| 已批准变更（P12-10I） | `CR-P12-10I-004`：用户授权开始下一步。复用 Jakarta `grok-register` 的 SSO→Device OAuth 转换，最多 5 个新 SSO、固定串行且单次尝试；只记录固定步骤/状态类别，凭据仅在远端内存与受控管道流转。若得到新的 Build OAuth，仅进入 Oracle Singapore 独立 loopback staging，并通过 CPAR base URL + client key 做首败 JSON/SSE 验证；同时先做 Console egress 只读分类。不得重放 P12-10I-02 的失败 refresh tuple，不改变生产、旧 CPA、grok2api、CC Switch、Caddy/DNS 或公开流量。 |
| 已批准变更（P12-10I） | `CR-P12-10I-005`：用户授权追加 5 个全新 SSO 转换探针。候选按源 map 固定顺序取 P12-10I-03 之后的记录，串行、每个只尝试一次、不重试旧候选；只记录 Device OAuth 步骤类别，不落盘或输出任何凭据。只有成功产生新的 refresh 才可进入后续 Build staging，生产、旧 CPA、grok2api、CC Switch、Caddy/DNS 与公开流量保持不变。 |
| 已批准变更（P12-10I） | `CR-P12-10I-006`：按用户要求复用当前本机已运行的 Grok CLI 会话做一次受控可用性探针。仅允许既有活跃会话、单轮固定最小提示、禁止工具/子代理/重试；只记录退出状态、固定响应类别与超时类别，不读取或导出 Bearer/OAuth/账号身份，不把 CLI 会话 token 当作 CPAR Build refresh，不写入 CPAR、服务器、grok2api、旧 CPA、CC Switch、Caddy/DNS 或公开流量。该探针成功也只证明本机 CLI 会话可用，不能替代 CPAR base URL + client key 的真实 Grok E2E。 |
| 已批准变更（P12-10I） | `CR-P12-10I-006`：按用户要求复用当前本机已运行的 Grok CLI 会话做一次受控可用性探针。仅允许既有活跃会话、单轮固定最小提示、禁止工具/子代理/重试；只记录退出状态、固定响应类别与超时类别，不读取或导出 Bearer/OAuth/账号身份，不把 CLI 会话 token 当作 CPAR Build refresh，不写入 CPAR、服务器、grok2api、旧 CPA、CC Switch、Caddy/DNS 或公开流量。该探针成功也只证明本机 CLI 会话可用，不能替代 CPAR base URL + client key 的真实 Grok E2E。 |
| 已批准变更（P12-10I） | `CR-P12-10I-007`：用户完成一次浏览器登录并批准 Device OAuth；允许单个 Build 账号进入 Oracle Singapore 隔离 staging，通过 CPAR base URL + client key 执行 6 请求（3 协议 × JSON/SSE）矩阵，首败停止；凭证仅经受控内存/临时管道流转，生产与旧系统不变。 |
| 已批准变更（P12-10I） | `CR-P12-10I-009`：验证 P12-10I-08 导入的单个 Build 账号。Build 仅做一次隔离数据库副本上的真实 native provider probe；Console/Web 仅把同一 Build OAuth 送入独立副本的错误 Provider shape，必须在导入前 fail closed。不得把 Build 结果计为 Console/Web 成功；active Config Version、生产图和公开 HTTP 边界不变。 |
| 已批准变更（P12-10I） | `CR-P12-10I-010`：用户要求继续使用 Autoreg SSO 验证 Console、Web、Build 转换。允许从 task-55 的 SSO 通过受控内存/管道执行一次 SSO→Build OAuth 转换，并分别以独立 provider shape 导入隔离 store；不得输出或落盘凭证，不改生产图、旧 CPA、grok2api、CC Switch、Caddy/DNS 或公开流量。发现 `grok-probe --batch` 的 whole-pool cardinality 假设后，修正为只租用选定 batch account；修复后须重建制品再做一次 Build native probe。Web expiry 继续执行 90 天上限，不得静默延长。 |
| 已批准变更（P12-10I） | `CR-P12-10I-011`：用户要求继续。为 P12-10I-10 的 batch-scoped probe 修复生成精确 SHA 的双架构私有 OIDC/Sigstore 制品；仅允许 ARM 制品进入 Oracle Singapore root-only 临时目录，以生产数据库在线副本和复制的 master key 接收一个 Jakarta Autoreg SSO→Build 内存管道记录并发送一次 native Build probe。必须形成两账号池、精确选择新 batch account、完成 rollback/clean，并证明生产 Config Version、release pointer、服务、数据库和公开路由不变。 |
| 已批准变更（P12-10I） | `CR-P12-10I-012`：用户要求开始下一步。先在 Oracle Singapore 同一出口用冻结 grok2api `v3.0.10`、生产源库在线副本与 loopback-only 临时容器执行一次 Console 参考请求；只改变副本中的候选选择/恢复状态，不改生产源库、CPAR、公开路由或服务。参考仍 403 时只登记外部同出口 parity，不猜测修改 CPAR；随后仅做 Web 180 天源期限与 90 天本地安全上限的源码/策略分析，证明可安全收窄后才允许实现和新的受控 Web 验证。 |
| 已批准变更（P12-10I） | `CR-P12-10I-022`：用户要求通过 Autoreg 重新注册新账号后继续测试 Web 与 Console。允许一次串行、单账号注册与受控导入，Web 仅使用临时一小时租约，不改变全局 90 天安全上限；两条路由均必须通过 CPAR Base URL + client key 的真实 `/v1/models` 与文本 JSON/SSE 边界，首败停止。仅进入 Oracle Singapore 隔离 staging，完成后回滚账号批次、路由、进程和临时材料；不得改变生产图、公开路由、CC Switch、旧 CPA 或 grok2api。 |
...
3. **一次普通远端交付。** 全部 Task 本地验收后，执行整合本地 full、跨 Crate/契约/Phase-specific
   ...
   Phase 分支和 tag 的单次交付事件运行 Fast + Full。tag 通过前不得开始下一 Phase 或发布；分支
   合并仍按 branch protection、required status 与协作需要执行，不把 merge 频率误当成 Gate 频率。
...
6. **缓存与单次交付配套。** Phase 分支可按需要持续 push；P5-00 仍必须先建立 default-ref
   的受版本约束 cache seed，并验证 tag 能恢复它。缓存仅缩短安装；每次仍做版本验证、`cargo deny`
   和 `cargo audit`，cache miss 必须安全重新安装并记录为性能调查项。
...
| P13 | Release 1.1 管理、账号池、路由与协议扩展 | G12 + `CR-P13-001` | 独立计划；P13-04/P13-05/P13-06A/B/C、P13-07A/B/C/D、P13-08、P13-09A/B/C、P13-10A、P13-11A/B/C/D、P13-11E E0-E3 与 P13-11E4 均已通过各自正式 Delivery Gate并以 `DONE_WITH_BOUNDARY` 收口；Provider-native WS、reauth/UI/E5 real-network backlog 继续按独立边界推进 | IN_PROGRESS |
...
| P12-10 | CPAR 原生接管所需渠道收口，完成可用文本渠道验证、回滚与运维手册；长时观察改为可选 | P12-09 | [native Grok account pool](reports/p12-10-native-grok-account-pool.md) 的 A-H 切片、迁移/回滚/三协议 parity；[G12 closeout receipt](reports/evidence/p12-g12-closeout-receipt-20260810.md) 与 [review](reports/evidence/p12-g12-closeout-review-20260810.md)；Build/Console/Codex/Krill 文本收口，Web 保留外部出口阻塞，旧 CPA 已停止/禁用且 rollback material 保留；旧 [observation](reports/p12-10-observation.md) 不再作为硬门槛 | DONE |
...
| P12-10H | grok2api 停服回滚演练、CPAR 合成观察与 HTTP E2E | [100-cycle synthetic receipt](reports/evidence/p12-10h-grok-synthetic-100-receipt-20260804.md)、[native route live E2E receipt](reports/evidence/p12-10h-grok-cpar-e2e-live-20260804.md)、[native Console route public curl receipt](reports/evidence/p12-10h-grok-console-native-route-cpar-curl-20260804.md)、[eligible-pool sweep receipt](reports/evidence/p12-10h-grok-console-build-pool-sweep-20260805.md) 与 [review](reports/evidence/p12-10h-grok-console-build-pool-sweep-review-20260805.md)：25 个不同 eligible Console credential 均到达上游并为 `EgressRejected/egress`；唯一 eligible Build credential 的真实 Responses JSON 1/1 通过；两个 pool 回滚、clean staging 与生产不变性通过 | BLOCKED_EXTERNAL_CONSOLE_403 |
| P12-10I-01 | 复用 `grok-register` 的 refresh-first reauth 行为：native 状态、串行批处理、CAS 加密替换、Device Code/外部浏览器 bridge 契约 | [native reauth local receipt](reports/evidence/p12-10i-01-grok-reauth-local-20260805.md) 与 [review](reports/evidence/p12-10i-01-grok-reauth-review-20260805.md)：3 项 reauth 回归、store schema/迁移、Clippy 通过；真实 OAuth、grok2api、服务器和生产图不变 | LOCAL_PASS_PENDING_PHASE_GATE |
| P12-10I-02 | 少量 Build OAuth/Console SSO 恢复与 CPAR 隔离 HTTP 验证 | [受控 receipt](reports/evidence/p12-10i-02-grok-oauth-cpar-receipt-20260805.md) 与 [review](reports/evidence/p12-10i-02-grok-oauth-cpar-review-20260805.md)：Build refresh 0/2；Console SSO 2/2 导入，`/v1/models` 通过、首条 Responses JSON 为 `EgressRejected/egress`，SSE 按首败规则未发送；staging 回滚/清理、生产不变性通过 | BLOCKED_WITH_EVIDENCE |
| P12-10I-03 | 用源侧 SSO 转换新的 Build OAuth，并分类 Console egress 后做 CPAR 隔离 HTTP 复测 | 待 P12-10I-02；CR-P12-10I-004 | [受控回执](reports/evidence/p12-10i-03-grok-sso-conversion-egress-receipt-20260805.md) 与 [review](reports/evidence/p12-10i-03-grok-sso-conversion-egress-review-20260805.md)：5/5 SSO 在 Device Verify 终止；无新 Build OAuth、无新 CPAR 推理；Oracle 无凭据探测 TLS/连接正常但 HTTP 4xx；生产不变 | BLOCKED_WITH_EVIDENCE |
| P12-10I-04 | 追加 5 个全新 SSO→Build OAuth 转换探针，按固定步骤归类失败原因 | P12-10I-03；CR-P12-10I-005 | [受控回执](reports/evidence/p12-10i-04-grok-sso-conversion-followup-receipt-20260805.md) 与 [review](reports/evidence/p12-10i-04-grok-sso-conversion-followup-review-20260805.md)：5/5 在 Device Verify 4xx 终止；累计 10 个 SSO 均无新 Build OAuth；未进入 CPAR staging；生产不变 | BLOCKED_WITH_EVIDENCE |
| P12-10I-05 | 复用当前本机 Grok CLI 活跃会话做单轮最小请求探针；区分 CLI 会话可用性与 CPAR Build OAuth 可导入性 | P12-10I-04；CR-P12-10I-006 | [受控回执](reports/evidence/p12-10i-05-grok-local-cli-session-receipt-20260805.md) 与 [review](reports/evidence/p12-10i-05-grok-local-cli-session-review-20260805.md)：活跃进程仍在但 `auth.json` 不存在；单次 continuation 退出 `1`，固定类别 `auth_required_or_denied`；未发 CPAR 请求、未导出凭证 | BLOCKED_WITH_EVIDENCE |
| P12-10I-06 | 浏览器 Device OAuth 后以 CPAR base URL + client key 做单账号 Build JSON/SSE 受控矩阵 | P12-10I-05；CR-P12-10I-007 | [受控回执](reports/evidence/p12-10i-06-grok-device-oauth-cpar-receipt-20260805.md) 与 [review](reports/evidence/p12-10i-06-grok-device-oauth-cpar-review-20260805.md)：OAuth、`/v1/models` 与首个 Responses JSON 通过；第二个 Chat JSON `http_5xx` 首败；2/6 请求、无 SSE/Messages；staging rollback/clean 与生产不变性通过 | BLOCKED_WITH_EVIDENCE |
| P12-10I-08 | Autoreg 修复后把一个 Build OAuth 以加密形式导入 CPAR native pool | [Autoreg repair and CPAR import](reports/evidence/p12-10i-08-autoreg-cpar-import-20260805.md)：注册 1/1、批次导入 1/1、服务/schema/loopback admission 通过；active graph 无 Grok route | BLOCKED_WITH_EVIDENCE |
| P12-10I-09 | 验证该导入账号的 Build/Console/Web provider 边界 | [Autoreg account provider matrix](reports/evidence/p12-10i-09-autoreg-account-provider-matrix-20260805.md)：Build 隔离真实 probe 通过；Console/Web 错误 shape 均在导入前拒绝；生产无 Grok route，未声称公共 HTTP 成功 | BLOCKED_WITH_EVIDENCE |
| P12-10I-10 | Autoreg SSO→Build/Console/Web 隔离矩阵与 batch-scoped probe 修正 | [Autoreg SSO provider matrix](reports/evidence/p12-10i-10-autoreg-sso-provider-matrix-20260805.md) 与 [review](reports/evidence/p12-10i-10-autoreg-sso-provider-matrix-review-20260805.md)：SSO→Build 转换/导入通过，batch-scoped probe 已由 P12-10I-11 精确制品复测闭合；Console 为 `EgressRejected/egress`；Web 因超过 90 天 session lifetime fail closed；生产无 Grok route | BLOCKED_WITH_EVIDENCE |
| P12-10I-11 | 以精确签名制品复测 Autoreg SSO-derived Build batch-scoped native probe | [exact-artifact receipt](reports/evidence/p12-10i-11-autoreg-sso-build-exact-artifact-receipt-20260805.md) 与 [review](reports/evidence/p12-10i-11-autoreg-sso-build-exact-artifact-review-20260805.md)：两账号隔离池精确选择新 batch account，真实 Build probe、Health/Quota、Canonical、Chat/Responses/Messages 全部通过；rollback/cleanup 与生产不变性通过 | DONE |
| P12-10I-12 | 在同一 Oracle 出口运行精确 grok2api Console 参考，区分 CPAR 请求差异与外部拒绝 | [reference parity receipt](reports/evidence/p12-10i-12-console-reference-parity-receipt-20260806.md) 与 [review](reports/evidence/p12-10i-12-console-reference-parity-review-20260806.md)：历史成功账号在精确 `v3.0.10` 参考中仍得到上游 403；CPAR 25-account 403 与参考一致，无 CPAR Console 改码依据；临时容器/副本清理且生产不变 | BLOCKED_EXTERNAL_EGRESS_WITH_REFERENCE_PARITY |
| P12-10I-13 | 将有效的长期限 Web 源凭证收窄到 CPAR 本地 90 天，不放宽严格 direct import | [local receipt](reports/evidence/p12-10i-13-web-expiry-cap-local-receipt-20260806.md) 与 [review](reports/evidence/p12-10i-13-web-expiry-cap-local-review-20260806.md)：迁移持久化 `min(source, observed+90d)` 并输出 value-free capped count；180→90 天回归、91 天严格拒绝、provider Full test 与 Clippy 全通过；无服务器/网络变更 | LOCAL_PASS_PENDING_WEB_E2E |
| P12-10I-14 | 组成 native Web production Statsig/session transport，并通过 CPAR base URL/client key 做真实 HTTP E2E | [local receipt](reports/evidence/p12-10i-14-web-native-runtime-local-receipt-20260806.md)、[E2E receipt](reports/evidence/p12-10i-14-web-native-cpar-e2e-receipt-20260806.md) 与 [reviews](reports/evidence/p12-10i-14-web-native-runtime-local-review-20260806.md)、[E2E review](reports/evidence/p12-10i-14-web-native-cpar-e2e-review-20260806.md)：clean4 隔离图在修正 15s bootstrap bound 后启动，CPAR 模型列表预检通过，首个 Responses 推理真实发出但为 value-free `EgressRejected/egress` 5xx，按首败未继续 Chat/Messages/SSE；生产不变 | BLOCKED_WITH_EVIDENCE |
| P12-10I-19 | 使用 Autoreg 新账号重新验证 Console 与 Web 的真实 CPAR 文本入口 | [receipt](reports/evidence/p12-10i-19-autoreg-new-account-console-web-cpar-receipt-20260806.md) 与 [review](reports/evidence/p12-10i-19-autoreg-new-account-console-web-cpar-review-20260806.md)：注册/导入 1/1；Console Responses JSON 先成功、Chat JSON 5xx 首败；Web session capped 后真实请求为 `EgressRejected/egress`；两条 route/account batch 全部回滚，生产不变 | BLOCKED_WITH_EVIDENCE |
...
#### CPAR 与 Autoreg 的独立项目边界（`CR-P12-AUTOREG-SEPARATION-001/002` + `CR-P13-RUNTIME-OAUTH-REFRESH-001`）

本节覆盖当前计划的状态解释；历史 receipt 和历史计划变更不被改写。

- **Autoreg 项目负责**：注册、浏览器首次登录、SSO/Device OAuth 首次授权、refresh grant 撤销后的
  交互 reauth、账号权益/额度修复、账号源生命周期健康、replenishment、任务队列、浏览器运行时、
  Autoreg SQLite 以及 Jakarta/Oracle Autoreg scheduler。
- **CPAR/P12 负责**：在收到外部凭证包后，严格解析和 AEAD 导入、Provider/Channel/Route binding；并独立
  ...
  failure feedback、cooldown/recovery、CPAR Base URL + client key 的真实 JSON/SSE 反代、错误分类和
  rollback；已保存且 Provider/channel 明确支持 refresh 的 OAuth 还由 CPAR 做日常自动续期、encrypted
  CAS 和 runtime atomic replacement。
- **禁止项目耦合**：CPAR 不读取 Autoreg 数据库，不调用 Autoreg HTTP/浏览器，不启动 Autoreg scheduler，
  不把 Autoreg 当作上游 Provider，也不在 CPAR 内修复账号注册、首次 SSO/OAuth、revoked grant 或账号
  权益；常规 token refresh 不需要 Autoreg 在线。
- **失败归属**：CPAR 负责记录并控制 `CredentialUnauthorized`、expiry、Quota、cooldown、Circuit 和
  `EgressRejected` 等运行时状态，并先对可刷新的 OAuth 执行 exact-channel 后台 refresh；只有 grant
  撤销后的账号登录、权益和新凭证生成根因修复属于 Autoreg。Provider 出口属于 CPAR 的
  Provider-specific egress。shape、导入、binding、refresh/CAS、lease、协议投影、错误分类、
  secret/rollback 等 CPAR 行为错误都属于 CPAR 实现缺陷。
...
该边界不改变 CPAR 既有 Provider adapter 或生产配置，也不新增管理 OpenAPI/Prism/前端工作。
...
状态机固定为：`JAKARTA_PRIMARY_ORACLE_ABSENT` →
`JAKARTA_PRIMARY_ORACLE_STAGED_VALIDATED` →
`JAKARTA_PRIMARY_ORACLE_CANARY_ACCEPTED` →
`ORACLE_PRIMARY_JAKARTA_ROLLBACK_READY` → `ORACLE_PRIMARY_JAKARTA_RETIRED`。
...
## 19. P13 - Release 1.1 候选范围与优先级

`CR-P13-001` 根据 CPA/CPAMP、Sub2API、grok2api 的行为参考重新整理 Release 1.1。
P13 不把参考项目的全部产品能力机械搬入 CPAR，而是优先补齐生产反代真正需要的管理、账号池、
选路和可审计运维能力；支付/SaaS、媒体和更多 Provider 只有在单独批准后才进入实现。

P13-01/P13-02 已由 `CR-P12-COMPAT-001` 前移到 P12-08，不再重复开发。用户明确不需要
New API/AxonHub 一次性 Import Adapter，因此原 P13-03 从当前实施范围移除，保留为未来候选记录而不进入
下一阶段。下面的 `PENDING` 表示已排入下一阶段但尚未开始，`DEFERRED` 表示本轮不启动。

### 19.1 已核对的管理面基线

Claude Code 已完成的管理面基础不是空白，当前 CPAR 已有：

- 版本化 Management OpenAPI、loopback/受控私网 admission、Management Key 与 CSRF 边界；
- Upstream、Endpoint、Credential、Binding、Public Model、Alias、Route、Candidate、Access Group、
  Client Key 的资源工作流；
- Catalog preview/apply、Route Validate/Explain、runtime availability、quota recovery、metrics、
  audit、backup preflight/restore；
- Codex OAuth start/status/callback/refresh、CPA/Sub2API export，以及受保护的 credential metadata 查询；
- 生成式 TypeScript client 和静态管理 SPA，当前 UI 是 P10 的安全 draft-management workspace，提供
...
| P13-07 | Cost-aware、Fill-first、Least-loaded 路由：在 Provider scope 内依据额度、并发、健康和 capability 选择；保留 max_attempts、首败边界和 Config Version 回滚 | P13-05、P13-06 | P13-07A/B/C/D 的实现、独立 review、本地 Full 43/43 与正式 routing phase Delivery Gate run 31875826495 全部通过；exact Config-Version catalog 绑定为共享六维 `rate_dominance_v1` evidence，serving/Explain 共用 immutable map，OpenAPI/Prism 已同步并完成 Claude Code 留痕；未做 staging/production canary，不跨 Provider fallback | DONE |
...
| P13-11 | Generic compatible-endpoint + Provider-specific Egress/Proxy Pool：CPA/Sub2API/Krill/custom `base_url + api_key` 统一按端点实例建模；协议格式、凭证来源、出口、账号池 Health/Quota 和失败域分离；Grok Web clearance 使用 Provider-specific 后续边界 | P13-04、P13-06、现有 EgressPolicy/EndpointUrl；Provider 外部授权仅在真实探针阶段需要 | P13-11A/B/C 已由 `phase-p13-egress-complete` / exact `a716eaaa` / Gate 31959162202 正式验收；P13-11D 已由 `phase-p13-egress-management-complete` / exact `1beb230248fb75ced146b87c547eb020ee9cd010` / Gate 31996324578 正式验收为 `DONE_WITH_BOUNDARY`。P13-11E E0-E3 的 Provider-aware typed state、Build/Console exact-lease attempt 与 Web transport-free sticky/session/clearance seam已由 `phase-p13-provider-egress-complete` / exact `ba2261a5414fe73d147a102a266abd3e9a7fbb5b` / Gate 32044424886 正式验收为 `DONE_WITH_BOUNDARY`。独立 E4 只读管理投影也已由 `phase-p13-provider-egress-status-complete` / exact `ce98faa9306d076f5af53b9eef0c818abb1cb9c8` / Gate 32110872875 正式验收为 `DONE_WITH_BOUNDARY`；当前 source 仅 Build/Console，production Web/clearance 可空，synthetic HTTP clearance 仅 serializer-contract 证据，generic compatible 必须由独立 source owner 注入。Build/Console native fixed/pool egress、Web production transport 与 E5 real-network canary 未包含；E5 保持独立 `DEFERRED_UNAUTHORIZED`，不会使已授权且已验收的 P13-11 umbrella 继续占用活动任务状态。Autoreg 为独立项目且不属于本任务 | DONE |
| P13-12 | Provider/channel-scoped account entitlement：Grok Build、Grok Web、Grok Console、ChatGPT/Codex、Claude 分域识别账号等级；缺失证据为 null，不从 quota/请求成功推断，不跨渠道复用 | P13-06；`CR-P13-ACCOUNT-TIER-001` | additive persistence、严格 tier/source/confidence 类型、Grok Build live subscription + signed-token fallback、GPT/Claude exact parser、protected account-pool/OpenAPI；当前 Build 账号已受控 backfill 为 authoritative `supergrok`，生产 projection/egress available；Claude Code UI sync 与 formal Gate 待完成 | LOCAL_PASS_PENDING_PHASE_GATE |
...
| P13-15 | 全渠道上游模型目录透传：Grok Build/Web/Console、Official、Kiro 与任意 compatible `baseURL + API key` 都按 Provider/channel/Endpoint/Credential 精确发现；公开 `id` 保留上游值，Client-Key 只看到授权 union，请求只能 lease exact-model binding；不靠 CPAR alias 或前端白名单 | P4 catalog 基线、P13-06 pool、P13-07 router、P13-11 egress；`CR-P13-UPSTREAM-MODEL-CATALOG-001` | A exact-model serving 已 production pass；B 的 Build/Codex source 已本地实现并真实观测 `grok-4.6`/Luna 等 ID；C/D 的 migration 0021、last-success/freshness/removal、safe protected status、runtime worker、exact Credential materialization、atomic publication 与 request snapshot pin 已在 Grok Build 生产边界通过：Oracle 授权 `/v1/models` 动态包含 `grok-4.6`，真实 Responses `200/completed`。其余 channel source、target Go/Codex evidence、E isolation、Claude Code 与 formal Gate 待完成 | IN_PROGRESS |
| P13-16 | 已导入 OAuth runtime 自动续期：active exact Provider/channel Credential 在 expiry 前由 CPAR 后台 refresh、encrypted CAS 持久化并原子替换后续 lease；API key/SSO cookie 不伪刷新；注册、首次授权、revoked grant 交互 reauth 与 replenishment 仍在 Autoreg/operator 边界 | P6/P12 refresh transaction、P13-06 pool、P13-11 egress；`CR-P13-RUNTIME-OAUTH-REFRESH-001` | A 已实现当前生产活动的 Grok Build/Codex startup catch-up + periodic worker、Build provider-only claim、Codex exact binding、in-flight lease pinning和失败指数退避；Oracle 已证明 Build 到期凭据 CAS 刷新与继续请求成功。当前两个旧 Codex grant 需交互 reauth，因此 Codex 有效 grant 的 production success receipt 与 Claude/Kiro 延期渠道 executor 仍待完成，之后统一 formal Gate | LOCAL_PASS_PENDING_PHASE_GATE |

Autoreg 注册、首次登录/SSO/OAuth、refresh grant 撤销后的交互 reauth、账号修复和 replenishment
属于独立 Autoreg 项目，不占用 CPAR 的 P13 编号，也不作为 P13-12 的依赖或完成条件。CPAR 管理
已导入凭证、支持渠道的 OAuth 日常自动 refresh、entitlement observation、Health/Quota/Circuit、
lease、cooldown、调度和反代。
...
| P13-11E4 Provider egress status | generated `listProviderEgressStatus` 已可用；Claude Code 须按 `egress`/`session`/`clearance` 分区展示、传递 exact `X-Config-Version`/opaque cursor 并在 `409` 后重启读取，不合成 overall health、不增加 action，不把 empty Web/clearance 解释为 healthy |
...
7. **P13-11 Egress/Proxy Pool（A/B/C/D、E0-E3 与独立 E4 均已正式收口）**：P13-11A 已统一 generic compatible endpoint (`base_url + api_key`/CPA/Sub2API/直接 OAuth/custom relay) 的 typed egress binding；P13-11B 已把它组合到 active Config Version、现有 Credential pool、Health/Quota 和受控 Direct/local-DNS SOCKS5 transport registry；P13-11C 已将该 composition 接入 serving exact Credential lease、模式化 transport 与精确 failure feedback；P13-11D 已完成 Config-Version-owned pool/node/binding persistence、AEAD、protected management 与 active runtime composition，并由 `phase-p13-egress-management-complete` / exact `1beb230248fb75ced146b87c547eb020ee9cd010` / run 31996324578 正式验收为 `DONE_WITH_BOUNDARY`。P13-11E E0 冻结 Provider 状态域/no-fallback，E1 完成 typed capability/state/budget，E2 将 Build/Console 接入 CPAR exact lease/native adapter，E3 完成 Web transport-free exact lease + named sticky egress + session/clearance 与 atomic refresh ownership；unknown/confirmed/challenge `403` 分离且当前 attempt 不重发。E0-E3 已由 `phase-p13-provider-egress-complete` / exact `ba2261a5414fe73d147a102a266abd3e9a7fbb5b` / run 32044424886 以 `DONE_WITH_BOUNDARY` 正式收口；Build/Console 仍为 typed Direct，Web 仍 transport-free。独立 E4 的 atomic exact source-domain snapshot、protected GET 与 contract/client 已由 `phase-p13-provider-egress-status-complete` / exact `ce98faa9306d076f5af53b9eef0c818abb1cb9c8` / run 32110872875 以 `DONE_WITH_BOUNDARY` 正式收口；process source 只覆盖 Build/Console，production Web/clearance 可空，synthetic HTTP clearance 仅 serializer-contract 证据，generic compatible 保持独立 owner；E5 真实代理/FlareSolverr/DNS/Provider canary 仍为 `DEFERRED_UNAUTHORIZED`。任何边界都不能引入跨 Provider 隐式 fallback。
...
9. **P13-12/15/16 当前活动项**：P13-12 是 Provider/channel entitlement，后端和当前 Build `supergrok` production backfill 已完成但等待前端/formal Gate；P13-15 的 Build/Codex source 与 C/D durable catalog/exact Credential materialization 已通过 Grok Build Oracle production acceptance，下一步是 remaining channel sources、target Go/Codex evidence 与 E isolation/Claude Code/formal Gate；P13-16A 已在 Oracle 证明 Build OAuth startup refresh 与请求连续性，Codex 有效 grant 仍待目标账号交互 reauth。Autoreg 只保留注册、首次授权、revoked grant 交互 reauth 与 replenishment，不是日常 refresh executor。
...
| Deferred | native Grok/Provider runtime account pools、live Health/Quota/Circuit、refresh/reauth、proxy pool，以及 versioned priced usage/cost/billing；P13-04B 仅在后续切片补充 durable、明确 `unpriced` 的 Usage projection，Provider-owned live state 经 P13-06 injected facade 才可加入 |
...
### 19.5 P13-04B Task Card

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`；只读管理运营投影跨 control/HTTP/store/OpenAPI/generated client/docs，先跑定向本地门禁，P13 收口前不启动昂贵远端 Delivery Gate |
| 依赖 | P3/P4 bounded Request/Attempt/Usage event log；P10 Management Key、审计、runtime Health/Quota 状态与统一错误封装；P13-04A 管理路由基线 |
| 输出 | `GET /admin/operations/usage`；有界、可分页的 Provider/Channel/Account/public model/protocol/Client Key/Access Group 聚合 |
| 语义 | 只选择每个 Request 的最高编号 Attempt；仅成功 Attempt 关联 final Usage；失败/缺失 lineage/counter overflow fail closed；过滤按 identity/model/protocol/inclusive Attempt-end-time；重试失败 Attempt 不归因 |
| 使用量 | 每个 token 返回 `{total, confidence}`，confidence 为 `exact|partial|unknown`；checked addition；未知值不转成零 |
| 费用 | `cost_microunits=null`、`cost_confidence=unpriced`；不硬编码价格、不从 token 反推金额；versioned price catalog 与 billing ledger 由 P13-05 承担 |
| 分页 | 稳定 `(provider_id, channel_id, account_id, public_model, protocol, client_key_id, access_group_id)` keyset；default 50、max 100；cursor 只含非 Secret 维度且 HTTP 边界有长度/字符限制；不声称跨并发写入的快照一致性 |
| 生产组合 | 通过只读 SQLite event facade + bounded scan 读取 gateway-owned durable events；不迁移、不写 journal mode、不调用 Provider、不租约/不改配置 |
| 定向验收 | usage compiler unit（lineage/filter/group/confidence/pagination/unpriced）；受保护 HTTP fixture（auth、malformed query、secret/body/private-model absence）；OpenAPI closed schema/generated client；read-only store test；fmt/Clippy/docs/diff review |
| 禁止项 | 不改 production/server/active Config Version，不发送真实 Provider/OAuth 请求，不新增全局 proxy/fallback，不修改既有未跟踪 helper 文件 |

### 19.6 P13-05A Task Card

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`；先实现 Store/Control foundation，定向本地门禁；P13 收口前不启动昂贵远端 Delivery Gate |
| 依赖 | P13-04 durable Usage read model、P4 append-only event log、P10 management security/audit |
| 输出 | `billing_price_catalog_versions`、`billing_price_catalog_entries`、`billing_ledger_entries`；`SqliteBillingLedger`；`gateway-control::billing_service::quote_usage` |
| 计价 | 价格为整数 micro-units per million tokens；checked `u128` 乘加后转换为 `u64`；匹配键为 Provider/Channel/Model |
| 置信度 | `exact`（所有维度存在）、`partial`（部分维度存在的下界）、`unknown`（无可计价 token）、`unpriced`（无匹配 catalog）；缺失值不得变成零 |
| 幂等与恢复 | `source_event_id` 唯一；相同指纹 replay 为 no-op，不同指纹 fail closed；retention purge 有界；file reopen 后保留行一致 |
...
### 19.8 P13-05C Task Card

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`；关闭 catalog operator-write 缺口并做 P13-05A/B/C 整体 review；定向本地 Gate，不运行正式 P13 Delivery Gate |
| 依赖 | P13-05A immutable catalog；P13-05B materializer/read model；P10 Management Key、same-origin CSRF、selected Config Version、If-Match/ETag 与 resource audit |
| 输出 | `GET/POST /admin/billing/catalogs`；`POST /admin/billing/catalogs/{catalog_version_id}/rollback`；OpenAPI/generated client；Secret-free mutation receipt |
| 原子性 | catalog insert、selected draft Config Version exact revision compare/increment、`billing_catalog_imported|billing_catalog_rolled_back` audit 在同一 SQLite transaction；任一失败全部回滚 |
| 不可变与回退 | management create 对任何既有 id（含完全相同重试）稳定返回安全 409 且不改 revision/audit；Store exact replay 仅供内部 crash recovery；rollback 必须使用新 id，复制 retained predecessor entries 并记录 operator source；旧 catalog 与历史 ledger 永不改写或删除 |
...
| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `M`；先交付 Provider-owned 只读运营 facade 与管理读接口；只运行本地定向 Gate，不运行正式 P13 Delivery Gate |
| 依赖 | P13-04 management/OpenAPI 基线；现有 `EndpointCredentialPools` lease；Runtime Health/Quota/Circuit registry；Provider-specific native pool 由后续 adapter 注入 |
| 输出 | `gateway-control::provider_account_pool_service` typed snapshot/query/cursor/facade；`GET /admin/operations/provider-account-pools`；OpenAPI/generated client；HTTP 认证、分页、状态分离和 no-secret 回归 |
| 状态语义 | `auth_status` 独立于 `runtime_status`：支持 `active|reauth_required|disabled|expired` 与 `available|cooling|circuit_open|quota_blocked|unauthorized|recovery_in_flight|expired`，不得把认证失败、额度阻塞和 circuit 混为一个状态 |
| 快照与分页 | Provider adapter 提供单一 `snapshot_id + observed_at_ms`；按 `(provider_id, channel_id, account_id)` 稳定排序，default 50/max 100；cursor 绑定 snapshot 与过滤 fingerprint，跨快照或跨过滤条件返回 409 |
| 安全边界 | 仅输出 Provider/Channel/Account opaque id、account kind、状态、优先级/权重/并发占用和维护时间；不得输出 credential plaintext/ciphertext、endpoint URL、Header/Cookie/body、client-key digest；不调用 Provider、不改 active Config Version |
...
| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`；在既有 runtime facade 与 Provider account-pool adapter 上增加两类有界 operator action 和失败反馈读模型；只运行本地定向 Gate，不运行正式 P13 Delivery Gate |
| 依赖 | P13-06A/B typed provider-pool facade；同一 `EndpointCredentialPools`、`RuntimeHealthRegistry`、`RuntimeQuotaRegistry`；P10 Management Key、same-origin CSRF、`X-Config-Version` 与 resource audit；durable Attempt event log |
| 输出 | `POST /admin/operations/provider-account-pools/actions`：精确 Provider/Channel/Account 的 `cool_down` 或 `request_recovery`；`GET /admin/operations/provider-account-pools/failures`：精确过滤、稳定有界分页的脱敏 Attempt failure feedback；OpenAPI/Prism contract sync 留痕；value-free action receipt |
| Action 语义 | `cool_down` 只在 exact Endpoint-Credential 或 Endpoint-Credential-Model Health key 写入 bounded cooldown；`request_recovery` 复用 controlled account/quota recovery ticket，遵守 Unauthorized/Forbidden/Quota/RecoveryInFlight 的既有状态机；不直接发送 Provider 请求、不取得 lease、不改变 credential plaintext/status、不启动 refresh/reauth |
| 认证与审计 | POST 走受保护 `/admin`、Management Key、same-origin CSRF 和 `X-Config-Version`；target 必须属于当前 serving provider-pool facade；每次已接受/拒绝的 action 都记录 value-free resource audit；runtime action 不提升 Config revision，避免把 ephemeral Health/Quota 状态伪装成配置变更 |
| Failure feedback | 只读取 gateway-owned durable AttemptEvent；输出 Provider/Channel/Account opaque attribution、Attempt id/request id、ended time、closed GatewayError code/scope、retry decision；不输出 upstream model、URL、Header/Cookie/body、raw error、Secret、client-key digest；来源超限或解码失败 fail closed |
| Provider 隔离 | provider/channel/account 三元组精确匹配；model action 只影响该 model key；不跨 Provider fallback、不把 Grok Build/Web/Console、Codex、Krill 的状态或 credential 转换；普通 binding 的 enable/disable 继续走已有 Config Version mutation，native durable enable/disable/reauth 仍留给 Provider-specific P13-12 |
...
| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`；先实现 Provider scope 内 deterministic cost-aware/fill-first/least-loaded policy seam 与内部安全 decision projection；现有 Route Explain/serving scheduler 接入拆到 P13-07B，不把 Provider I/O、生产 canary 混入同一片 |
...
| 输出 | 新增 `ProviderScopedSelector` 与 secret-free candidate/decision types；对额度、并发、健康、expiry、capability 与已知成本做有界 deterministic 排序；decision 输出稳定排名及排除原因，不暴露 credential/URL/body；P13-07B 再从 shared runtime observations 组装输入并投影到现有 Route Explain |
| 选择语义 | `fill-first` 先排除不可用/过期/额度阻塞/并发饱和候选；同一 Provider/Channel 内再按 quota evidence → known cost → least-loaded ratio → priority → weight → channel/candidate identity 做稳定 deterministic tie-break；未知成本/未知 quota 不伪造为零，不跨 Provider fallback |
...
| 定向验收 | deterministic selector matrix：quota 缺失/耗尽与 controlled recovery、Health/Circuit/Unauthorized、expiry、并发饱和、capability mismatch、成本未知、least-loaded、同 Provider sibling isolation、跨 Provider 禁止 fallback、duplicate/identity/count bounds；existing scheduler 回归、strict Clippy/fmt/docs/review |
| 交付物 | [ADR-0084](adr/ADR-0084-provider-scoped-deterministic-routing-selector.md)、[BC-ROUTE-006](contracts/BC-ROUTE-006-provider-scoped-deterministic-selector.md)、[P13-07A report](reports/p13-07a-provider-scoped-selector.md)；不启动 P13-08/11/12，不做公网或生产 traffic canary |
| 状态 | `DONE_WITH_BOUNDARY`；P13-07A 本地实现与 review 已由 `phase-p13-routing-complete` exact commit `0c338ee8eef76e470c55515a24728324684365c5` 的正式 Delivery Gate run 31875826495 验收；P13 umbrella 仍 `IN_PROGRESS` |

### 19.13 P13-07B Task Card

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `M`；只把 Provider-scoped selector 接入现有只读 Route Explain 管理投影和共享 scheduler seam；不改变 serving 请求路径、不获取 lease、不调用 Provider、不做 staging/production canary |
| 依赖 | P13-07A selector；P13-06 shared credential pools、Health/Quota registry；现有 `RouteCredentialScheduler::explain`、Config Version snapshot、Route Explain HTTP/OpenAPI/Prism contract |
| 输出 | `RouteCredentialScheduler::explain_provider_scoped` 只读接口；同一 runtime composition 注入管理 facade；Route Explain 支持可选 `provider_id`；候选 selection/reason 仍为 value-free projection，保留 `provider_scope_required` 与 `provider_mismatch` |
| 选择语义 | provider identity 必须由 route candidate 的 owning upstream id 显式映射；显式 scope 只允许同一 Provider；未指定 scope 且存在多个 Provider 时 fail closed；唯一 Provider 可安全推断；exact Health/Quota、expiry、capability 和 capacity 只作为 advisory explain，真实 request lease 仍由 scheduler 在 request-time 再次校验 |
| 失败与回退 | 不把第一个 Provider 当隐式 fallback；精确 quota snapshot 缺失保持 Unknown；`expires_at_ms <= observed_at` 视为过期；不发送 Provider 请求、不 refresh/reauth、不启用 proxy pool、不改变 max_attempts/first-semantic-event/首败边界 |
| 前后端边界 | `docs/openapi/management-v1.json` 增加可选 `provider_id` 后，已运行 `npm --prefix web/prism run sync-contract`；Prism generated client/contract 与后端保持同步；`docs/cross-boundary-log.md` 已记录 Claude Code 后续必须增加 Provider selector，并渲染 `provider_scope_required`/`provider_mismatch`，本 Task 不实现正式 UI |
| 定向验收 | gateway-router 113 tests；gateway runtime targeted provider-scope test；gateway-http-actix serial all-targets；OpenAPI/Prism contract checks；known/unknown quota、expiry boundary、foreign Provider、multi-Provider no-scope、no lease mutation、secret-free projection、strict Clippy/fmt/source-policy/docs review |
| 交付物 | [ADR-0085](adr/ADR-0085-provider-scoped-route-explain-composition.md)、[BC-ROUTE-007](contracts/BC-ROUTE-007-provider-scoped-route-explain.md)、[P13-07B report](reports/p13-07b-provider-scoped-route-explain.md)；OpenAPI/Prism contract sync 与 [cross-boundary log](cross-boundary-log.md)；不启动 P13-08/11/12 |
| 状态 | `DONE_WITH_BOUNDARY`；P13-07B 本地实现、定向门禁与独立 review 已由 `phase-p13-routing-complete` exact commit `0c338ee8eef76e470c55515a24728324684365c5` 的正式 Delivery Gate run 31875826495 验收；P13 umbrella 仍 `IN_PROGRESS` |

### 19.14 P13-07C Task Card

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`；将 Provider-scoped selector 接入 request-time serving 的候选排序，并在同一 scheduler/pool 上做 exact lease revalidation；只运行本地定向 Gate，不运行正式 P13 Delivery Gate |
| 依赖 | P13-07A selector；P13-07B shared Route Explain/scheduler seam；P13-06 shared `EndpointCredentialPools`、Health/Quota registries；现有 AttemptOrchestrator、RouteSnapshot、protocol/candidate admission 与 Config Version guard |
| 输出 | serving 内部 selector-driven candidate order；exact candidate/binding lease helper；fresh expiry/Health/Quota/capacity revalidation；unique-Provider serving scope；multi-Provider no-scope fail-closed；不新增公开字段或管理路由 |
| 选择与租约语义 | selector 只输出有界、脱敏、稳定 candidate IDs，绝不获取 lease 或推进第二个 scheduler；每次真实 lease 前由现有 `RouteCredentialScheduler` 在新 `observed_at` 重新检查 route/binding、expiry、Health/Circuit、Quota/recovery 和原子 capacity；租约竞态只可尝试同一 Provider 内下一个已排序候选，不增加 attempt 或放宽 `max_attempts` |
| Attempt / stream 边界 | `AttemptOrchestrator` 继续拥有 retry budget、exact exclusion、first-semantic-event、timeout/cancellation、driver failure 和 lease release；首个 semantic event 后禁止 retry/provider switch；stale Config Version 在 lease/driver 前 fail closed |
| Provider 隔离 | admitted candidates 若属于多个 Provider 且无显式内部 scope，必须在 lease/driver 前返回安全 unavailable/provider-scope 类别；不把第一个候选当 scope，不跨 Provider fallback、不转换 credential、不触发 refresh/reauth/proxy pool |
| 前后端边界 | 不改变 Chat/Responses/Messages、管理 OpenAPI、Prism、generated client 或 frontend route；若后续需要显式 provider/route-policy 字段，先开 CR、更新 authoritative OpenAPI，再 sync Prism 并在 `docs/cross-boundary-log.md` 通知 Claude Code |
| 定向验收 | unique Provider selector order；lease race/饱和/expiry/Health/Quota/binding recheck；foreign/missing/duplicate candidate；max_attempts、pre/post-semantic retry、Config Version stale、cancel/timeout/driver lease release；no Store/Provider I/O、无 Secret；router/runtime/loopback tests、strict Clippy/fmt/source/crate-boundary/docs/secret/diff review |
| 交付物 | [ADR-0086](adr/ADR-0086-provider-scoped-serving-lease-revalidation.md)、[BC-ROUTE-008](contracts/BC-ROUTE-008-provider-scoped-serving-lease-revalidation.md)、[P13-07C report](reports/p13-07c-serving-lease-revalidation.md)；不启动 P13-08/11/12、不做公网或生产 canary |
| 状态 | `DONE_WITH_BOUNDARY`；实现、定向 Gate 与独立 review 已由 `phase-p13-routing-complete` exact commit `0c338ee8eef76e470c55515a24728324684365c5` 的正式 Delivery Gate run 31875826495 验收；P13 umbrella 仍 `IN_PROGRESS` |

### 19.15 P13-07D Task Card

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`；新增 Config-Version-bound price policy、六维 price evidence、runtime shared snapshot 与受保护 management contract；只运行本地定向 Gate，不运行正式 P13 Delivery Gate、staging 或 production canary |
| 依赖 | P13-05 immutable billing catalog/ledger；P13-07A selector；P13-07B shared Route Explain scheduler；P13-07C exact serving lease revalidation；P10 Management Key/CSRF/revision/audit/Config lifecycle |
| Version 绑定 | draft Config Version 最多绑定一个 exact immutable `catalog_version_id` 与 `rate_dominance_v1`；set/clear 复用 `X-Config-Version`、`If-Match`、CSRF 与 resource audit；active/archived 不可原地写；catalog import 不自动改变 active routing，rollback 恢复目标 Version 原 binding |
| 六维语义 | 输入为 input/output/reasoning/cache-read/cache-creation/cached 六个 per-million rates；不得压成 predicted request cost。对 exact Provider 内 eligible known vectors 计算六维 coordinate minima；同时命中全部 minima 才为 dominant/equal，交叉为 incomparable，其他 known 为 dominated，缺 tuple 为 unpriced，未参与为 not-evaluated；explicit all-zero 与 missing 严格区分 |
| Runtime 接线 | composition 只加载 Config Version 精确绑定 catalog 一次，按 owning upstream + endpoint + canonical public model 编译 bounded immutable candidate map；serving 与 Explain 共享同一 scheduler map；request hot path 零 SQLite/catalog refresh/token-count/Provider I/O；P13-07C lease/retry/FSE/Provider isolation不变 |
| 失败边界 | missing/future/malformed bound catalog、unsupported comparison、Config/Snapshot mismatch fail closed且不发布 partial map；missing exact tuple只投影 unpriced并回落 quota/load/priority/weight，不阻断本来可用的 candidate；不跨 Provider fallback、不 refresh/reauth/proxy pool |
| 前后端边界 | 新增 protected routing-price-policy resource，并让 Route Explain 返回 exact policy + closed price evidence；先更新 `docs/openapi/management-v1.json`，运行 `npm --prefix web/prism run sync-contract`，在 `docs/cross-boundary-log.md` 提醒 Claude Code展示 binding/evidence并禁止前端自行算 score；本 Task 不实现正式 Prism page |
| 定向验收 | migration/roundtrip/draft set-get-clear/stale/CSRF/active/audit/atomic rollback；catalog effective boundary/exact public model/zero/unpriced；dominant/equal/crossed/determinism；serving/Explain shared-map parity；hot-path no Store/Provider；P13-07C router/runtime/HTTP regression；OpenAPI/Prism sync、strict Clippy/fmt/source/crate/docs/secret/diff 与 independent review |
| 交付物 | [ADR-0087](adr/ADR-0087-config-bound-routing-price-evidence.md)、[BC-ROUTE-009](contracts/BC-ROUTE-009-config-bound-routing-price-evidence.md)、[P13-07D report](reports/p13-07d-config-bound-routing-price-evidence.md)、authoritative OpenAPI/Prism sync 与 [cross-boundary log](cross-boundary-log.md)；不启动 P13-08/11/12 |
| 状态 | `DONE_WITH_BOUNDARY`；P13-07D 本地实现、定向 Gate 与独立 review 已由 `phase-p13-routing-complete` exact commit `0c338ee8eef76e470c55515a24728324684365c5` 的正式 Delivery Gate run 31875826495 验收；P13-07 已收口，P13 umbrella 仍 `IN_PROGRESS`；未做 staging/production canary |

### 19.16 P13-08 Task Card

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`；受保护管理端单请求诊断后端与 deterministic mock 验证；不运行真实 Provider、production/staging canary 或新的昂贵 P13 Delivery Gate |
| 依赖 | P13-04 management admission/audit 与 selected Config Version；P13-06 Provider-owned account-pool facade、Health/Quota/Circuit 与 operator feedback；P13-07 exact Provider-scoped selector、Route Explain 与 serving lease revalidation |
| 输入边界 | operator 明确提供 `provider_id`、`channel_id`、`route_id`、`credential_id`、`requested_model`、`openai_chat_completions|openai_responses|anthropic_messages` protocol 与 `json|sse` mode；Config Version 来自既有 `X-Config-Version`；首片固定内部短 probe，不接受任意 body/URL/header/cookie/token |
| 执行语义 | 复用 serving composition、同一 route snapshot、同一 EndpointCredentialPools/lease owner、Health/Quota/Circuit 与 egress admission；精确绑定通过后最多一次 upstream send；首败终止、无 retry、无 quota recovery、无 sibling credential、无跨 Provider fallback、无 refresh/reauth/proxy-pool fallback |
| Receipt / audit | 返回 opaque request id、selected stable ids、protocol/mode、`succeeded|failed|rejected`、可选 observed `stage`、`upstream_sent`、`attempt_count`（0/1）、`response_started` 与 timestamp；请求与 `channel_pin_started` 通过 value-free audit 在 Provider 调用前持久化，返回 receipt 作为终态来源，不做 post-send audit append；不返回 URL、body、headers、cookies、token、ciphertext/plaintext、digest、raw Provider status/error；HTTP `no-store` |
| 失败与回滚 | foreign/ambiguous/missing/disabled/expired/health/quota/capacity/egress/capability target 在发送前 fail closed；lease 与临时 secret 在所有终态释放；关闭路由或注入 rejecting facade 可安全回滚，不修改 active Config Version、route graph、生产/staging 或公开 `/v1/*` 数据面 |
| 前后端边界 | 若新增管理 OpenAPI route，先改 authoritative `docs/openapi/management-v1.json`，运行 `npm --prefix web/prism run sync-contract`，并在同一提交更新 `docs/cross-boundary-log.md` 留下 Claude Code UI 接入/展示待办；本 Task 不做正式 Prism page |
| 定向验收 | auth/CSRF/version/unknown-field/size；exact ownership 与 ambiguity；JSON/SSE success；first failure zero-retry；upstream sent/not-sent；credential attribution；lease release；audit/no-secret；no cross-Provider fallback；serving regression；strict Clippy/fmt/docs/source/secret/diff review |
| 交付物 | [ADR-0088](adr/ADR-0088-channel-pin-diagnostic-execution.md)、[BC-MGMT-017](contracts/BC-MGMT-017-channel-pin-diagnostic.md)、[P13-08 report](reports/p13-08-channel-pin-diagnostic.md)；若发生 contract 变化另附 [cross-boundary log](cross-boundary-log.md) |
| 状态 | `DONE_WITH_BOUNDARY`；代码、OpenAPI/Prism、定向 Gate、corrective review、aggregate Full 43/43 与 exact `phase-p13-channel-pin-complete` / `7e14a2733c461d04198a6413efda420a03545eea` / run 31928169486 正式 Gate 全部通过；不宣称 native adapter、真实 Provider或生产/staging验收 |

### 19.17 P13-09 Task Card

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`，拆成 A/B/C 三个可独立 review 的后端切片；每片只运行适用本地 Gate，P13-09 aggregate 收口前最多一次正式远端 Delivery Gate；普通 commit/push 不受限 |
| 依赖 | P12-08 Responses Canonical/JSON/SSE baseline；P13-04 Client Key/management security baseline；existing external Master Key ring、SQLite migration、exact route/credential lease 与 Provider capability matrix |
| P13-09A foundation | migration 0017 + `SqliteStoredResponseStore`；exact `(ClientKeyId, ResponseId)` owner；Canonical request/response、public metadata 与 Config/Provider/Upstream/Channel/Route/Candidate/Credential lineage 全部 AEAD；clear index 仅 owner/id/time/version/ciphertext；30-day TTL、read-time expiry、bounded GC、restart/key rotation、idempotent/conflict/corruption/row-swap fail-closed |
| P13-09B store/retrieve/delete | `POST /v1/responses` 仅在显式 `store:true` 时保存完整成功终态；`GET /v1/responses/{id}` 与 `DELETE /v1/responses/{id}` 先做 exact Client Key owner lookup；missing/foreign/expired/deleted 统一安全 not-found；JSON 与 SSE 使用同一 Canonical payload，失败/partial/StreamError 不存为 completed；持久化失败不得虚构 retrievable success |
| P13-09C continuity/compact | `previous_response_id` 与 `POST /v1/responses/compact` 必须命中 owned、unexpired、完整 lineage；仅声明 capability 的 Provider/Channel 可执行；account/Credential unavailable 返回 continuity error，不使用 sibling、跨 Provider、跨 egress 或 credential-format fallback；gateway-owned compact blob 需要独立 AEAD domain/session binding，foreign blob fail closed |
| 固定边界 | plaintext ≤ 8 MiB、Canonical events ≤ 4096、GC batch 1..4096；TTL 不由客户端覆盖；opaque IDs 分别 bounded；SQLite/AEAD/serialization 使用 blocking boundary，不把 body/reasoning/tool/secret/ciphertext/digest 写入 GatewayEvent、audit、Debug 或 log |
| 前后端边界 | P13-09A 不改 management OpenAPI/Prism。B/C 新增的是公开 Responses 数据面而非 Prism 管理资源；若后续增加管理存储控制/状态，必须先改 authoritative OpenAPI、sync Prism，并在 `docs/cross-boundary-log.md` 提醒 Claude Code。任何 `web/prism/**` 改动不得手改 generated files |
| 定向验收 | A：migration/table inventory、AEAD owner/AAD、TTL/GC、reopen/key rotation、conflict/corruption/bounds/no-secret；B：auth/strict body、store opt-in、JSON/SSE durability ordering、GET/DELETE projection/not-found/isolation/restart；C：capability、exact lineage/credential、compact/continuation Tool+reasoning+usage+stop、foreign blob/no fallback；全部保留现有 Chat/Responses/Messages regression |
| 交付物 | [ADR-0089](adr/ADR-0089-client-owned-encrypted-stored-responses.md)、[BC-RESP-001](contracts/BC-RESP-001-client-owned-stored-response-foundation.md)、[P13-09A report](reports/p13-09a-stored-response-foundation.md)；[ADR-0090](adr/ADR-0090-gateway-owned-stored-responses-public-lifecycle.md)、[BC-RESP-002](contracts/BC-RESP-002-stored-response-public-lifecycle.md)、[P13-09B report](reports/p13-09b-stored-response-public-lifecycle.md)；C 在实现时追加独立 contract/report；不调用真实 Provider、不改 production/staging/server |
| 状态 | `DONE_WITH_BOUNDARY`；P13-09A/B/C 定向实现与 review、aggregate 43/43 Full preflight，以及 exact `phase-p13-responses-complete` / `d419c4678bd2ff563046849cef800c1985d48688` / run 31922870604 的正式 Delivery Gate 全部通过；未做 Provider/staging/production/server 验证，P13-10 未自动启动 |

### 19.18 P13-10A Task Card

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`；公开 Responses WebSocket ingress、Canonical transport projection、capability/cancellation/continuity 与 deterministic loopback review；仅运行适用本地 Gate，P13-10 收口前最多一次正式远端 Delivery Gate，普通 commit/push 不受限 |
| 权威参考 | OpenAI 官方 Responses WebSocket 文档/Node client 的 flat `response.create` 与 lifecycle events；冻结 CPA `v7.2.80` 只作行为参考，不复制其 permissive Origin 或内部执行链 |
| 公共入口 | `GET /v1/responses` upgrade；复用 existing Bearer/`x-api-key` exact Client Key auth；P13-10A 要求 no `Origin` native client；可选 single `x-codex-turn-state` ≤512 bytes 并在 101 回显；所有 handshake 结果 `no-store` |
| 消息/协议 | text-only `response.create`，`stream` absent/true 后 normalize `true`；继续走 strict Responses duplicate-name/unsupported/store/continuity decoder；输出一条 existing Responses lifecycle JSON text message，不加 SSE framing；Realtime、`response.append`、`generate:false`、binary/media、Chat/Messages WS 延后 |
| Runtime/capability | `ResponsesClientTransport::WebSocket` 进入同一 authenticated model view、RouteSnapshot、scheduler/lease、Health/Quota、Attempt、egress、usage、Canonical stream 与 first-semantic tracker；candidate 必须显式 `responses_websocket` capability；downstream WS 不要求 upstream-native WS；缺 capability 在 lease/Provider 前 fail closed |
| 并发/背压/关闭 | one active + one complete pending；第三项 `1008` 并取消 active；4-MiB frame/reassembled/output-message、64 fragments、4096 canonical events、8-MiB turn capture/output；15s write、90s event idle、10m turn、15s ping/45s pong、5m idle、2h session；`1002/1003/1007/1008/1009/1001` closed reasons |
| 存储/连续性 | `store:true` 保留 P13-09 durability-before-terminal/AEAD owner；connection-local 最多 16 completed turns/32 MiB，exact same-client/connection Canonical replay携带 Config/Provider/Upstream/Channel/Route/Candidate/Credential revision；cache miss 可走 durable exact owner；不得 forward downstream locator 或跨 Credential/Provider fallback |
| 取消/回滚 | disconnect/Close/protocol/timeout/backpressure abort turn，drop Canonical source 并释放 lease；删除 GET route/`responses_websocket` capability 即可回滚，POST JSON/SSE、stored tables、Provider adapters不变；不调用 Provider、不改 production/staging/server/DNS/Caddy/account pool |
| 前后端边界 | 这是公开 inference client surface，不改 management OpenAPI 或 `web/prism/**`；在 `docs/cross-boundary-log.md` 留痕提醒 Claude Code 后续客户端文档/测试入口，不要求本片实现 Prism 管理 UI |
| 定向验收 | official shape/strict decode；real RFC6455 auth 401/Origin 403/101/turn-state/no-store；lifecycle/continuation；fragment/type/size；one-active+pending/1008/source drop；capability fail closed；HTTP JSON/SSE regression；fmt/strict Clippy/tests/source/crate/dependency/docs/secret/diff review |
...
| 状态 | `DONE_WITH_BOUNDARY`；P13-11A 已由 `phase-p13-egress-complete` / exact `a716eaaa` / Gate `31959162202` 正式通过；未调用 Provider/网络，后续 persistence/management 归 P13-11D |
...
### 19.20 P13-11B Task Card

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`；完成 generic compatible endpoint 的本地 runtime composition；只运行 gateway-router/gateway-control 定向测试、Clippy、fmt、docs 与本地 Fast review，不启动真实 Provider、代理探针或正式 P13 Delivery Gate |
| 依赖 | P13-11A `CompatibleEndpointEgressProfile`；active Config Version / `SnapshotVersion`；existing `EndpointCredentialPools`；shared Runtime Health/Quota registries；现有 `UpstreamTransportProfile`/`UpstreamProxy` 的 Direct 与 local-DNS SOCKS5 安全边界 |
| 输出 | `CompatibleEgressTransportRegistry`、bounded egress-node capacity/availability/lease；`CompatibleEndpointRuntime`；gateway-control 的 generic-compatible active-config compiler；secret-free binding observations 与 exact credential/egress lease |
| 组合语义 | 每个 runtime composition 只包含一个 exact Upstream/Endpoint/Protocol/EgressPolicy；同一 Endpoint 的多个 Credential 由既有 `EndpointCredentialPools` 负责 priority/weight/capacity；Health、Quota、Credential expiry、Endpoint health 和 Egress-node availability 在 lease 前全部检查；一次 lease 同时持有 Credential lease 与 Egress-node lease，任一路径 drop/cancel 都释放两者 |
| Transport | `Direct` 可使用现有 Direct `UpstreamTransportProfile`；`FixedProxy`/`ProxyPool` 仅允许现有安全的 local-DNS `socks5` profile，HTTP/HTTPS remote-DNS proxy 不在本片放宽；代理节点状态和容量独立于 Credential Health/Quota |
| Provider 隔离 | 只编译 generic `openai-compatible.chat-completions`、`openai-compatible.responses`、`anthropic-compatible.messages`；Grok Build/Console/Web、Kiro、官方 Codex 特殊 OAuth/浏览器 clearance 仍由各自 adapter/provider boundary 管理；每个 transport registry 明确绑定一个 Upstream，不能跨 Provider 共享 node/cooldown/capacity；不同 Upstream/Provider 不共享 profile、lease、Health 或 Quota |
| Active Config Version | compiler 只接受 active caller-selected `ControlPlaneConfiguration`，并校验 supplied EgressPolicy snapshot 与该 graph 完全一致；每个现有 pool entry 还必须匹配 Credential revision、priority、weight、concurrency；不写 Store、不 publish snapshot、不改变 Config Revision；已编译 runtime 不读取 SQLite |
| 失败边界 | endpoint/credential/model Health、account status、binding/model Quota、expiry、credential capacity、egress-node cooldown/disabled/capacity 各自保留；任何一项不可读或不满足都 fail closed，不跨 Credential/Endpoint/Upstream fallback；P13-11C 再加入 Provider-specific failure feedback/probe |
| 安全边界 | runtime/observation/debug 不含 URL、API key、OAuth/SSO、Cookie、proxy secret、body 或 Provider response；实际 `EndpointUrl` 只保留在内部执行对象，并继续由 `EgressPolicy::admit_url` 负责 per-attempt DNS/address admission |
| 定向验收 | direct/fixed/pool target resolution；weighted bounded node selection；node cooldown/disable 与 credential Health/Quota 隔离；exact lease/capacity/drop release；expiry boundary；same-Upstream multi-credential selection；foreign Upstream/mixed endpoint/missing pool/kind/revision/schedule/policy drift reject；draft Config Version reject；active Config Version compiler；Debug/error value-free；no Store/Provider/network |
| 交付物 | [ADR-0094](adr/ADR-0094-generic-compatible-endpoint-runtime-composition.md)、[BC-SEC-006](contracts/BC-SEC-006-compatible-endpoint-runtime-composition.md)、[P13-11B report](reports/p13-11b-compatible-endpoint-runtime-composition.md) |
| 状态 | `DONE_WITH_BOUNDARY`；P13-11B 已由 `phase-p13-egress-complete` / exact `a716eaaa` / Gate `31959162202` 正式通过；P13-11 parent 因后续 P13-11D 继续 `IN_PROGRESS` |
...
| 状态 | `DONE_WITH_BOUNDARY`；P13-11A/B/C 与 aggregate 均已由 `phase-p13-egress-complete` / exact `a716eaaa` / Gate `31959162202` 正式通过；P13-11 parent 因后续 P13-11D 继续 `IN_PROGRESS` |
| 下一边界 | P13-11D：新增受保护、Config-Version-owned 的 proxy-pool persistence/management；Provider-specific probe/recovery、Grok Web clearance、Autoreg 和真实网络验证仍不自动启动 |

### 19.22 P13-11A/B/C aggregate formal closeout
...
| 状态 | `DONE_WITH_BOUNDARY`；A/B/C aggregate 的 slice reports、ADR/Contract、aggregate receipt/review 与其原始 formal Gate 已完成；D 作为独立 Task Card 另行由 `phase-p13-egress-management-complete` / exact `1beb230248fb75ced146b87c547eb020ee9cd010` / run `31996324578` 收口；P13-11 parent 仍 `IN_PROGRESS` |
| 允许的下一步 | 已批准另立 `P13-11E` Provider-specific egress/health/recovery 计划；本 Task Card 不自动启动 Grok Web clearance、Autoreg、真实代理/DNS 或生产网络验证 |
| 明确未做 | Provider/代理/DNS/服务器/staging/production、Autoreg、真实 egress probe/recovery、native Provider transport 与 broader proxy schemes |

### 19.23 P13-11D Task Card — Config-Version-owned compatible proxy-pool management

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`，拆成 D1/D2/D3 三个可独立 review 的本地切片；D1 typed persistence/AEAD，D2 protected management HTTP/OpenAPI，D3 active runtime composition。每片只跑定向 Gate；D aggregate 收口前才运行一次 Full/正式 Delivery Gate |
| 依赖 | P13-11A/B/C 已正式收口；现有 `ControlPlaneConfiguration`、draft revision/ETag、Management Key/CSRF/audit、`SecretStore`、`CompatibleEgressTransportRegistry` 与 `CompatibleEndpointBindingRuntimeSettings` |
| D1 输出 | Config-Version-owned proxy pool、proxy node 与 exact Endpoint-Credential egress binding profile typed records；proxy endpoint 仅接受 local-DNS `socks5://host:port` 且以独立 AEAD domain 保存；pool/node/binding ownership、capacity、weight、target/failure-scope/stickiness/pre-submit retry 全部闭合验证；migration up/down、clone/rollback/reopen/corruption/row-swap/no-secret tests |
| D2 输出 | 受保护 management list/get/create/update/delete：Management Key、same-origin CSRF、`X-Config-Version`、`If-Match`、draft-only atomic revision 与 value-free audit；响应只返回 opaque IDs、enabled、weight/capacity、target/failure/stickiness/retry 与 `proxy_configured`，不返回 proxy URL/ciphertext/key version/credential/请求体 |
| D3 输出 | active Config Version 在进程组合时一次性解密并编译为 P13-11B 既有 fixed/pool registry 与 binding settings；serving hot path 不读 Store/解密；配置漂移、跨 Upstream ownership、未知 target、无成员 pool 或不安全 proxy fail closed；默认无配置仍保持 Direct |
| 所有权模型 | pool 和 node 必须属于一个 exact Upstream；node 只能是 standalone fixed target 或一个 pool member，不得被多个 Provider/Upstream 复用；binding profile 的 Endpoint/Credential 必须是既有同 Upstream binding；target id 只能引用同 Upstream 的 standalone node 或 pool |
| 安全边界 | proxy endpoint 即使不含 user-info 也按敏感运营配置加密；AAD 绑定 Config Version、Upstream、pool/node identity，防 row swap；禁止 `socks5h`、HTTP/HTTPS proxy、userinfo、query/fragment/path；不从 Provider 名、credential format 或客户端请求推断 proxy；不调用 Provider、真实代理、DNS、Autoreg、服务器、staging/production |
...
| 当前状态 | `DONE_WITH_BOUNDARY`：D1 `8849b9c`、D2 `edf1f6f`、D3 `5bd04a7`、boundary fix `acf4e47`、aggregate Full 43/43、[phase review](reports/evidence/p13-11d-phase-review-20260817.md) 与唯一正式 run [31996324578](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/31996324578) 已完成；tag `phase-p13-egress-management-complete` 固定于 `1beb230248fb75ced146b87c547eb020ee9cd010`。P13-11A/B/C 的正式 tag/Gate 不回写或移动；真实 Provider/代理/DNS/生产验证仍明确排除 |

### 19.24 P13-11E Task Card — Provider-specific egress, health, and recovery isolation

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `L`；E0 计划/CR/review，E1 typed state/capability，E2 Build/Console，E3 Web，E4 可选管理投影，E5 独立真实 canary。E0-E4 不调用 Provider/真实代理/DNS；E5 不在本 Task Card 授权 |
| 依赖 | P13-11A/B/C/D；P13-06 CPAR runtime account-pool Health/Quota/Circuit、lease、failure feedback；`CR-P12-AUTOREG-SEPARATION-001/002` |
| E0 输出 | [CR-P13-11-PROVIDER-SPECIFIC-EGRESS-001](change-requests/CR-P13-11-PROVIDER-SPECIFIC-EGRESS-001.md)、[ADR-0097](adr/ADR-0097-provider-specific-egress-health-recovery.md)、[BC-SEC-008](contracts/BC-SEC-008-provider-specific-egress-health-recovery.md)、[P13-11E report](reports/p13-11e-provider-specific-egress-planning.md)；冻结渠道矩阵、状态域、failure ownership、bounded recovery 和 no-fallback |
| E1 输出 | 已在 `gateway-router` 实现 Provider-aware `EgressRuntime`/`ProviderSessionRuntime`/`ClearanceRuntime` capability seam；与 Credential/Quota 状态分离；deterministic clock、rejecting synthetic transport、state transition、cross-Provider isolation、Web sticky-loss 和 hidden-auxiliary-request bounds 测试；all-target `158/158`、strict Clippy/fmt/diff 通过 |
| E2 输出 | Grok Build 与 Grok Console 仅复用 CPAR 已导入 account pool、现有 exact lease 和 adapter transport；Build/Console egress/session/DPoP/bootstrap 各自隔离，不能引入 Autoreg DB/worker 或 Web clearance；仅 local/loopback/fake evidence |
| E3 输出 | 已完成 transport-free Grok Web provider-local seam：exact `grok_web_sso` lease、named sticky egress、active session、exact clearance、generation-owned atomic refresh ticket；未知/confirmed/challenge 403 分离，挑战仅供下一逻辑 attempt 一次有界恢复；Web `11/11`、clearance `13/13`；真实 adapter/proxy pool/FlareSolverr/DNS/public request 另立 CR |
| E4 输出 | `DEFERRED_OPTIONAL`：exact egress/session/clearance 管理投影是独立产品可观测性边界，不阻塞 E0-E3 收口；未来如启动，先新增独立 management contract，按权威 OpenAPI → Prism sync → `docs/cross-boundary-log.md` → Claude Code handoff执行，不能在 E0-E3 隐式改变前端/管理 API |
| E5 输出 | `DEFERRED_UNAUTHORIZED`：真实 Provider/代理/DNS canary、目标/次数/预算/脱敏 receipt/rollback；必须获得新的明确授权，不能把 synthetic 或历史账号结果当作 E5 证据 |
| 所有权模型 | generic endpoint 以 exact Upstream/Endpoint-Credential/egress profile 为边界；Grok Build/Console/Web 使用独立 channel namespace；Official、Kiro、Claude/其他 Provider 不继承 Grok browser/clearance 状态；Autoreg 只交付外部 credential revision，不属于 CPAR executor |
| 失败归属 | pre-submit DNS/TLS/connect/proxy handshake → Egress；401/expiry/confirmed unauthorized → Credential/Session；429/quota → exact Quota；未知 403 → ambiguous；协议/解码 → Adapter；首语义事件后不 replay。投影可排序但不得丢失 source domain |
| 安全边界 | 不跨 Provider/Channel/Credential/egress pool fallback；不从 credential format 或 relay 名称推断代理；不把 Console/Web 辅助 HTTP 隐藏在“一次请求”外；不泄露 URL、Cookie、OAuth/SSO、proxy secret、body、DNS answer 或 Provider response |
| 前后端边界 | E0-E3 无 OpenAPI/Prism/frontend 变化；E4 若改变管理契约，必须同提交更新 authoritative OpenAPI、同步 Prism 并追加 cross-boundary log；没有前端改造可提前开始 |
| 定向验收 | state namespace isolation、unknown/confirmed 403 attribution、sticky loss fail closed、credential/quota/egress/session/clearance separation、auxiliary call bounds、semantic-event no-replay、fake transport zero Provider/DNS calls、no-secret/diff/docs review |
| 当前状态 | `DONE_WITH_BOUNDARY`：E0-E3 本地 Full `43/43`、[phase review](reports/evidence/p13-11e-phase-review-20260817.md) 与唯一正式 run [32044424886](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/32044424886) 已完成；immutable tag `phase-p13-provider-egress-complete` 精确固定于 `ba2261a5414fe73d147a102a266abd3e9a7fbb5b`。Authorize/Fast/Full supply-chain/Required 全部成功（3s/6m41s/1m03s/3s）。Build/Console 当前仅为 typed Direct native seam，Web 仍 transport-free；无 Provider/代理/DNS/服务器/staging/production 验证 |
| 下一步 | E0-E3 收口未自动启动后续；用户随后以独立 [E4 CR](change-requests/CR-P13-11E4-PROVIDER-EGRESS-STATUS-001.md) 启动 §19.25。E4 不属于本 Task Card 的 immutable tag/Gate；E5 保持 `DEFERRED_UNAUTHORIZED`。真实代理池、FlareSolverr、DNS、Autoreg 或 Provider/网络仍需新 CR/授权 |

### 19.25 P13-11E4 Task Card — Provider-specific egress runtime status projection

| 字段 | 固定值 |
|---|---|
| 范围等级 / 预算 | `M`；E4A atomic snapshot/control facade，E4B protected GET/OpenAPI/Prism/client/handoff，最后 aggregate review；每片只跑 focused checks，收口前至多一次 Full/正式 Gate |
| 授权 | 用户于 2026-08-18 明确批准启动；[CR-P13-11E4-PROVIDER-EGRESS-STATUS-001](change-requests/CR-P13-11E4-PROVIDER-EGRESS-STATUS-001.md)；不继承或移动 E0-E3 formal tag/Gate |
| 依赖 | P13-11E E1 typed runtime state；P13-06 Provider pool facade；P10 Management Key/peer-origin/version/no-store admission；P13-11B/D compatible source ownership |
| E4A 输出 | 已完成三个独立 source-domain row 的 one-lock atomic snapshot：Egress、Provider Session、Clearance；Config Version/revision 与 monotonic runtime revision；固定 `sampled_at_ms`；domain-compatible identity/state；bounded filter、stable order、retained snapshot/filter-bound keyset cursor；rejecting/unavailable source |
| E4B 输出 | 已完成受保护、GET-only 的 `GET /admin/operations/provider-egress-status`；strict query/closed response/safe `400/409/503`/`Cache-Control: no-store`；authoritative OpenAPI、Prism sync/generated client freshness 与 Claude Code action-required handoff 已同步；未实现正式 UI |
| 初始 source | 当前 process composition source 只覆盖已存在的 Grok Build/Console runtime facts；production Web/clearance 可如实为空，不能伪造 healthy/available，synthetic HTTP clearance 只证明 serializer contract；generic compatible egress 由独立 P13-11B/D source owner 提供，不按名称、credential format 或 Provider label 静默合并；其他 channel 未注入即不投影 |
| 状态模型 | Egress：available/cooling_down/circuit_open/probe_due/probe_in_flight/disabled；Session：absent/active/expired/challenge_required/invalid；Clearance：absent/fresh/expired/refresh_required/refresh_in_flight/invalid。deadline 仅按 snapshot observation 纯投影 effective state，不触发 runtime mutation |
| 所有权 / 一致性 | exact Provider/Upstream/Endpoint；`channel_id` 即 exact configured `EndpointId`，`channel_kind` 按 exact channel identity 唯一校验但不重复进 cursor key。Session 仅 Console/Web；Clearance 仅 Web 且 Web target 必须 named；`refresh_in_flight` 必须存在 exact owner，但 owner generation/ticket 永不投影。Cursor 同时绑定 Config lineage、runtime snapshot/revision、observation time、filter fingerprint 与 stable key；retained snapshot 在有界保留期内不因 live drift 失效，未知/过期快照或 lineage mismatch safe `409` |
| 安全边界 | value-free IDs/revisions/deadlines only；runtime IDs 按 bytes 有界且 opaque cursor 最长 4096 字符；不返回 URL、proxy endpoint、DNS、API key、OAuth/SSO/Cookie/Header、plaintext/ciphertext、body/raw Provider response、client-key digest、refresh ticket/generation；不聚合 overall health，不把 missing 当 available |
| 明确不做 | operator POST/action、probe/refresh/reauth/replenishment/quota reset、Provider/代理/DNS 请求、Store decrypt、serving lease、Autoreg、server/staging/production、Build/Console fixed/pool wiring、Web production transport/Statsig/FlareSolverr、E5 canary |
| 定向验收 | Router/control atomic snapshot/revision/concurrency/deadline/isolation/bounds/cursor；HTTP admission/version/strict query/no-store/safe errors；OpenAPI/Prism/client/forbidden-field；existing account inventory/provider pool/actions/failures/generic serving/public data-plane regression；zero external/operational side effect；fmt/strict Clippy/docs/secret/diff/review |
| 交付物 | [ADR-0098](adr/ADR-0098-provider-specific-egress-management-projection.md)、[BC-MGMT-019](contracts/BC-MGMT-019-provider-specific-egress-runtime-status.md)、[E4 report](reports/p13-11e4-provider-egress-management-projection.md)；authoritative OpenAPI、Prism contract/generated client 与 [cross-boundary log](cross-boundary-log.md) 已同步 |
| 当前状态 | `DONE_WITH_BOUNDARY`：Rust snapshot/control facade、process adapter、HTTP/OpenAPI/Prism/client、focused evidence、独立最终 review 与 [aggregate local Full](reports/evidence/p13-11e4-aggregate-full-20260818.md) `43/43` 已通过；immutable tag `phase-p13-provider-egress-status-complete` 精确固定于 `ce98faa9306d076f5af53b9eef0c818abb1cb9c8`，[formal Gate 32110872875](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/32110872875) 的 Authorize/Fast/Full supply-chain/Required 全部成功（4s/7m02s/1m00s/3s）。[phase review](reports/evidence/p13-11e4-phase-review-20260818.md) 无剩余 P0-P3 finding。E0-E3 继续独立 `DONE_WITH_BOUNDARY`，E5 继续 `DEFERRED_UNAUTHORIZED` |
| 完成定义 | E4A/E4B focused checks + independent review 后仅可标 `LOCAL_PASS_PENDING_PHASE_GATE`；aggregate Full 与单一 formal Gate 确实通过后才可标 `DONE_WITH_BOUNDARY`。完成只证明已注入 CPAR runtime facts 的安全可见性，不证明真实账号/节点/Provider/网络/生产可用 |
...
> 当前版本置顶变更：`v1.316`（2026-09-03）见下方第一行；历史版本保持原样。

| 版本 | 日期 | 变化 | 批准状态 |
|---|---|---|---|
| v1.316 | 2026-09-03 | P13-15C/D Grok Build 生产验收通过：exact commit `058805e556d5b22a00bd56b846f75ed8b81696fd` 经 release workflow 33651468181 构建并校验 ARM64 artifact 后部署 Oracle Singapore；schema 21、SQLite、health、release/hash 与 rollback invariants 通过。授权 `/v1/models` 返回四个 exact ID 并动态包含 `grok-4.6`，protected catalog 为 1 Fresh/3 Missing，唯一一次真实 non-streaming `grok-4.6` Responses 返回 `200/completed`。只变更 CPAR release/current grant import 与 entitlement sync；未修改 Autoreg/Caddy/Cloudflare/DNS/frontend/其他服务，rollback 未触发。remaining channel sources、target Go/Codex evidence、P13-15E isolation、Claude Code 和 formal Gate 仍待完成。 | `P13-15C/D PRODUCTION_PASS_FOR_GROK_BUILD`; P13-15 `IN_PROGRESS` |
| v1.315 | 2026-09-02 | 完成 P13-15C/D 本地实现：migration 0021 持久化 exact target 的 monotonic success、Fresh/refresh/expiry deadlines、successful-miss removal evidence 与 safe failure；Build/Codex worker 到期发现后从 active Config 模板原子物化 exact Credential-restricted route。所有 lease path recheck credential set，Expired/ambiguous fail closed，HTTP admission snapshot 精确固定到最终 scheduler selection。protected catalog status/OpenAPI 增加 optional operational fields，双语 README/部署指南与 Claude Code handoff 同步。无前端代码、无套餐模型映射、无本地真实 Provider send；Oracle `grok-4.6` acceptance、remaining B channels、E/formal Gate 待完成。 | `P13-15C/D LOCAL_PASS_PENDING_PRODUCTION_AND_PHASE_GATE`; P13-15 `IN_PROGRESS` |
| v1.314 | 2026-09-02 | 用户纠正 CPAR 已保存 refreshable OAuth 的日常续期归属，并指出生产静态列表漏掉 Build `grok-4.6` 与 Codex `gpt-5.6-luna` 等上游模型。新增 `CR-P13-RUNTIME-OAUTH-REFRESH-001`/P13-16；实现 Build/Codex startup catch-up、periodic worker、encrypted CAS 与 atomic runtime replacement，Autoreg 继续仅负责注册/首次授权/revoked grant 交互 reauth/replenishment。P13-15B 新增 Build/Codex exact-Credential catalog sources，真实目录 ID 原样解析且不按套餐硬编码。Gateway 118、相关 crate suites、strict Clippy、fmt/diff 全通过；Oracle deployment/post-refresh continuity、其余 channel sources、P13-15C/D/E 与 formal Gate 待完成，Prism/OpenAPI 未改。 | `P13-15B PARTIAL_LOCAL_PASS`; `P13-16A LOCAL_PASS_PENDING_PRODUCTION`; P13 `IN_PROGRESS` |
| v1.313 | 2026-09-02 | P13-15A production live pass：revision `ac23f387` 的 aarch64 release 上线，生产 catalog 仅广告 exact Candidate ID 且旧 `grok-cpar-build` alias 缺席；修复 Grok native runtime one-based revision 与 Responses typed prompt-cache preservation，跨协议仍拒绝。真实 `grok-4.5` JSON Responses、Pi 0.84.3 streaming Responses、Provider egress、health/SQLite 均通过；Pi/CC Switch 保持 `openai-responses` 与 `/v1`。P13-12 当前 Build entitlement 已 authoritative `supergrok`。A 不代表全渠道动态 discovery，P13-15B-D 与 E 剩余隔离/formal Gate仍未完成，Prism 未改。 | `P13-15A PRODUCTION_LIVE_PASS`; next `P13-15B`; P13-15 `IN_PROGRESS` |
| v1.312 | 2026-09-01 | P13-15A 本地实现收口：认证后的 `/v1/models` 只列 Access Group 可见且 hard-eligible Candidate 的 exact upstream model ID，不再广告 CPAR alias；Chat/Responses/Messages/WebSocket 共用 exact resolver，跨 Route 同名 ID fail closed，请求选择再次约束 Candidate model。同步修复 Grok Build `prompt_cache_key` 的 Client-Key/模型/独立 deployment key HMAC 隔离，并为 systemd 增加第六 credential。workspace tests、strict Clippy、fmt 与 duplicate-ID 回归通过；P13-15B-E 的真实渠道 discovery、durable freshness/removal、Credential-scoped materialization、真实隔离 E2E 和正式 Gate仍未完成，Prism 未改。 | `P13-15A LOCAL_PASS_PENDING_PRODUCTION`; P13-15 `IN_PROGRESS` |
| v1.308 | 2026-08-19 | 审计已 Gate 后端与 GitHub/main 集成差距、全部 59 个分支、CI trigger、缺失的 main ruleset/protection 和公共自助部署缺口；确认后端已 push 到 feature/integration line，main 未合并；本次 Codex-owned README、部署指南、审计清单和 Docker 资产已在本地 checkpoint，Claude Prism 改动仍未提交/推送。新增分离的中英文 README、双语部署指南、后端完成度/完整分支清单、Linux amd64/arm64 source Dockerfile、Linux host-network Compose 与 `.dockerignore`；只承诺已验证/可审查边界，明确无公开 image/release、macOS/Windows/musl/ARMv7/Kubernetes 支持等级。等待 Claude 前端完成后配置 main protection、只走一个 integration PR，旧 P13-08 定向 reconciliation，不逐分支 merge；无 Provider/代理/DNS/server/staging/production/Autoreg，E5 未授权 | public-main/release integration `IN_PROGRESS`; E5 `DEFERRED_UNAUTHORIZED` |
| v1.307 | 2026-08-18 | 清理 P13 总览与优先级段落中遗留的 E4 本地收口状态，将当前事实统一为 `phase-p13-provider-egress-status-complete` / exact `ce98faa9306d076f5af53b9eef0c818abb1cb9c8` / Gate 32110872875 正式 `DONE_WITH_BOUNDARY`；v1.304 等历史时点记录保持原样。用户批准执行既有 Claude Code action-required handoff：前端消费 generated `listProviderEgressStatus`，分区呈现 egress/session/clearance，传 exact Config Version、保留 cursor、在 409 后重启读取；不合成 overall health、不新增 action、不把空 Web/clearance 当作健康。前端完成仍以 Claude Code 实现与 Prism gate 为准；无 Provider/代理/DNS/server/staging/production/Autoreg，E5 仍未授权 | `P13-11E4 DONE_WITH_BOUNDARY`; frontend handoff `IN_PROGRESS`; E5 `DEFERRED_UNAUTHORIZED` |
| v1.306 | 2026-08-18 | P13-11E4 正式收口：annotated tag `phase-p13-provider-egress-status-complete` 精确固定于 pushed closeout commit `ce98faa9306d076f5af53b9eef0c818abb1cb9c8`；唯一正式 Delivery Gate [32110872875](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/32110872875) 由 tag push 触发，exact `headSha` 与 tag peel 一致，Authorize/Fast/Full supply-chain/Required 全部成功（4s/7m02s/1m00s/3s）。结合本地 Full `43/43` 与独立 review，E4 以 `DONE_WITH_BOUNDARY` 验收；唯一 annotation 为 checkout/cache 的 Node20→GitHub Node24 非阻断迁移提示。当前 source 仍仅 Build/Console，production Web/clearance 可空，synthetic clearance 仅 serializer evidence，generic compatible 独立；未调用 Provider/代理/DNS/server/staging/production/Autoreg，E5 仍未授权，四个 helper 未触碰；旧 E0-E3 tag/Gate 不移动 | `P13-11E4 DONE_WITH_BOUNDARY`; P13-11 `IN_PROGRESS`; E5 `DEFERRED_UNAUTHORIZED` |
| v1.305 | 2026-08-18 | 完成 P13-11E4 aggregate closeout candidate：首次 Full 在 source-policy 发现 test fixture `expect()` 后安全停止，修复提交 `f62cd0f44e730aef23fbd04490529f55da1b103d` 改为显式 `must(Result)`；最终 `CHECK_REPORT_PATH=/tmp/cpar-p13-11e4-phase-preflight-20260818-retry.md ./scripts/check.sh full` 于 Darwin arm64 从 `06:59:51Z` 至 `07:01:49Z` 以 `43/43` PASS。新增 [aggregate receipt](reports/evidence/p13-11e4-aggregate-full-20260818.md) 与 [phase review](reports/evidence/p13-11e4-phase-review-20260818.md)；独立 review 无剩余 P0-P3 finding。只允许新的 annotated `phase-p13-provider-egress-status-complete` 固定到包含 receipts 的 exact pushed closeout commit并触发一次 formal Delivery Gate；旧 E0-E3 tag/Gate 不移动。当前 source 仅 Build/Console，production Web/clearance 可空，synthetic clearance 仅 serializer evidence，generic compatible 独立；未调用 Provider/代理/DNS/server/production/Autoreg，E5 仍未授权，四个 helper 未触碰 | `P13-11E4 READY_FOR_FORMAL_DELIVERY_GATE`; P13-11 `IN_PROGRESS`; E5 `DEFERRED_UNAUTHORIZED` |
| v1.304 | 2026-08-18 | 完成 P13-11E4 本地实现、focused gate 与独立最终 review：atomic three-domain runtime snapshot/monotonic revision、retained Config-lineaged cursor、strict source-shape facade、protected GET、OpenAPI/Prism/generated client 和 Claude Code handoff。gateway-router `170/170`、gateway-control `87/87`、gateway bin `114/114`、E4 HTTP `4/4`、management OpenAPI `12/12`、existing management runtime `3/3`、provider-grok 全量套件（authorized/live tests intentionally ignored）、strict Clippy/fmt/Prism check 通过；review 无剩余 P0-P3 finding。当前 source 仅 Build/Console；production Web/clearance 可空，synthetic HTTP clearance 仅 serializer-contract 证据，generic compatible 独立；source-shape 不可用 safe `503`。aggregate local Full/GitHub formal Gate 未运行；无 Provider/代理/DNS/server/production/Autoreg/真实 Health 证据。E0-E3 immutable tag/Gate 不变，E5 仍未授权 | `P13-11E4 LOCAL_PASS_PENDING_PHASE_GATE`; P13-11 `IN_PROGRESS`; E5 `DEFERRED_UNAUTHORIZED` |
| v1.303 | 2026-08-18 | 用户以独立 CR 批准启动 P13-11E4。新增 CR-P13-11E4、ADR-0098、BC-MGMT-019、§19.25 Task Card 与 planning report，冻结 GET-only Provider-specific egress/session/clearance management snapshot：Config/runtime 双 revision、固定 observation、domain row、stable cursor、Build/Console-first source、empty Web/clearance 和 separate generic-compatible source owner。governance slice 不改 Rust/OpenAPI/Prism/cross-boundary log，E4B 实际 contract 变化才执行 authoritative OpenAPI -> Prism sync/client -> Claude Code handoff。无 recovery/action、Provider/代理/DNS/Autoreg/server/staging/production；E0-E3 formal history 不变，E5 仍未授权 | `P13-11E4 IN_PROGRESS`; next `E4A` |
| v1.302 | 2026-08-18 | P13-11E E0-E3 正式收口：immutable annotated tag `phase-p13-provider-egress-complete` 精确固定于 closeout commit `ba2261a5414fe73d147a102a266abd3e9a7fbb5b`；唯一 Delivery Gate run [32044424886](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/32044424886) 的 Authorize（3s）、Fast（6m41s）、Full supply-chain（1m03s）与 Required（3s）全部成功。结合本地 Full `43/43`、独立 phase review、ADR-0097、BC-SEC-008 与 E0-E3 reports，状态为 `DONE_WITH_BOUNDARY`；P13-11 umbrella 继续 `IN_PROGRESS`。唯一非阻断 annotation 为 pinned `actions/checkout` 目标 Node.js 20 但 GitHub 强制 Node.js 24，未影响 Gate。Build/Console 仍仅 typed Direct，Web 仍 transport-free；E4 `DEFERRED_OPTIONAL`、E5 `DEFERRED_UNAUTHORIZED`，未调用 Provider/代理/DNS/server/staging/production，无 OpenAPI/Prism/frontend 变化，四个 helper 未触碰 | `P13-11E E0-E3 DONE_WITH_BOUNDARY`; `P13-11 IN_PROGRESS` |
| v1.301 | 2026-08-17 | 完成 P13-11E E0-E3 aggregate closeout candidate：exact implementation baseline `7c182af13912a72be304448308dba7fda963ec82` 上的本地 Full 从 `15:10:21Z` 至 `15:14:07Z` 以 `43/43` PASS；目标回溯与独立 review 无剩余 P1/P2 blocker。冻结候选 annotated tag `phase-p13-provider-egress-complete`，只能在明确授权后指向包含 aggregate receipt/review 的 exact pushed commit并触发一次正式 Gate。Build/Console 仅证明 typed Direct native seam，Web 仍 transport-free；E4 `DEFERRED_OPTIONAL`、E5 未授权，不声称真实账号、native proxy pool、Statsig/clearance/FlareSolverr、DNS、公网或生产可用。无 OpenAPI/Prism/frontend 变化，不新增 Claude Code handoff，四个 helper 未触碰 | `P13-11E E0-E3 READY_FOR_FORMAL_DELIVERY_GATE`; formal tag/Gate pending explicit authorization |
| v1.300 | 2026-08-17 | 完成 P13-11E E3 Grok Web transport-free seam：绑定 exact `grok_web_sso` Credential lease/revision、named sticky egress、active session 和 exact clearance；gateway-router 新增 generation-owned atomic clearance refresh ticket/singleflight，防 concurrent overwrite、stale/same-deadline ABA 与跨 lineage completion；Web ledger 分别计数 Statsig environment/signer、clearance 与 sole inference，第五 auxiliary、第二 recovery/inference、semantic event 后 replay 均 fail closed；unknown/confirmed/challenge `403` 保持独立 owner，challenge 仅供下一逻辑 attempt 恢复。Web `11/11`、clearance state `13/13`、provider-grok/router regressions、strict Clippy/fmt/docs/diff 通过；无 Autoreg、Provider/代理/DNS/Store/FlareSolverr/server/staging/production，无 OpenAPI/Prism/frontend 变化，四个 helper 未触碰。下一步 aggregate review；E4/E5 不自动启动 | `P13-11E E3 LOCAL_PASS_PENDING_PHASE_GATE`; next `aggregate review` |
| v1.299 | 2026-08-17 | 完成 P13-11E E2 Grok Build/Console native adapter seam：复用 CPAR imported native account pool、exact `CredentialLease`、现有 Build/Console transport 与 Router lease 生命周期；新增 provider-local attempt ledger，Build 在 egress admission/request construction 后计一次 inference，Console 将 DPoP/bootstrap 计入 bounded auxiliary、401 仅标记 exact session `challenge_required` 并停止 E2 内部 refresh/second inference；Build/Console namespace、credential、egress、session 与 generic/Web/其他 Provider 隔离。E2 专项 `4/4`，provider-grok 全量、gateway-router `158/158`、gateway `109/109`、strict Clippy/fmt/diff 通过；无 Autoreg、Provider/代理/DNS/Store/server/staging/production、无 OpenAPI/Prism/frontend 变化；下一片 E3 Grok Web fake-only seam，真实网络仍需 E5 新 CR，四个既有 untracked helper 未触碰 | `P13-11E E2 LOCAL_PASS_PENDING_PHASE_GATE`; next `E3` |
| v1.298 | 2026-08-17 | P13-11E E1 完成本地 typed Provider/Channel capability 与独立 Egress/Provider-session/Clearance state seam：新增 `gateway-router/src/provider_egress_state.rs`，支持 deterministic clock expiry、Web sticky target fail-closed、unknown/confirmed 403 failure ownership 和 bounded hidden-auxiliary/pre-submit ledger；7 个 E1 测试、gateway-router all-target `158/158`、strict Clippy/fmt/diff 通过。未接 Provider/代理/DNS/Store/Autoreg，未改 HTTP/OpenAPI/Prism/frontend/server/staging/production；新增 E1 state report，下一片 E2 Build/Console synthetic adapter seam，四个既有 untracked helper 未触碰 | `P13-11E E1 LOCAL_PASS_PENDING_PHASE_GATE`; next `E2` |
| v1.297 | 2026-08-17 | 用户批准启动 P13-11E E0。新增 CR/ADR/Contract/report，冻结 generic compatible、Grok Build、Grok Console、Grok Web、Official、Kiro/Claude/other Provider 的 egress、Credential、Quota、Session、Clearance 状态域、failure ownership、bounded recovery 和 no-fallback；明确 Autoreg 仍是独立项目。E0 只做计划/本地 review，不改 Rust/OpenAPI/Prism/frontend，不调用 Provider/代理/DNS/服务器/staging/production；下一片为 E1 synthetic typed state/capability。四个既有 untracked helper 未触碰 | `P13-11E E0 PLANNED_REVIEWED`; next `E1` |
| v1.296 | 2026-08-17 | P13-11D 正式收口：新 annotated tag `phase-p13-egress-management-complete` 固定到 exact commit `1beb230248fb75ced146b87c547eb020ee9cd010`；唯一 Delivery Gate run [31996324578](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/31996324578) 的 Authorize/Fast/Full supply-chain/Required 以 2s/7m03s/1m22s/3s 全部成功。D1/D2/D3、aggregate、phase review、ADR/Contract/traceability 统一为 `DONE_WITH_BOUNDARY`；旧 P13-11A/B/C tag 不移动。没有 Provider/代理/DNS/server/staging/production 活动，四个 untracked helper 未触碰 | `P13-11D DONE_WITH_BOUNDARY`; formal evidence: [phase review](reports/evidence/p13-11d-phase-review-20260817.md), [run 31996324578](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/31996324578) |
| v1.295 | 2026-08-17 | P13-11D closeout candidate review 完成：D1/D2/D3、aggregate local Full 43/43、Config-Version/secret/ownership/runtime/Provider 边界与前端 handoff 均完成目标回溯复核，无剩余 P1/P2 blocker。冻结新的 immutable annotated tag `phase-p13-egress-management-complete`，只允许 exact pushed closeout commit 触发一次 Authorize/Fast/Full supply-chain/Required；旧 `phase-p13-egress-complete` 不移动。用户已批准执行正式步骤；未调用 Provider/代理/DNS/服务器/staging/production，四个 untracked helper 未触碰 | `P13-11D READY_FOR_FORMAL_DELIVERY_GATE`; evidence: [phase review](reports/evidence/p13-11d-phase-review-20260817.md) |
| v1.294 | 2026-08-17 | 完成 P13-11D aggregate local Full/review preflight：持久化 receipt [p13-11d-aggregate-full-20260817.md](reports/evidence/p13-11d-aggregate-full-20260817.md) 在 Darwin arm64 以 43/43 通过（01:41:24Z–01:43:02Z）；首次 crate-boundary 失败定位为 `gateway-control` 已声明但白名单漏列 `sha2`，由 `acf4e47` 修复后重跑通过。D1/D2/D3 与 aggregate 均保持 `LOCAL_PASS_PENDING_PHASE_GATE`，不自动运行 GitHub formal Gate；未调用 Provider/代理/DNS/服务器/生产，四个 untracked helper 未触碰 | `P13-11D aggregate local PASS`; formal Gate 需独立授权 |
| v1.293 | 2026-08-17 | 完成 P13-11D3 本地 active runtime composition：enabled node 在启动组合期用 exact Config-Version/Upstream/pool/node AAD 打开并立即重验 local-DNS SOCKS5，编译为现有 Upstream-owned Direct/fixed/pool registry 与 exact Endpoint-Credential binding settings；enabled empty pool、foreign/native owner、orphaned exact binding、wrong AAD、unsafe material、unknown target 和 drift fail closed，请求 hot path 不读 Store/解密，无配置保持 Direct。Gateway 109、Control 77、Router 151、Upstream 37 与 strict Clippy/fmt/docs/secret/diff review 通过；D3 未改 OpenAPI/Prism，D2 Claude Code handoff 继续有效；仅清理本仓库 `target/`，未触碰四个 untracked helper，未调用 Provider/代理/DNS/服务器/生产 | `P13-11D1/D2/D3 LOCAL_PASS_PENDING_PHASE_GATE`; 下一步 D aggregate Full/review |
| v1.292 | 2026-08-17 | 完成 P13-11D2 本地管理契约 slice：新增 protected `/admin/operations` pool/node/exact binding-profile CRUD，复用 Management Key/CSRF/X-Config-Version/If-Match/audit/revision；node endpoint 仅 write-only、local-DNS SOCKS5 立即校验并 AEAD 保存，update omission 保留密文；revisioned success responses 加 `Cache-Control: no-store`。authoritative management OpenAPI 新增 15 operations，Prism contract/generated client 同步，`p10_01`/`p10_04`、Prism/SPA checks、JSON/diff checks 通过；cross-boundary log 已记录 Claude Code handoff。未调用 Provider/代理/DNS/服务器/生产；D3 未启动 | `P13-11D2 LOCAL_PASS_PENDING_PHASE_GATE`; 下一片 P13-11D3 |
...
| v1.161 | 2026-08-04 | `CR-P12-10H-002`：当前无 Grok 业务流量时，P12-10H 合成门槛改为固定 100 次 Console fixture 循环 + 四项 native 支撑回归 + loopback health/auth 边界；100/100 与 4/4 通过，输出 `upstream_request=not_sent`。本次不再等待 72h/1250-success，但 receipt 明确不证明 live Grok 上游可用；grok2api 继续保持已授权停止，旧 CPA/公开入口/CC Switch 未改 | P12-10H `DONE`（synthetic-only）；P12-10 总体仍 `IN_PROGRESS`，待其余收尾项 |
...
| v1.166 | 2026-08-04 | `CR-P12-10H-006`：按用户要求执行真实 Console 公共 HTTP 验收；CPAR client-key `/v1/models` 预检为 200 JSON，但无可见 Grok/Console route，100 次推理零发送并记录 `BLOCKED_NO_GROK_ROUTE`。新增 receipt/review，明确 native Console probe 不能替代 CPAR base URL + client key curl；后续所有 CPAR live channel 测试均采用同一真实数据面规则 | P12-10H `BLOCKED`；待绑定 native Console route 后重跑公共 HTTP matrix |
| v1.167 | 2026-08-04 | `CR-P12-10H-007`：在 Oracle Singapore 隔离 loopback staging 绑定 native Console route，导入 4 条 active Console 记录（源池 898，接受 4，拒绝 0），并以签名 ARM 制品执行真实 CPAR base URL + client key matrix。`/v1/models` 与单候选 route explain 通过；Responses JSON（100 次预算，首个请求）、Chat/Responses/Messages JSON/SSE 六条路径均发送到上游但均为 `http_5xx`，attempt 投影统一为 `EgressRejected/egress`，对应上游 403 分类；无转换前 `ClientRequestError`。输出上限映射修复及本地 gates 通过；账号/图回滚、clean staging、生产不变性通过 | P12-10H `BLOCKED_EXTERNAL_CONSOLE_403`；不声称 Console 公共可用 |
| v1.168 | 2026-08-05 | `CR-P12-10H-008`：按用户要求执行 eligible-pool sweep。只读源池为 Console 898/eligible 25、Build 829/eligible 1；Console 25/25 不同 credential 通过 CPAR 公共 Responses JSON 到达上游且均为 `EgressRejected/egress`，无 retry/fallback；Build 唯一 eligible credential 真实 CPAR Responses JSON 1/1 通过。首轮 route activation-only `RouteNotFound` 零 upstream attempt，重启后排除；两批 rollback、clean staging、grok2api 停止、生产不变性通过 | P12-10H `BLOCKED_EXTERNAL_CONSOLE_403`；其余源账号因无模型证据、过期或 `reauthRequired` 继续 fail closed |
| v1.169 | 2026-08-05 | `CR-P12-10I-001`：按用户要求复用雅加达 `grok-register` 的 refresh-first、交互式授权 fallback 与串行批处理行为；先实现 CPAR 原生 reauth state、CAS 加密替换、单并发 coordinator 与 Device/外部浏览器 bridge 契约。真实 OAuth、服务器、生产图和 grok2api 均不改变 | P12-10I-01 `IN_PROGRESS`；P12-10H 仍为 `BLOCKED_EXTERNAL_CONSOLE_403` |
| v1.170 | 2026-08-05 | P12-10I-01 本地实现与 review 完成：schema v13 新增 reauth state，coordinator 固定单并发/最多 200 条，Refresh→Device Code→Browser SSO fallback、provider 形状校验、AEAD/CAS、transient backoff、operator-required/requeue 和 restart claim 回归通过；provider-grok 3/3、gateway-store 41/41、定向 Clippy `-D warnings` 通过。真实 OAuth、外部浏览器、服务器、生产图和 grok2api 均未触发 | P12-10I-01 `LOCAL_PASS_PENDING_PHASE_GATE`；P12-10H 仍为 `BLOCKED_EXTERNAL_CONSOLE_403` |
| v1.171 | 2026-08-05 | `CR-P12-10I-002`：用户批准最多 2 个 Build OAuth 与 2 个 Console SSO 账号串行恢复，并在 Oracle Singapore 隔离 loopback staging 以 CPAR base URL/client key 做有界 JSON/SSE 验证；凭证只走远端受控内存/临时管道，单账号首败停止，不跨 Provider fallback，生产图与旧 CPA 不变 | P12-10I-02 `IN_PROGRESS`；P12-10I-01 `LOCAL_PASS_PENDING_PHASE_GATE`；P12-10H 仍为 `BLOCKED_EXTERNAL_CONSOLE_403` |
| v1.172 | 2026-08-05 | `CR-P12-10I-003`：P12-10I-02 受控真实结果：Build refresh 2/2 失败，Console SSO 2/2 导入；CPAR `/v1/models` 通过但首条 Responses JSON 为 `EgressRejected/egress`，按边界未发送 SSE；隔离 staging 已恢复/清理，生产 listener/active-version 不变 | P12-10I-02 `BLOCKED_WITH_EVIDENCE`；P12-10I-01 `LOCAL_PASS_PENDING_PHASE_GATE`；P12-10H 仍为 `BLOCKED_EXTERNAL_CONSOLE_403` |
| v1.173 | 2026-08-05 | `CR-P12-10I-004`：用户授权下一步，开始少量串行 SSO→Device OAuth 转换，并先做 Console egress 只读分类；成功的 Build OAuth 才能进入隔离 staging 做 CPAR JSON/SSE，所有值保持远端内存/受控管道，生产与既有失败 tuple 不变 | P12-10I-03 `IN_PROGRESS`；P12-10I-02 保持 `BLOCKED_WITH_EVIDENCE`；P12-10H 仍为 `BLOCKED_EXTERNAL_CONSOLE_403` |
| v1.174 | 2026-08-05 | `CR-P12-10I-004` 结果：5 个新 SSO 串行转换均未产生 Build OAuth，3 个明确在 Device Verify 4xx 终止，2 个仅保留通用异常；Oracle 无凭据 egress 探测 TLS/连接正常、HTTP 4xx、无重定向；无新 CPAR 推理，生产不变 | P12-10I-03 `BLOCKED_WITH_EVIDENCE`；P12-10I-02 保持 `BLOCKED_WITH_EVIDENCE`；P12-10H 仍为 `BLOCKED_EXTERNAL_CONSOLE_403` |
| v1.175 | 2026-08-05 | `CR-P12-10I-005`：用户授权追加 5 个全新 SSO 转换探针；固定串行、每个一次、不重试旧候选，只有成功 refresh 才进入 Build staging | P12-10I-04 `IN_PROGRESS`；P12-10I-03 保持 `BLOCKED_WITH_EVIDENCE`；P12-10H 仍为 `BLOCKED_EXTERNAL_CONSOLE_403` |
| v1.176 | 2026-08-05 | `CR-P12-10I-005` 结果：新增 5 个 SSO 全部 Device Code 2xx 但 Verify 4xx，未产生新 Build OAuth；累计 10 个 SSO 在同一 Verify 边界失败，停止继续盲扫，未进入 CPAR staging，生产不变 | P12-10I-04 `BLOCKED_WITH_EVIDENCE`；P12-10I-03 保持 `BLOCKED_WITH_EVIDENCE`；P12-10H 仍为 `BLOCKED_EXTERNAL_CONSOLE_403` |
| v1.177 | 2026-08-05 | `CR-P12-10I-006`：用户要求使用当前本机 Grok Build 登录会话试用；登记单次、单轮、无工具、无重试的本机 CLI 可用性探针。仅记录固定类别，不导出 CLI 会话凭证，不把它转换为 CPAR Build OAuth，不改服务器、生产图或公开流量 | P12-10I-05 `IN_PROGRESS`；P12-10I-04 保持 `BLOCKED_WITH_EVIDENCE`；P12-10H 保持 `BLOCKED_EXTERNAL_CONSOLE_403` |
| v1.178 | 2026-08-05 | `CR-P12-10I-006` 结果：本机活跃 Grok CLI 进程仍存活但无 `auth.json`；单次 headless continuation 以退出 `1`、固定 `auth_required_or_denied` 终止，未发送 CPAR 请求、未导出凭证；活跃会话与当前仓库目录不匹配，不能证明 CLI 内存会话可被复用 | P12-10I-05 `BLOCKED_WITH_EVIDENCE`；P12-10I-04 保持 `BLOCKED_WITH_EVIDENCE`；P12-10H 保持 `BLOCKED_EXTERNAL_CONSOLE_403` |
| v1.179 | 2026-08-05 | `CR-P12-10I-007` 结果：用户完成一次浏览器登录并批准 Device OAuth；单个 Build 凭证进入 Oracle Singapore 隔离 staging。真实 CPAR curl 的 `/v1/models` 与首个 Responses JSON 成功，第二个 Chat JSON 返回 `http_5xx` 后按首败停止，实际 2/6 请求且未发送 Messages/SSE；账号批次回滚、staging 清理和生产不变性均通过 | P12-10I-06 `BLOCKED_WITH_EVIDENCE`；需先匹配远端发行制品/schema 后另行登记受控复测 |
| v1.180 | 2026-08-05 | `CR-P12-10I-008`：修复 Jakarta Autoreg 根分区满、x.ai Next.js bundle query-string 导致的 `Config fetch failed`，并将已停止 grok2api sink 改为可选跳过；单账号注册 1/1 后以现有 ARM 制品通过有界 pipe 导入 CPAR 生产 native Build pool，批次 `1/1`、密文落库、quick-check/服务/loopback admission 通过；active Config Version 与公开 Grok route 未改变 | P12-10I-08 `BLOCKED_WITH_EVIDENCE`；账号已在池但未绑定 active Grok route，未来 Autoreg 自动跨机交付仍需独立安全 transport |
| v1.181 | 2026-08-05 | `CR-P12-10I-009`：对 P12-10I-08 导入的单个 Build 账号执行 provider matrix：隔离副本上的真实 native Build probe 通过，错误标注为 Console/Web 的同一 Build OAuth 均在导入前拒绝；active Config Version 仍无 Grok route，未发送公共 Grok curl | P12-10I-09 `BLOCKED_WITH_EVIDENCE`；Build native provider 已验证，Console/Web 需独立 SSO/session credential 与单独 route 才能继续 |
| v1.182 | 2026-08-05 | `CR-P12-10I-010`：读取 Autoreg task-55 SSO，通过受控管道完成一次 SSO→Build OAuth 转换与 CPAR 加密导入；Console SSO 导入后真实 native probe 到达 Provider 但为 `EgressRejected/egress`；Web Cookie 秒→毫秒归一化后仍因源会话超过 CPAR 90 天上限而 fail closed。定位并修正 `grok-probe --batch` 把整个 Provider pool 强制要求为单账号的错误，改为只租用指定 batch account；本地 Full/Review 通过，需重建制品后复测 Build，生产图与公开流量不变 | P12-10I-10 `BLOCKED_WITH_EVIDENCE`；Console/Web 外部边界仍未闭合 |
| v1.183 | 2026-08-05 | `CR-P12-10I-011`：精确修复 SHA 的 GitHub Fast/Full/Required gate、ARM manifest/receipt/OCI 与独立 Sigstore 验证通过；Oracle Singapore 隔离副本从 1 个 Build 账号导入一个 SSO-derived 账号形成两账号池，修复后的 batch-scoped probe 精确选择新账号并通过真实上游、Health/Quota、Canonical 与 Chat/Responses/Messages 投影。一次 macOS 毫秒格式错误在 CLI 参数解析前停止并如实记录；替代管道成功。batch rollback、root-only 临时材料清理和生产不变性均通过 | P12-10I-11 `DONE`；P12-10I-10 仅保留 Console egress 与 Web lifetime-policy 阻塞 |
| v1.184 | 2026-08-06 | `CR-P12-10I-012`：Oracle Singapore 同出口、精确 grok2api `v3.0.10`、历史成功 Console 账号的单次公共参考请求仍在 upstream-response 阶段得到 403；与 CPAR 25-account sweep 形成 reference parity。未修改 CPAR Console；临时容器、在线数据库副本与 root-only 目录已清理，生产不变。下一步仅分析 Web 长期限凭证能否安全截断到本地 90 天 | P12-10I-12 `BLOCKED_EXTERNAL_EGRESS_WITH_REFERENCE_PARITY`；P12-10I-10 仅保留 Web lifetime-policy 阻塞 |
| v1.185 | 2026-08-06 | Web source parity 证明约 180 天是上游 Cookie 期限，而 90 天是 CPAR 本地风险窗口。迁移入口现将有效来源持久化为 `min(source expiry, observed+90d)`，输出 value-free capped count；严格 direct importer 仍拒绝 91 天输入。provider 全测试、gateway/provider Clippy 与 review 通过；为减少无效制品轮次，远程签名 artifact 与 P12-10I-14 公共 Web E2E 合并 | P12-10I-13 `LOCAL_PASS_PENDING_WEB_E2E`；P12-10I-14 `IN_PROGRESS` |
...
| v1.190 | 2026-08-06 | `P12-10I-17`：使用精确 SHA ARM64 签名制品在 Oracle Singapore 隔离图中，通过 CPAR base URL + client key 分别测试 Build 与 Console；两者 `/v1/models` 预检通过但首个 Responses 请求均为 `http_5xx`，Web 因唯一候选 `web_expiry_unavailable` 在导入前 fail closed；三批 staging/账号回滚、生产不变性和脱敏 review 通过 | P12-10I-17 `BLOCKED_WITH_EVIDENCE`；Build/Console 需新授权诊断，Web 需有效期限内 session |
| v1.191 | 2026-08-06 | `P12-10I-18`：刷新 Autoreg task-55 的 Build OAuth，在隔离库移除旧 Build 记录后精确导入并通过真实 CPAR base URL/client key 验证；6/6 成功，Responses/Chat/Messages 各 JSON 与 SSE 各 1 次；路由、账号批次和临时 listener 全部回滚，生产不变 | P12-10I-18 `DONE`；Build 文本通道已闭合，Console/Web 仍按既有边界处理 |
| v1.192 | 2026-08-06 | `P12-10I-19`：Autoreg 新账号注册 1/1；Console 与 Web 独立导入并通过真实 CPAR `/v1/models`。Console Responses JSON 成功后 Chat JSON 5xx 首败；Web 在 90 天 capped session 下真实请求到达上游但为 `EgressRejected/egress`。修正 Web staging helper 的 Build-only capability override，所有隔离图/账号批次回滚，生产不变 | P12-10I-19 `BLOCKED_WITH_EVIDENCE`；Console/Web 外部边界仍未闭合 |
...
| v1.197 | 2026-08-08 | `CR-P12-CODEX-CRED-001`：确认 Codex/ChatGPT OAuth 渠道必须统一兼容三类凭证：CPAR 原生 OAuth、CPA 导出 JSON、sub2api 导出 JSON。OpenAI-compatible runtime 新增受限 allow-list 归一化（扁平/`token` 嵌套、`expires_at`/`expires_at_ms`/`expires_in`），管理前端新增一次性 JSON 文件导入并转为 write-only `oauth_json` secret；新增格式与未知字段回归。直接 OAuth 授权端点仍是 P12-10I 后续实现项，当前管理端 OAuth workflow 仍为 fail-closed 占位，不宣称已完成真实登录 | P12-CODEX-CRED-001 `LOCAL_PASS`; P12-10 总体状态不变 |
| v1.198 | 2026-08-08 | `CR-P12-CODEX-CRED-002`：扩展 sub2api 兼容到 ChatGPT Go 导出 envelope（顶层 `auth_mode/tokens/_meta`），仅读取 `tokens` 内认证字段并从 JWT `exp` 推导本地 expiry；用户提供文件完成离线导入验证，未发网络请求。确认当前管理 API 没有 secret 导出端点；CPA/sub2api 可选导出列为下一项受控管理操作，禁止把元数据、日志或普通 GET 变成明文泄露面 | P12-CODEX-CRED-002 `LOCAL_PASS`; export format selector `PLANNED` |
| v1.199 | 2026-08-08 | `CR-BUILD-LOCAL-001`：日常开发与隔离 staging 默认在目标服务器本机使用锁定 Rust 工具链和持久 Cargo 缓存构建，减少 GitHub runner 排队、依赖安装与重复 release 流程；每个 P 仅在 closeout 时最多执行一次 GitHub 双架构 `release-artifact`，纯文档/状态和普通 Task 不触发发布构建。生产发布仍必须使用 ARM64 原生 runner 产物、SBOM、manifest、Sigstore 签名和 receipt；本机构建只用于开发/staging，不降低正式发布证据边界 | APPROVED；待在 Oracle staging 安装构建工具链并完成一次本机 ARM64 构建验证 |
| v1.200 | 2026-08-08 | `CR-BUILD-LOCAL-001` 首次实测：Oracle Singapore 已具备 Rust `1.97.1`、clang/cmake/libclang、Node/npm 与 ripgrep；当前工作树在 ARM64 本机以 `cargo +1.97.1 build --locked --release --package gateway` 成功，冷缓存完成约 1m15s，热构建约 56s，产物为 Linux ARM64 ELF。隔离 staging 用新构建完成 credential binding、重启后 capability profile、route validate 与真实 CPAR `/v1/models` 预检；ChatGPT Go 临时请求确实从 `cpar.example.invalid` 发出并返回 `CredentialUnauthorized/credential`，证明当前绑定/入口/请求发送链路已闭合，但暂用 Krill Codex endpoint 不适用于官方 ChatGPT Go OAuth，未将其误判为 Go 账号失败。staging、临时凭证和 Caddy 临时切换均已清理，旧 CPA/生产 CPAR 不变 | `CR-BUILD-LOCAL-001` `LOCAL_PASS`; ChatGPT Go official upstream `BLOCKED_WITH_EVIDENCE`; 下一步实现官方 Codex endpoint/header 轮廓后再做一次真实验证 |
| v1.201 | 2026-08-08 | `CR-P12-CODEX-OAUTH-003`：按旧 CPA/sub2api 行为复核官方 ChatGPT OAuth 出口：OAuth 不走 Krill/API-key base URL，而固定 `https://chatgpt.com/backend-api/codex/responses`；请求需 `Authorization: Bearer`、`ChatGPT-Account-ID`（由 token/account 元数据绑定）、`OpenAI-Beta: responses=experimental`、Codex `originator`/版本/UA 身份，OAuth Responses 需 `store:false`，并保留 Codex namespace/tool 字段；Chat/Anthropic 输入先桥接为 Responses，回程再投影。当前 CPAR 仅实现通用 OpenAI-compatible bearer + Krill UA，尚未实现该官方出口/头/恒定 SSE 轮廓，因此不重发 502 请求；下一步只实现并测试官方 Codex adapter 与严格 headers，完成离线回归后再做一次 Oracle staging 真实验证 | `CR-P12-CODEX-OAUTH-003` `PLANNED`; 官方 OAuth upstream `BLOCKED_WITH_EVIDENCE` |
| v1.202 | 2026-08-08 | `CR-P12-CODEX-OAUTH-003` 实现完成：Codex OAuth 凭证暴露受控 account binding；官方 ChatGPT Codex Responses 目标启用 `store:false`、`chatgpt-account-id`、`OpenAI-Beta: responses=experimental`、`originator`、`version` 和 Codex UA，其余 OpenAI-compatible 路线保持原有头集合；增加 `store:false` 与请求身份回归测试。`provider-openai-compatible` 20/20、`gateway` 82/82 通过，ARM64 Oracle 本机构建通过。隔离 staging 真实验证未发送上游请求：旧 staging SQLite 基线在新构建重启时报告 `gateway runtime is unavailable`，已停止并保留生产不变；需下一步用与当前源码同源的干净 staging 数据库重建图后再做一次真实 CPAR 验证，不能把本次结果记为上游成功或账号失败 | `CR-P12-CODEX-OAUTH-003` `IMPLEMENTED_LOCAL`; 官方 OAuth upstream `BLOCKED_WITH_EVIDENCE` |
| v1.203 | 2026-08-08 | 使用全新 SQLite staging 图完成官方 Codex OAuth 真实探针前置：路由发布、重启、Route Explain 单候选均通过；账号直连 `chatgpt.com/backend-api/codex/models?client_version=0.144.1` 返回可用模型清单（脱敏记录为 `gpt-5.6-terra`、`gpt-5.6-luna` 等），直连 Responses 使用 `gpt-5.6-terra` + `stream:true` 返回 HTTP 200。通过 CPAR staging 真实 Base URL 请求仍返回 HTTP 502，attempt 分类为 `CredentialUnauthorized`；未将其误判为账号失效，因为同一账号同一 Oracle 出口的直连请求成功。当前差异需要继续排查 CPAR 发出的最终 OAuth token/header/body 轮廓（尤其是 credential import 后的 request-scoped token 与官方 Responses 必需字段），生产保持不变，隔离 staging 已清理 | `CR-P12-CODEX-OAUTH-003` `BLOCKED_WITH_EVIDENCE`; direct upstream `PASS`, CPAR adapter `BLOCKED_WITH_EVIDENCE` |
| v1.204 | 2026-08-08 | 对本机提供的 ChatGPT Go JSON 做临时脱敏边界测试：`import_compatible` 成功、account binding 存在、当前 JWT expiry 仍在未来且 `bearer_at(now)` 成功；因此 CPAR 502 的 `CredentialUnauthorized` 不是本地导入/过期判断直接造成。临时测试已删除。下一步继续对 CPAR 最终上游请求的 OAuth header/body 进行受控差分，仍不触碰生产 | `CR-P12-CODEX-OAUTH-003` `BLOCKED_WITH_EVIDENCE`; credential import `PASS` |
| v1.205 | 2026-08-08 | 按 sub2api 复刻 OAuth Responses 的默认 `instructions`（仅请求缺失时注入，不覆盖客户端字段），增加回归断言；并对照尝试 `session_id` 会话头。默认 instructions 变更后的 Oracle ARM64 staging 真实请求仍为 HTTP 502 / `CredentialUnauthorized`，加入 session_id 后结果不变，故已撤回该无效变更。当前保持默认 instructions 适配，session_id 不作为强制头；官方直连仍 HTTP 200，CPAR 差异尚未解决 | `CR-P12-CODEX-OAUTH-003` `BLOCKED_WITH_EVIDENCE`; instructions `IMPLEMENTED_LOCAL`, session header `REJECTED_AS_INSUFFICIENT` |
| v1.206 | 2026-08-09 | `CR-P12-CODEX-OAUTH-003` 根因修复：按 CPA/sub2api 的官方 Codex 请求轮廓固定 ChatGPT Codex endpoint、account binding/Codex headers、`store:false` 与 upstream SSE；对明确 `unknown/unsupported + max_output_tokens` 只做一次 request-local 窄重试；官方 SSE 的 encrypted private reasoning item 进入显式 reasoning-suppressed decoder，仅保留可公开 message/tool 生命周期，默认 decoder 仍严格。当前源码同步到 Oracle Singapore ARM64 本机构建并重启隔离 staging，真实 CPAR HTTP harness `12/12`（Chat/Responses/Messages × JSON/SSE × text/tool）通过；provider 24/24、Responses 24/24、gateway 83/83、strict Clippy、fmt/diff 全通过。生产 CPAR、旧 CPA、Caddy/DNS 和正式凭证池不变，无 GitHub CI/push | `CR-P12-CODEX-OAUTH-003` `DONE_FOR_RUNTIME`; 直接 OAuth UI/PKCE、CPA/sub2api 导出格式选择与元数据展示仍 `PLANNED` |
| v1.207 | 2026-08-09 | operator 确认 P12 Codex runtime staging 验证可计为通过；`CR-P12-CODEX-OAUTH-003` 从 `DONE_FOR_RUNTIME` 收口为 `DONE`。下一阶段按既定顺序进入 Codex 管理面：直接 OAuth PKCE/device 授权与 refresh CAS；随后实现 CPA/sub2api 导出格式选择，并把套餐、额度、平台、邮箱等非认证元数据投影到受保护管理读模型。不得把 secret 放入普通 GET、日志、receipt 或导出以外的响应；runtime 12/12 evidence、生产不变性和降低 CI 频率规则保持不变 | `CR-P12-CODEX-OAUTH-003` `DONE`; `CR-P12-CODEX-MGMT-001` `PLANNED` |
| v1.208 | 2026-08-09 | operator 调整执行边界：`CR-P12-CODEX-MGMT-001/002/003` 先只实现后端管理 API、状态机、持久化/CAS、受保护元数据投影和接口级回归；前端暂不做正式交互，仅保留最小测试页/脚本验证 API。后端接口稳定、脱敏和权限边界通过后，再单独安排正式管理 UI；不因前端未完成阻塞 P12 Codex 后端验收 | `CR-P12-CODEX-MGMT-001` `PLANNED_BACKEND_FIRST`; `CR-P12-CODEX-MGMT-002/003` `PLANNED_BACKEND_FIRST` |
| v1.209 | 2026-08-09 | 完成 P12 Codex 管理 API 的后端契约草案：固定 OAuth start/status/cancel 生命周期、一次性 state/PKCE 与摘要存储边界、token exchange/refresh CAS、CPA/sub2api 显式导出接口、受保护 metadata read model 及验收顺序。仅新增契约文档，未接真实 OAuth、未改生产、未制作正式前端 | `CR-P12-CODEX-MGMT-001` `CONTRACT_DRAFT`; `CR-P12-CODEX-MGMT-002/003` `CONTRACT_DRAFT` |
| v1.210 | 2026-08-09 | 按旧 CPA/Sub2API 已验证的“严格 token 响应 + refresh rotation”行为，在 `provider-openai-compatible` 增加后端 `CodexOAuthRevisionedCredential` CAS 包装：成功响应校验后才递增 revision，过期 worker 以 value-free `Conflict` 拒绝且不改变新 token；新增 secret-redaction/stale-worker 回归。未接真实 OAuth、未改管理路由/生产 | `CR-P12-CODEX-MGMT-001` `LOCAL_PASS_PARTIAL`; provider 25/25 |
| v1.211 | 2026-08-09 | 新增后端 `codex_oauth_management` session 状态机：随机 state/PKCE verifier 仅以摘要进入会话状态，原始值只保留在受控进程内存；支持 pending→complete/failed/cancelled/expired 单向迁移、10 分钟 TTL、绑定 credential、value-free failure class 与终态拒绝。gateway-http-actix 45 个单元/资源测试及既有 management contract 全部通过；尚未接真实 callback/token exchange，前端仍未开始 | `CR-P12-CODEX-MGMT-001` `LOCAL_PASS_PARTIAL`; session state machine `DONE_LOCAL` |
| v1.212 | 2026-08-09 | 将 session 状态机接入新的 `CodexOAuthManagementWorkflow` 后端 workflow：`start/status/cancel` 使用真实 10 分钟 TTL、自动 expired 投影和单向终态；既有 rejecting workflow 与历史 contract 保持兼容。gateway-http-actix lib 45/45 通过，未接真实 callback/token exchange、未改生产、未做正式前端 | `CR-P12-CODEX-MGMT-001` `LOCAL_PASS_PARTIAL`; management workflow `DONE_LOCAL` |
| v1.213 | 2026-08-09 | 新增受保护 `POST /admin/credentials/{credential_id}/oauth/callback` 后端接口：仅接收有界 `state`，由 workflow 校验 session 绑定并执行一次性终态迁移；state mismatch/未知/过期统一 value-free conflict，成功只返回 credential_id 与 complete 状态，不返回 code/token。gateway-http-actix lib 45/45、OpenAPI contract 7/7、management resources 2/2 通过；token exchange 仍未接入，前端仅保留后续测试调用 | `CR-P12-CODEX-MGMT-001` `LOCAL_PASS_PARTIAL`; callback boundary `DONE_LOCAL` |
| v1.214 | 2026-08-09 | 按旧 CPA/Sub2API 的固定 token endpoint、refresh form、Bearer/expiry 校验和 rotation 语义新增 `CodexOAuthTokenTransport` 与 `refresh_with_transport`：请求前先做 revision CAS，stale worker 不发网络请求；响应只经严格 parser 后才更新 access/refresh/expiry，transport 与错误均 value-free。provider-openai-compatible 26/26 通过；尚未接真实 HTTP transport、未改生产 | `CR-P12-CODEX-MGMT-001` `LOCAL_PASS_PARTIAL`; token transport boundary `DONE_LOCAL` |
| v1.215 | 2026-08-09 | 新增 `CodexOAuthRefreshCoordinator` 后端编排：把 revisioned credential 与 injected transport 收口到单一 refresh 边界，捕获当前 revision、调用固定 token transport、严格解析后才推进 revision，并为管理持久化暴露 value-redacted credential/revision。provider-openai-compatible 27/27 通过；真实 HTTP 与管理 SQLite CAS 仍未接入 | `CR-P12-CODEX-MGMT-001` `LOCAL_PASS_PARTIAL`; refresh coordinator `DONE_LOCAL` |
| v1.216 | 2026-08-09 | 在 `gateway-control` 增加 `update_credential_if_revision`：OAuth refresh worker 必须同时匹配 Config Version revision 与 Credential record revision，stale worker 在 AEAD seal/事务写入前以 `CredentialRevisionConflict` 终止；错误映射为管理 409，现有 gateway-control 36/36 通过。尚未把真实 coordinator 接入持久化调用链，前端/生产不变 | `CR-P12-CODEX-MGMT-001` `LOCAL_PASS_PARTIAL`; persistence CAS boundary `DONE_LOCAL` |
| v1.217 | 2026-08-09 | 修正 callback 完成语义以匹配 CPA/Sub2API：callback 现在必须提交有界 state + authorization code；workflow 先校验一次性 state，再调用注入的 `ManagementCodexOAuthExchange`，只有 exchange 已验证并持久化成功才进入 complete。默认 exchange fail closed，失败投影为 token_exchange_failed 且 code 由 Zeroizing 临时持有，不进入响应/日志。gateway-http-actix 45/45、management resources 2/2 通过 | `CR-P12-CODEX-MGMT-001` `LOCAL_PASS_PARTIAL`; callback-to-exchange orchestration `DONE_LOCAL` |
| v1.218 | 2026-08-09 | 新增 `persist_oauth_credential_if_revision`，把经严格解析的 OAuth envelope 固定为 `oauth_json` 并复用 CPAR 既有 AEAD、审计、Config Version transaction 与 Credential revision CAS；新回归证明首次 0→1 成功、旧 revision 再写被拒且持久 revision 保持 1。gateway-control 37/37 通过；真实 authorization exchange transport 尚未组合 | `CR-P12-CODEX-MGMT-001` `LOCAL_PASS_PARTIAL`; encrypted OAuth persistence `DONE_LOCAL` |
| v1.219 | 2026-08-09 | 为 `CR-P12-CODEX-MGMT-002` 增加后端一次性解密出口 `open_credential_for_export`：仅返回受 `PlaintextSecret` 保护的临时值，使用现有 credential AAD 解密，普通 Credential GET 仍不含 secret；新增导出边界回归。gateway-control 38/38 通过，CPA/sub2api JSON 格式适配器尚未接入 | `CR-P12-CODEX-MGMT-002` `LOCAL_PASS_PARTIAL`; one-time secret handoff `DONE_LOCAL` |
| v1.220 | 2026-08-09 | `provider-openai-compatible` 新增 allow-listed `CodexCredentialExportFormat::{Cpa,Sub2Api}` 与 `export_json`：按旧 CPA flat envelope / Sub2API `type=codex` + nested `token` 形状导出已验证 access/refresh/expiry/account 字段，API key 不可导出为 OAuth；未知格式 fail closed，输出以 Zeroizing bytes 返回。provider-openai-compatible 28/28 通过，尚未接 HTTP export route/metadata projection | `CR-P12-CODEX-MGMT-002` `LOCAL_PASS_PARTIAL`; format adapters `DONE_LOCAL` |
| v1.221 | 2026-08-09 | 接入受保护 `POST /admin/credentials/{credential_id}/export`：allow-list `format=cpa|sub2api`，通过现有 AEAD `PlaintextSecret` 一次性打开后重新走 CPAR importer，再调用格式适配器；响应设置 `Cache-Control: no-store`，记录 `credential_exported` 管理审计，失败不回显 secret。gateway-http-actix lib 45/45、management resources 2/2 通过；正式前端仍未开始 | `CR-P12-CODEX-MGMT-002` `LOCAL_PASS_PARTIAL`; export HTTP boundary `DONE_LOCAL` |
| v1.222 | 2026-08-09 | 为 CPA/sub2api 导出补齐 round-trip 回归：CPAR 原生 OAuth 分别导出 flat CPA 与 nested sub2api envelope，再通过现有兼容 importer 导回，Bearer、account binding 与 expiry 语义保持一致；provider-openai-compatible 29/29 通过。下一步进入 `CR-P12-CODEX-MGMT-003` 元数据受保护读模型 | `CR-P12-CODEX-MGMT-002` `DONE_LOCAL`; round-trip evidence `PASS` |
| v1.223 | 2026-08-09 | `CR-P12-CODEX-MGMT-003` 首段完成：导入时从 CPA/sub2api `_meta`/token/root allow-list 提取 plan、quota、platform、email、source_format 到独立非 secret `CodexCredentialMetadata`；新增受保护 `GET /admin/credentials/{credential_id}/metadata`，仅投影 metadata + credential kind/revision，不返回原始 JSON 或认证字段。provider 30/30、gateway-http-actix lib 45/45 通过 | `CR-P12-CODEX-MGMT-003` `LOCAL_PASS_PARTIAL`; metadata projection `DONE_LOCAL` |
| v1.224 | 2026-08-09 | 完成管理收口回归：`CodexCredentialMetadata` 自定义 Debug 对邮箱做 `[REDACTED]`，metadata 与 credential revision/secret-present 关系保持独立；provider metadata 30/30、gateway-control 38/38、gateway-http-actix lib 45/45、OpenAPI 7/7、management resources 2/2 全通过，CPA/sub2api round-trip 与 export/metadata 接口边界均通过。三项后端管理任务达到 `DONE_LOCAL`，正式前端明确后置 | `CR-P12-CODEX-MGMT-001/002/003` `DONE_LOCAL`; backend-first closeout `PASS_LOCAL` |
| v1.225 | 2026-08-09 | 将 OAuth callback、CPA/sub2api export、metadata 三组新后端路由补入 `docs/openapi/management-v1.json`，新增 `CredentialExportInput`/`CredentialMetadata` schema，并重新生成 `web/admin-ui/src/generated/management-client.ts`；仅同步测试客户端契约，不制作正式 UI。OpenAPI JSON parse 与 client generation 通过 | `CR-P12-CODEX-MGMT-001/002/003` `DONE_LOCAL`; minimal-client-contract `PASS_LOCAL` |
| v1.226 | 2026-08-09 | 按方式一增加后端 OAuth 启动材料：`CodexOAuthManagementWorkflow.start` 生成固定 OpenAI authorize URL，带一次性 base64url state、S256 PKCE challenge、固定 client_id/redirect_uri；`start` 响应仅额外返回 `authorization_url`，status 不重复返回。callback 将 base64url state 解码后再校验 session digest。OpenAPI OAuthOperation 同步 expired/authorization_url，gateway-http-actix lib 45/45、client generation 通过；真实浏览器授权和 token exchange 仍未启动 | `CR-P12-CODEX-MGMT-001` `LOCAL_PASS_PARTIAL`; backend-start-url `DONE_LOCAL` |
| v1.227 | 2026-08-09 | 完成方式一启动链路的本地编译收口：`cargo check -p gateway --all-targets` 通过；新增 URL/PKCE/state 逻辑未触碰生产或公网入口。下一步必须在隔离 staging 调用 start，用户在浏览器完成一次授权，再回传 callback code；真实 token exchange 需要受控 OAuth transport，未在无人值守状态下自动发起 | `CR-P12-CODEX-MGMT-001` `LOCAL_PASS_PENDING_STAGING_AUTH` |
| v1.228 | 2026-08-09 | 按 CPA/Sub2API 源码逻辑一次性收口 OAuth 后端：接入官方 authorization-code exchange 与独立可选 SOCKS5 egress；refresh 使用 `grant_type=refresh_token`、固定 client id、`scope=openid profile email`，严格解析可选 rotation 字段并以 revision CAS 写回；callback 支持 relay pair/完整 loopback URL/provider rejection，pending start 幂等、错 state 不消费会话；active `oauth_json` 在进程重启后从 durable control plane 投影为 `complete`。新增刷新 claim 的 RAII 释放；session 进入 complete/failed/cancelled/expired 后立即 zeroize 并释放原始 state/verifier，避免异常路径永久卡住同一凭证或终态保留 replay material | `CR-P12-CODEX-MGMT-001` `IMPLEMENTED_BACKEND`; local implementation `PASS` |
| v1.229 | 2026-08-09 | 完成 OAuth 全链路回归与文档 review：CPA flat、Sub2API nested、ChatGPT Go envelope 导入；CPA/Sub2API 导出 round-trip；plan/quota/platform/email/source metadata 脱敏与权限；PKCE、duplicate start、state mismatch、provider rejection、refresh rotation/account drift/stale revision/concurrent claim 均通过。联合测试为 provider 35、gateway-http-actix lib 54、management resources 3、gateway 85 + component smoke 1；strict Clippy、fmt、relay `py_compile` 通过。Oracle staging 已复核持久 OAuth status=`complete`；此前有效真实 CPAR runtime receipt 仍为 12/12。最近 fresh client-key 仅在路由前 401，不计作 OAuth 失败或新通过；生产和 GitHub CI 均未触碰 | `CR-P12-CODEX-MGMT-001/002/003` `DONE_LOCAL`; backend-first closeout `PASS_WITH_BOUNDARY` |
| v1.230 | 2026-08-09 | operator 明确授权把 staging 通过的 ARM64 本机构建部署到公网 CPAR。先创建 root-only 二进制/Caddy/unit/SQLite 回滚包，在线 `quick_check=ok`；生产数据库快照预启动新 binary 的 `/v1/models` 与 Chat JSON 均 200。原子切换 `current` 后服务 active、health=200、真实 client-key `/v1/models`=200、active Config Version/Caddy 不变。公网 Base URL 验收按首败规则在不同轮次分别于 Chat SSE text/tool 停止；本机 18180 同形 text/tool SSE 均通过，日志没有 CredentialUnauthorized，故不归因 OAuth 回归；生产保持新 binary 但公网验收仍为 `BLOCKED_PUBLIC_SSE_INTERMITTENT`。本次为本机 ARM64 build，未运行 GitHub CI/Sigstore，不能标记正式 signed release closeout | `CR-P12-CODEX-PROD-DEPLOY-001` `DEPLOYED_WITH_BOUNDARY`; receipt/review: [`p12-codex-production-deployment-20260809.md`](reports/evidence/p12-codex-production-deployment-20260809.md)
...
| v1.236 | 2026-08-10 | `CR-EXEC-008` 实施收口：`.github/workflows/ci.yml` 仅承担 PR/main 的轻量检查；新增 `delivery-gate.yml`，只在 `phase-p*-complete` tag、显式 `workflow_dispatch` 或 `delivery-closeout` label 下运行 Fast/Full/supply-chain。closeout label 绑定 exact head SHA；PR synchronize 后旧 label 会显式失败并要求重新授权，避免 skipped required job 伪通过。新增 live credential expiry 过滤：编译后/租约前按 `expires_at_ms` 跳过过期 Build credential，保留密文、revision 与 reauth 边界；`gateway-upstream`、`gateway-router`、`provider-grok` 定向测试与 strict Clippy/fmt 通过。push、PR、review、rebase、merge 频率不受限制；未主动启动昂贵 Delivery Gate，push 后仅自动触发轻量 PR workflow；branch-protection API 403 作为已知边界记录 | `CR-EXEC-008` `IMPLEMENTED_LOCAL`; evidence: [`ci-delivery-gate-split-20260810.md`](reports/evidence/ci-delivery-gate-split-20260810.md) |
...
| v1.238 | 2026-08-10 | `CR-P12-AUTOREG-CONSOLE-001`：先修复 CI clean-runner 的 Node/npm 管理面依赖顺序，轻量 PR gate 在 commit `05da177` 上通过且未启动昂贵 Delivery Gate。随后从 Oracle Autoreg task-67 以严格 source adapter 导出一个 Console SSO，隔离 CPAR import/probe 与 Responses/Chat/Messages × JSON/SSE `6/6` 通过并回滚；在 operator 批准下将同一 `grok_console` batch 保留到生产。首轮公网 6/6 因发生在服务重载前降级为 pre-reload 观察，不计最终验收；随后重启 CPAR、同批次 native probe 通过，post-reload 真实公网六元组 `6/6` 通过。Oracle service active、Jakarta 已 fenced/stopped、数据库快照和 batch rollback window 保留；scheduler/自动 reauth 仍手动/禁用，旧 CPA/Caddy/DNS/CC Switch/grok2api 未改。此前旧 harness 误用 `--help` 产生的 12 次无关生产请求不计入验收 | `CR-P12-AUTOREG-CONSOLE-001` `DONE_WITH_BOUNDARY`; evidence: [`autoreg-oracle-console-source-adapter-20260810.md`](reports/evidence/autoreg-oracle-console-source-adapter-20260810.md)、[`autoreg-oracle-console-production-canary-20260810.md`](reports/evidence/autoreg-oracle-console-production-canary-20260810.md)、[`autoreg-oracle-console-production-canary-20260810-review.md`](reports/evidence/autoreg-oracle-console-production-canary-20260810-review.md) |
| v1.239 | 2026-08-10 | `CR-P12-AUTOREG-CONSOLE-001` 运行时来源修订：源码复核确认 native account pool 在服务启动时编译，`grok-import` 不热加载。因而将导入后、重启前的公网 6/6 明确标为非验收观察；在不改变 active Config Version/route/DB 的单次服务重启后，同批次 probe 再次通过，post-reload 公网 Responses/Chat/Messages × JSON/SSE `6/6` 才作为最终来源闭环。 | `CR-P12-AUTOREG-CONSOLE-001` `DONE_WITH_BOUNDARY`; post-reload evidence 追加至同一 receipt/review |
...
| v1.252 | 2026-08-15 | 启动 P13-06C Task Card：冻结受保护 `cool_down`/`request_recovery` operator actions 与脱敏 Attempt failure-feedback 读模型；动作只改变同一 serving Health/Quota registry 的有界本地状态并记录 resource audit，不修改 Config revision、不发送 Provider 请求、不启动自动 refresh/reauth。后端 contract 变更要求 Prism `sync-contract`，并在同一提交更新 `docs/cross-boundary-log.md` 通知 Claude Code；暂不实现正式前端 UI | `P13-06C` `IN_PROGRESS`; evidence target: [`p13-06c-provider-account-operator-actions.md`](reports/p13-06c-provider-account-operator-actions.md) |
...
| v1.257 | 2026-08-15 | 完成 P13-07A 本地实现与 review：`gateway-router` 新增 side-effect-free `ProviderScopedSelector`，对 exact Provider scope 内的 Health、Quota、capability、expiry、并发、cost、load、priority 与 weight 做有界 deterministic eligibility/ranking；foreign Provider 永不成为 fallback，unknown cost/quota 不伪造为零或无限，load ratio 使用 `u128` 交叉乘法。候选数、opaque identity、空白和 duplicate identity 在排序前 fail closed。聚焦 selector 9 tests、router 全量回归、strict Clippy、source/crate-boundary/docs/diff checks 通过；未获取 lease、未读 Store、未调用 Provider、未改 OpenAPI/Prism/production，也未触发昂贵 GitHub Delivery Gate。下一片 P13-07B 才把 selected Config/runtime observations 接入既有 scheduler/Route Explain | `P13-07A` `LOCAL_PASS_PENDING_PHASE_GATE`; evidence: [`p13-07a-provider-scoped-selector.md`](reports/p13-07a-provider-scoped-selector.md) |
| v1.258 | 2026-08-15 | 完成 P13-07B 本地实现、定向门禁与独立 review：`RouteCredentialScheduler` 新增只读 provider-scoped Route Explain seam，管理 facade 复用 serving 使用的同一 scheduler、credential pools、Health/Quota registry；expiry 在边界时刻 fail closed，exact quota 缺失保持 Unknown，显式 `provider_id` 限定 Provider，多个 Provider 未指定范围时 `provider_scope_required`，外部 Provider 为 `provider_mismatch`，不隐式 fallback。同步 `docs/openapi/management-v1.json` 与 Prism contract/generated client，并在 `docs/cross-boundary-log.md` 留下 Claude Code 前端 Provider selector/安全 reason 展示待办。路由/运行时/HTTP/契约/文档本地检查通过；不获取 lease、不调用 Provider、不改变公开 serving 请求路径、不改 production、不启动昂贵 GitHub Delivery Gate。下一片为 P13-07C serving-path lease revalidation | `P13-07B` `LOCAL_PASS_PENDING_PHASE_GATE`; evidence: [`p13-07b-provider-scoped-route-explain.md`](reports/p13-07b-provider-scoped-route-explain.md) |
| v1.259 | 2026-08-15 | 启动 P13-07C serving-path lease revalidation：将 P13-07A selector 的 advisory candidate order 接入 request-time serving，但真实 lease 仍只由同一 `RouteCredentialScheduler`/`EndpointCredentialPools`/Health/Quota registry 在 fresh observation time exact revalidate；lease race 只在同一 Provider 内尝试下一个已排序候选，不增加 attempt 或放宽 `max_attempts`。多 Provider 无显式内部 scope 在 lease/driver 前 fail closed；AttemptOrchestrator 保持 retry、first-semantic-event、timeout/cancellation、Config Version 与 lease release owner。新增 ADR-0086、BC-ROUTE-008、P13-07C report；不改变公开协议/OpenAPI/Prism/frontend，不调用 Provider、不改 production/server/traffic，不启动 P13-08/11/12 | `P13-07C` `IN_PROGRESS`; evidence target: [`p13-07c-serving-lease-revalidation.md`](reports/p13-07c-serving-lease-revalidation.md) |
| v1.260 | 2026-08-15 | 完成 P13-07C 本地实现、定向 Gate 与两轮独立 review：Provider-scoped selector 已接入真实 serving AttemptOrchestrator，但只提供 advisory order；同一 scheduler/pool/Health/Quota 在 lease 前精确重验 Provider/channel/candidate、hard eligibility、binding、expiry、Health、Quota 与原子 capacity。首轮 review 发现 legacy quota-recovery fallback 未携带 Provider scope；修复后 recovery 同样要求 hard eligibility 与 exact upstream→Provider ownership，foreign hard-ineligible recovery-due 回归为零 lease/零 driver，same-Provider recovery 仍成功，复核无剩余 blocker。Router 121、Gateway 100、HTTP 116 passed（4 ignored），strict Clippy/fmt/source/crate-boundary/diff 通过；无 OpenAPI/Prism/frontend/生产/Provider 变更。P13-07C 为 `LOCAL_PASS_PENDING_PHASE_GATE`；因 versioned cost 仍为 `Unknown`，P13-07 保持 `IN_PROGRESS`，下一步先冻结 P13-05 catalog→request routing cost signal 边界 | evidence: [`p13-07c-serving-lease-revalidation.md`](reports/p13-07c-serving-lease-revalidation.md) |
| v1.261 | 2026-08-15 | 启动 P13-07D Config-bound routing price evidence：禁止把 P13-05 六维 token rates 压成伪造的 request `cost_microunits`；新增 draft Config Version 对 exact immutable catalog + `rate_dominance_v1` 的 revisioned/audited binding。Runtime 仅在 composition 按 owning upstream、endpoint、canonical public model 编译共享 immutable map；serving/Explain hot path不读 Store或调用 Provider。六维 coordinate-minimum dominance、crossed incomparable、missing unpriced 与 explicit zero边界冻结；protected management/OpenAPI 变化必须 sync Prism 并为 Claude Code 留痕，不实现正式 UI、不做 staging/production canary、不启动 P13-08/11/12 | `P13-07D` `IN_PROGRESS`; evidence target: [`p13-07d-config-bound-routing-price-evidence.md`](reports/p13-07d-config-bound-routing-price-evidence.md) |
| v1.262 | 2026-08-15 | 完成 P13-07D 本地实现、定向 Gate 与独立 review：Config-Version exact catalog binding、六维 `rate_dominance_v1` evidence、serving/Route Explain shared immutable map、draft-only protected management API、OpenAPI/Prism 82-operation sync 与 PUT generator guard 全部通过；未来/不匹配/越界输入 fail closed，未知价格不伪造为零，请求 hot path 不读 Store/调用 Provider。已补齐 `price_policy: null` disabled 语义、每 candidate 必填 closed evidence、Claude Code cross-boundary 留痕；报告与计划状态更新为 `LOCAL_PASS_PENDING_PHASE_GATE`。非阻断剩余仅为额外 missing-row composition 与 active/archived/unknown HTTP 矩阵证据；P13-07 umbrella 等待一次独立 phase Gate，不启动生产/staging/P13-08/11/12 | `P13-07D` `LOCAL_PASS_PENDING_PHASE_GATE`; evidence: [`p13-07d-config-bound-routing-price-evidence.md`](reports/p13-07d-config-bound-routing-price-evidence.md) |
| v1.263 | 2026-08-15 | P13-07A/B/C/D closeout preflight 与独立 phase review 完成：`CHECK_REPORT_PATH=/tmp/cpar-p13-07-phase-preflight-20260815.md ./scripts/check.sh full` 在 Darwin arm64 上 43/43 steps PASS（Prism 82-operation contract/generated-client 双构建、fmt、workspace strict Clippy/tests、serve/offline regressions、source/crate/docs/contract/secret/whitespace、cargo-deny 与 RustSec），并修正 ADR/Contract/traceability 的状态漂移和 P13-07B/D cross-boundary 归属。形成 annotated `phase-p13-routing-complete` 唯一远端 closeout target；只有 exact tag 的 Authorize/Fast/Full supply-chain/Required 全 PASS 后才将 P13-07 标记 `DONE_WITH_BOUNDARY`。不调用 Provider、不改 server/production/staging、不启动 P13-08/11/12 | `P13-07` `READY_FOR_FORMAL_DELIVERY_GATE`; evidence: [`p13-07-phase-preflight-20260815.md`](reports/evidence/p13-07-phase-preflight-20260815.md)、[`p13-07-phase-review-20260815.md`](reports/evidence/p13-07-phase-review-20260815.md) |
| v1.264 | 2026-08-15 | P13-07 正式收口：annotated tag `phase-p13-routing-complete` 固定到 exact commit `0c338ee8eef76e470c55515a24728324684365c5`，唯一正式 Delivery Gate run [31875826495](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/31875826495) 的 Authorize（3s）、Fast（5m57s）、Full supply-chain（1m16s）和 Required（2s）全部 PASS。结合 43/43 本地 Full preflight、独立 phase review、四份 ADR/Contract/report 与 OpenAPI/Prism/Claude Code handoff，P13-07A/B/C/D 和 aggregate 统一收口为 `DONE_WITH_BOUNDARY`。P13 umbrella 因 P13-08 及后续独立 backlog 继续 `IN_PROGRESS`；P13-08 尚未启动，本次未调用 Provider、未改 server/production/staging，也未重复运行第二个昂贵 Gate | `P13-07` `DONE_WITH_BOUNDARY`; formal evidence: [`p13-07-phase-preflight-20260815.md`](reports/evidence/p13-07-phase-preflight-20260815.md)、[`p13-07-phase-review-20260815.md`](reports/evidence/p13-07-phase-review-20260815.md) |
| v1.265 | 2026-08-15 | 启动 `P13-08` Channel Pin diagnostic：冻结受保护 `POST /admin/operations/channel-pin` 的 exact Provider/channel/route/credential/model 与 JSON/SSE 输入；首片固定 bounded probe，复用 serving composition/lease/Health/Quota/egress，最多一次 upstream send，首败、无 retry、无 quota recovery、无 sibling 或跨 Provider fallback；receipt/audit 仅输出 stable IDs、closed 状态、`upstream_sent`/`attempt_count`/`response_started`，禁止 URL/body/header/cookie/token/ciphertext/digest/raw Provider error。已建立 ADR-0088、BC-MGMT-017、P13-08 report 与 traceability；代码、OpenAPI/Prism 同步和定向 review 尚在进行，不调用 Provider、不改 production/staging/server、不运行新的昂贵 Delivery Gate | `P13-08` `IN_PROGRESS`; evidence target: [`p13-08-channel-pin-diagnostic.md`](reports/p13-08-channel-pin-diagnostic.md) |
| v1.266 | 2026-08-15 | P13-08 安全收窄与实现修订：Channel Pin 仅准入 generic OpenAI Chat/Responses 与 Anthropic Messages 的 Canonical/bridge 候选；`NativeExact` 及 Grok Console/Web 等含隐藏 token/Statsig/refresh 请求的原生 adapter 在 lease/network 前 fail closed。管理请求同时要求 `X-Config-Version` 与 `If-Match`；固定 probe 为 OpenAI/Anthropic 加入有界输出上限（Codex OAuth Responses 保留其上游兼容例外）；drain 使用 `CanonicalEventState`，显式识别 `StreamError`/截断/超时，stage 在 drain 失败时更新为闭合 Decoder/BodyRead；全局最多两项并发，request id 使用进程随机 nonce。Pinned Attempt 不再写入 serving event sink；请求与 `channel_pin_started` 在 Provider 调用前写入 value-free audit，返回 receipt 作为终态来源，避免 source drain 前错误成功、队列丢失或 post-send audit 失败造成歧义；OpenAPI/Prism 与 Claude Code cross-boundary 留痕保持同步，继续不调用 Provider、不改 production/staging/server、不运行新的昂贵 Delivery Gate | `P13-08` `IN_PROGRESS`; evidence target: [`p13-08-channel-pin-diagnostic.md`](reports/p13-08-channel-pin-diagnostic.md) |
| v1.267 | 2026-08-15 | P13-08 本地实现与独立 review 收口：新增 cursor-free exact Credential lease，Channel Pin 不推进普通 route/pool 调度游标；同一 serving scheduler/pool/Health/Quota/egress composition 在 lease 前重验 exact Provider/channel/credential。generic Chat/Responses/Messages 固定 probe、一次 driver/send、无 retry/quota recovery/fallback，Canonical drain 与 value-free receipt 完成；native adapters/`NativeExact` 在 lease/network 前拒绝。最终 upstream 32、router 134、gateway 105、HTTP/OpenAPI/security 19、Grok one-shot 4 全部通过，strict Clippy/fmt、Prism、docs/secret/diff 与独立 review 无 P1 blocker；未调用 Provider、未改 production/staging/server、未运行昂贵 Delivery Gate。P13-08 为 `LOCAL_PASS_PENDING_PHASE_GATE`，P13-09 尚未启动 | evidence: [`p13-08-channel-pin-diagnostic.md`](reports/p13-08-channel-pin-diagnostic.md) |
| v1.268 | 2026-08-16 | 启动 P13-09 并完成 A 片 encrypted foundation：新增 migration 0017 与独立 `SqliteStoredResponseStore`，以 exact Client Key + downstream Response ID 做 owner lookup；canonical request/events、public metadata 与 exact Config/Provider/route/Credential lineage 全部进入 domain-separated AEAD，明文仅保留 owner/id/time/version/ciphertext。固定 30-day TTL、read-time expiry、4096-event/8-MiB/4096-GC bounds；identical replay 幂等，冲突、AAD row-swap、ciphertext corruption、missing key、foreign owner、invalid lifecycle 均 fail closed；file reopen 与 old/new Master Key 版本回归通过。A 不开放 `store:true`/GET/DELETE/compact，不改 Prism/serving/production，不调用 Provider；B/C 继续实现公开 retrieval 与 exact continuity/compact | `P13-09A` `LOCAL_PASS_PENDING_PHASE_GATE`; `P13-09` `IN_PROGRESS`; evidence: [`p13-09a-stored-response-foundation.md`](reports/p13-09a-stored-response-foundation.md) |
| v1.269 | 2026-08-16 | 完成 P13-09B public stored-response lifecycle：Responses decoder 接受严格 boolean `store`，opt-in 请求在进入既有 serving path 前把 native payload 规范化为 `store:false`；production routed executor 通过 request-local single-assignment recorder 记录实际成功 Attempt 的 Config/Provider/Upstream/Channel/Route/Candidate/Credential revision。JSON/SSE 共用完整 Canonical payload，逐事件执行 4096-event/8-MiB 捕获边界，只有成功生命周期通过 AEAD transaction 后才向下游发送完成终态；`GET/DELETE /v1/responses/{id}` 先做 Client Key auth 和 exact owner lookup，foreign/missing/expired/deleted 统一 404。HTTP 58、router 135、store 56、protocol 25、gateway 106 unit 回归及全部适用 integration、strict Clippy/fmt/docs/source/crate/secret/diff checks 通过；review 修复捕获时内存边界并保留 production HTTPS-only egress。未启用 `previous_response_id`/compact，不改 management OpenAPI/Prism，不调用真实 Provider或部署生产 | `P13-09B` `LOCAL_PASS_PENDING_PHASE_GATE`; evidence: [`p13-09b-stored-response-public-lifecycle.md`](reports/p13-09b-stored-response-public-lifecycle.md) |
| v1.270 | 2026-08-16 | 启动 P13-09C：冻结 exact stored lineage continuation 与 gateway-owned compact contract。`previous_response_id` 只做 Client-Key-owned 本地解密/Canonical replay，不转发上游；Config/Provider/Upstream/Channel/Route/Candidate/Credential revision 全量匹配并单次 lease，无 cursor、retry、quota recovery、sibling/egress/Provider fallback。新增 `stored_responses`/`response_compaction` 显式 capability，内建 Grok Build=二者、Web=continuity only、Console=neither；compact token 使用独立 AEAD domain/owner/TTL/AAD，foreign blob fail closed。该公开 Responses 数据面扩展不改 management OpenAPI/Prism，不触碰生产/staging/服务器或真实 Provider | `P13-09C` `IN_PROGRESS`; ADR-0091 / BC-RESP-003 |
| v1.271 | 2026-08-16 | 完成 P13-09C 本地实现与 review：公开 Responses 严格解析 owned `previous_response_id`/compact item，本地重放完整 Canonical history且不转发 locator；exact Config/Provider/Upstream/Channel/Route/Candidate/Credential revision 在同一 scheduler/pool/Health/Quota 上单次 lease，不推进 cursor、无 retry/recovery/sibling/egress/Provider fallback。新增 migration 0018 与独立 compact AEAD domain/owner/random locator/TTL/GC，固定 2048 output-token、4096-event、8-MiB、两分钟边界；Build/Web/Console capability matrix显式。protocol 29、router 138、store 58、HTTP 61、gateway 106 及其余 touched-package/integration tests、strict Clippy/fmt/docs/source/crate/secret/diff 通过；review 补齐 corrupt owner-safe 404、model mismatch零执行、TTL起点、Tool order和 capability ledger。未改 management OpenAPI/Prism，未调用 Provider或修改 production/staging/server；P13-09 aggregate 等待一次 Full + 正式 Delivery Gate，P13-10不启动 | `P13-09A/B/C` `LOCAL_PASS_PENDING_PHASE_GATE`; evidence: [`p13-09c-exact-continuity-and-compaction.md`](reports/p13-09c-exact-continuity-and-compaction.md) |
| v1.272 | 2026-08-16 | P13-09 aggregate closeout candidate 完成：exact implementation commit `020aa61055f904f6210b7521252b23d4a503f3a3` 上运行 `CHECK_REPORT_PATH=/tmp/cpar-p13-09-phase-preflight-20260816.md ./scripts/check.sh full`，Darwin arm64 从 `02:45:33Z` 至 `02:48:18Z` 的 43/43 steps 全部 PASS，覆盖 Prism 83-operation 双构建、workspace fmt/strict Clippy/tests、serve/offline regressions、source/crate/docs/contracts/secret/whitespace、cargo-deny 与 RustSec。phase review 无剩余 P1/P2 blocker；只允许把 annotated `phase-p13-responses-complete` 固定到包含两份 receipt 的 exact pushed commit 并运行一次正式 Delivery Gate。未调用 Provider、未改 management OpenAPI/Prism、server/production/staging 或四个 untracked helper；P13-10 仍阻塞 | `P13-09` `READY_FOR_FORMAL_DELIVERY_GATE`; evidence: [`p13-09-phase-preflight-20260816.md`](reports/evidence/p13-09-phase-preflight-20260816.md)、[`p13-09-phase-review-20260816.md`](reports/evidence/p13-09-phase-review-20260816.md) |
| v1.273 | 2026-08-16 | P13-09 正式收口：annotated tag `phase-p13-responses-complete` 固定到 exact closeout commit `d419c4678bd2ff563046849cef800c1985d48688`，唯一正式 Delivery Gate run [31922870604](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/31922870604) 的 Authorize（2s）、Fast（6m10s）、Full supply-chain（59s）和 Required（3s）全部 PASS；仅有 pinned checkout/cache 的 GitHub Node.js 20→24 非阻断提示。结合 43/43 本地 Full、phase review、ADR-0089..0091、BC-RESP-001..003 与三份 slice report，P13-09A/B/C 和 aggregate 统一为 `DONE_WITH_BOUNDARY`。P13 umbrella 因 P13-08 及后续独立 backlog 继续 `IN_PROGRESS`；未调用 Provider、未改 management OpenAPI/Prism、server/production/staging 或四个 untracked helper，P13-10 未启动，也未重复运行第二个昂贵 Gate | `P13-09` `DONE_WITH_BOUNDARY`; formal evidence: [`p13-09-phase-preflight-20260816.md`](reports/evidence/p13-09-phase-preflight-20260816.md)、[`p13-09-phase-review-20260816.md`](reports/evidence/p13-09-phase-review-20260816.md) |
| v1.274 | 2026-08-16 | 用户明确要求 WebSocket 后启动 P13-10A：公开 `GET /v1/responses` 采用 OpenAI Responses WebSocket flat `response.create`，upgrade 前 exact Client Key auth/no-Origin，复用 existing Canonical/runtime lease/usage/stored continuity，并以 explicit `responses_websocket` capability fail closed。固定 text/fragment/message/event/byte/backpressure/liveness/turn/session/close/cancel 边界；one active + one pending，disconnect drop source/lease。Realtime、Chat/Messages WS、`response.append`/prewarm、browser support 与 Provider-native upstream WS 延后；management OpenAPI/Prism不变但已写 Claude Code cross-boundary 提醒。本片不调用 Provider、不改 production/staging/server、不触发昂贵 GitHub Gate | `P13-10A` `IN_PROGRESS`; evidence target: [`p13-10a-public-responses-websocket.md`](reports/p13-10a-public-responses-websocket.md) |
| v1.275 | 2026-08-16 | P13-10A 本地实现/review 收口：公共 `GET /v1/responses` 通过 exact Client Key/no-Origin upgrade 接收 strict flat `response.create`，复用 existing Responses decoder、Canonical executor、runtime scheduler/lease、usage、stored response 和 exact continuity 输出 JSON lifecycle；新增显式 `responses_websocket` capability、one-active+one-pending、4-MiB/64-fragment/4096-event/8-MiB bounds、write/liveness/session deadlines、closed RFC 6455 codes、disconnect cancellation 与 response-ID collision fail-closed。protocol 30、catalog 15、router 138、gateway 106、HTTP 65、workspace Fast steps、cargo-deny/RustSec、strict Clippy/source/crate/docs/secret/diff 全部通过；首轮 Fast 仅发现并修复依赖 allow-list，boundary/tail 重跑通过。README 双语说明和 Claude Code cross-boundary 留痕已同步；management OpenAPI/Prism unchanged。未调用 Provider、不改 production/staging/server，未运行正式 Delivery Gate | `P13-10A` `LOCAL_PASS_PENDING_PHASE_GATE`; evidence: [`p13-10a-public-responses-websocket.md`](reports/p13-10a-public-responses-websocket.md) |
| v1.276 | 2026-08-16 | P13-10 aggregate closeout candidate 完成：exact implementation commit `fd1e21fc398c732763108bbe12036fa3999818f0` 上运行 `CHECK_REPORT_PATH=/tmp/cpar-p13-10-phase-preflight-20260816.md ./scripts/check.sh full`，Darwin arm64 从 `04:26:59Z` 至 `04:28:58Z` 的 43/43 steps 全部 PASS，覆盖 Prism 83-operation 双构建、workspace fmt/strict Clippy/tests、serve/offline regressions、source/crate/docs/contracts/secret/whitespace、cargo-deny 与 RustSec。phase review 无剩余 P1/P2 blocker；只允许把 annotated `phase-p13-websocket-complete` 固定到包含两份 receipt 的 exact pushed commit并运行一次正式 Delivery Gate。未调用 Provider、不改 management OpenAPI/Prism、server/production/staging 或四个 untracked helper；P13-11 仍阻塞 | `P13-10` `READY_FOR_FORMAL_DELIVERY_GATE`; evidence: [`p13-10-phase-preflight-20260816.md`](reports/evidence/p13-10-phase-preflight-20260816.md)、[`p13-10-phase-review-20260816.md`](reports/evidence/p13-10-phase-review-20260816.md) |
| v1.277 | 2026-08-16 | P13-10 正式收口：annotated tag `phase-p13-websocket-complete` 固定到 exact closeout commit `dc48ec40e4fb38961925f203bf3cd0f7434a34a0`，唯一正式 Delivery Gate run [31926927914](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/31926927914) 的 Authorize（4s）、Fast（5m51s）、Full supply-chain（1m33s）和 Required（3s）全部 PASS；仅有 pinned checkout/cache 的 GitHub Node.js 20→24 非阻断提示。结合 43/43 本地 Full、phase review、ADR-0092、BC-RESP-004 与 P13-10A report，P13-10A 和 aggregate 统一为 `DONE_WITH_BOUNDARY`。P13 umbrella 因 P13-08 及后续独立 backlog 继续 `IN_PROGRESS`；未调用 Provider、未改 management OpenAPI/Prism、server/production/staging 或四个 untracked helper，P13-11 未启动，也未重复运行第二个昂贵 Gate | `P13-10` `DONE_WITH_BOUNDARY`; formal evidence: [`p13-10-phase-preflight-20260816.md`](reports/evidence/p13-10-phase-preflight-20260816.md)、[`p13-10-phase-review-20260816.md`](reports/evidence/p13-10-phase-review-20260816.md) |
| v1.278 | 2026-08-16 | P13-08 retrospective closeout candidate：从 pure Channel Pin implementation `b6a7085ed69144f395d3b5579541844fabf424e9` 建立独立 Gate line；首次 aggregate Full 发现 P13-05C billing catalog GET 错用 write context，失败不计作通过。只重放一行 `read_context` corrective commit `04a8d315750ae2eb08a3f7d6f4fd72efbb101144`，未引入 P13-09A；focused billing 2/2 后 authoritative Full 在 Darwin arm64 从 `04:58:01Z` 至 `05:00:35Z` 43/43 PASS，phase review 无剩余 P1/P2 blocker。只允许 `phase-p13-channel-pin-complete` 触发一次正式 Delivery Gate；不调用 Provider、不改 production/staging/server，P13-09/P13-10 仅存在于后续开发线而不计入该标签 | `P13-08` `READY_FOR_FORMAL_DELIVERY_GATE`; evidence: [`p13-08-phase-preflight-20260816.md`](reports/evidence/p13-08-phase-preflight-20260816.md)、[`p13-08-phase-review-20260816.md`](reports/evidence/p13-08-phase-review-20260816.md) |
| v1.279 | 2026-08-16 | P13-08 正式收口：annotated tag `phase-p13-channel-pin-complete` 固定到 exact closeout commit `7e14a2733c461d04198a6413efda420a03545eea`，唯一正式 Delivery Gate run [31928169486](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/31928169486) 的 Authorize（5s）、Fast（6m55s）、Full supply-chain（1m9s）和 Required（2s）全部 PASS；仅有 pinned checkout/cache 的 GitHub Node.js 20→24 非阻断提示。结合 43/43 本地 Full、phase review、ADR-0088、BC-MGMT-017、OpenAPI/Prism 与 Claude Code handoff，P13-08 以 `DONE_WITH_BOUNDARY` 收口。P13 umbrella 因后续独立 backlog 继续 `IN_PROGRESS`；未调用 Provider、未改 production/staging/server 或四个 untracked helper，P13-11 未启动，也未重复运行第二个昂贵 Gate | `P13-08` `DONE_WITH_BOUNDARY`; formal evidence: [`p13-08-phase-preflight-20260816.md`](reports/evidence/p13-08-phase-preflight-20260816.md)、[`p13-08-phase-review-20260816.md`](reports/evidence/p13-08-phase-review-20260816.md) |
