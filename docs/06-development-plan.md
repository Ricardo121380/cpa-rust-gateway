...
| 当前任务 | `P13-15C/D Build + Codex credential-scoped materialization`：P13-16A 已在 Oracle 生产证明 CPAR 能自动刷新一个到期 Build OAuth，encrypted CAS 后 revision `0 -> 1`，新 material 进入运行池且 Responses 连续成功；失败的 Codex refresh 已增加 `1/2/4/.../60` 分钟有界退避。当前两个生产 Codex Credential 均投影为 `free`，不是 Go，且现有 refresh grant 均被上游拒绝，需要 operator 对目标账号重新 OAuth 后才能产生该 exact production Credential 的 Luna 目录证据。Build/Codex source 已真实观测 `grok-4.6`/`grok-4.5` 与 `gpt-5.6-terra`/`gpt-5.6-luna`/`gpt-5.5`/`gpt-5.4-mini`，但公开 `/v1/models` 仍只是已物化 Candidate；下一切片必须完成 durable freshness/removal 与 exact Credential lease 物化，不得硬编码套餐或前端模型白名单。 |
...
9. **P13-12/15/16 当前活动项**：P13-12 是 Provider/channel entitlement，后端和当前 Build `supergrok` production backfill 已完成但等待前端/formal Gate；P13-15 正在把所有渠道的模型 ID 改为 exact upstream discovery，Build/Codex source 已本地通过而 C/D 自动持久化/route materialization 仍待完成；P13-16A 已为当前生产活动的 Build/Codex OAuth 接入 CPAR startup/periodic refresh，等待 Oracle continuity 验收。Autoreg 只保留注册、首次授权、revoked grant 交互 reauth 与 replenishment，不是日常 refresh executor。
...
> 当前版本置顶变更：`v1.314`（2026-09-02）见下方第一行；历史版本保持原样。
