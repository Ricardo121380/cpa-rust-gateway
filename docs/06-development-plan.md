...
| 当前任务 | P12-10G `DONE`；P12-10H 已按 `CR-P12-10H-002` 完成 100 次合成验收：Console fixture 100/100，native account pool/scheduling/workers/migration 4/4，脚本明确不读取凭证、不发送 Grok 上游请求。grok2api 已按授权停止，CPAR 仍 active，health 200、未认证数据面 401、管理面 loopback-only。该变更豁免本次无流量 Grok 合成切片的 72h/1250-success 门槛，但不声称真实 Grok 上游可用；P12-10 总体仍保留旧 CPA 最终退役、Tag/运维手册等收尾项。CC Switch 仍不读取、不修改。 |
...
| P12-10H | grok2api 停服回滚演练与 CPAR-only 合成观察 | [100-cycle synthetic receipt](reports/evidence/p12-10h-grok-synthetic-100-receipt-20260804.md)：已授权停止 grok2api、Console fixture 100/100、四项 native 支撑回归和 CPAR loopback/auth 边界通过；按 `CR-P12-10H-002` 豁免本次无流量切片的 72h/1250-success，明确不作 live upstream claim | DONE |
