...
| 当前阶段 | `P1` 至 `P6`、P9、P10、P11 与 CPAR 侧 P12 已完成（P12 为 `DONE_WITH_BOUNDARY`）；P13-04、P13-05、P13-06A/B/C、P13-07A/B/C/D、P13-08、P13-09A/B/C、P13-10A、P13-11A/B/C/D、P13-11E E0-E3 与独立 P13-11E4 均已通过对应正式 Delivery Gate 并以 `DONE_WITH_BOUNDARY` 收口。P13-11E4 由 `phase-p13-provider-egress-status-complete` / exact `ce98faa9306d076f5af53b9eef0c818abb1cb9c8` / [Gate 32110872875](https://github.com/Ricardo121380/cpa-rust-gateway/actions/runs/32110872875) 正式验收；P13-11 umbrella 因所有已授权切片均已验收而收口为 `DONE_WITH_BOUNDARY`，E5 真实网络仍作为独立 `DEFERRED_UNAUTHORIZED` 边界，不占用活动任务状态。P13 umbrella 因 P13-12 正在实施而继续 `IN_PROGRESS`。P7 Kiro OAuth、P8 Official API-key E2E 与 Grok Web 外部 egress/真实网络边界继续延后。Autoreg 的注册、登录、SSO/OAuth、凭证 refresh/修复和 replenishment 不属于 CPAR/P12；CPAR 对已导入账号的 entitlement observation、credential pool Health/Quota/Circuit、lease、cooldown、failure feedback 和调度仍属于 CPAR，按 `CR-P12-AUTOREG-SEPARATION-001/002` 与 `CR-P13-ACCOUNT-TIER-001` 解释。 |
| 当前任务 | `P13-12 / CR-P13-ACCOUNT-TIER-001 IN_PROGRESS`：为 CPAR 已导入账号增加 Provider/channel-scoped entitlement。Grok Build 使用 `free/supergrok/heavy`，Grok Web 与 Console 是独立域且禁止复用 Build 结果；ChatGPT/Codex 与 Claude 使用各自闭集。先完成 additive persistence、Build live/JWT classifier、protected account-pool projection、OpenAPI/Claude Code handoff 和回归，再以受控 exact-batch 命令回填当前新 Build 账号为 `supergrok`。Autoreg 已从 CPAR P13 编号移出。 |
| 本次计划变更（2026-09-01，P13-12 account entitlement 启动） | 用户明确纠正 Grok `free/supergrok/heavy` 只属于 Build，Web 与 Console 必须单独建模；并要求 Autoreg 不占用 CPAR 开发阶段。批准 [CR-P13-ACCOUNT-TIER-001](change-requests/CR-P13-ACCOUNT-TIER-001.md)：原 P13-12 Autoreg handoff 行移出 CPAR phase sequence，P13-12 改为账号 entitlement。管理投影源不可用/快照失效的遗留映射已先修为 safe `503 management_runtime_unavailable` 并有 HTTP 回归。当前实现不改 `web/prism/**`；authoritative OpenAPI 变化后必须在 cross-boundary log 留给 Claude Code。 | `P13-12 IN_PROGRESS` |
...
> 当前版本置顶变更：`v1.308`（2026-08-19）见下方第一行；历史版本保持原样。
