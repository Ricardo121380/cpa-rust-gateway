...
| 当前任务 | `P13-15C/D production acceptance and P13-15E planning`：C/D 已完成 migration 0021、Config/Endpoint/Credential 级 last-success/freshness/removal、safe protected status、Build/Codex runtime worker、exact Credential route materialization、atomic publication 与 ingress-to-lease immutable snapshot pin，本地 gateway/HTTP/router/Provider 回归通过。下一步先部署 exact commit 到 Oracle Singapore，证明真实 Build catalog 自动把 `grok-4.6` 加入授权 `/v1/models` 且真实 Responses 成功；随后补 P13-15E isolation/formal Gate。两个生产 Codex Credential 仍为 `free` 且 refresh grant 已失效，Luna 的 production acceptance 等 operator 对目标 Go 账号重新 OAuth，不得用套餐推断或前端常量代替。 |
...
> 当前版本置顶变更：`v1.315`（2026-09-02）见下方第一行；历史版本保持原样。
