...
| 当前任务 | P12-10G `DONE`；P12-10H 的本地合成仍为 100/100，直接 CPAR HTTP E2E 仍因生产图无 Grok route 而 `BLOCKED_NO_GROK_ROUTE`。native Build route 绑定代码已按冻结 grok2api `v3.0.10` 行为完成并通过 [source parity review](reports/p12-10h-grok2api-parity-review.md)，修正了 Console canonical envelope 的 `sso_token` 解包；代码已提交并推送，生产图仍未写入。下一步仅使用已审查的 Linux ARM 制品，在 loopback-only 隔离 staging 图绑定 native route，重跑最多 100 次 CPAR Chat/Responses/Messages JSON/SSE HTTP E2E，并完整回滚；P12-10 总体仍保留最终退役、Tag/运维手册等收尾项。 |
...
| P12-10H | grok2api 停服回滚演练、CPAR 合成观察与 HTTP E2E | [100-cycle synthetic receipt](reports/evidence/p12-10h-grok-synthetic-100-receipt-20260804.md) 已通过；[source parity review](reports/p12-10h-grok2api-parity-review.md) 已通过；[CPAR curl E2E preflight](reports/evidence/p12-10h-grok-cpar-e2e-preflight-20260804.md) 认证 `/v1/models` 通过但因无 Grok route 在推理前停止，100 次实际发送 0 次 | BLOCKED |
