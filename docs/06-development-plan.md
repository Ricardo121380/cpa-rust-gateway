...
| 当前任务 | P12-10G `DONE`；P12-10H 的本地合成仍为 100/100，但按用户纠正新增了直接 CPAR HTTP E2E：新 harness 会用 CPAR base URL + client key，通过 `curl` 轮转 Chat/Responses/Messages 的 JSON/SSE。当前服务器认证 `/v1/models` 显示 3 个模型但无 Grok route，因此预检以 `grok_route_missing` fail closed，100 次推理请求实际发送 0 次。grok2api 已按授权停止，CPAR 仍 active；未修改 CPAR 图、Caddy、DNS、旧 CPA 或 CC Switch。下一步是先在受控 CPAR 图中绑定 native Grok route，再重跑 100 次 HTTP E2E；P12-10 总体仍保留最终退役、Tag/运维手册等收尾项。 |
...
| P12-10H | grok2api 停服回滚演练、CPAR 合成观察与 HTTP E2E | [100-cycle synthetic receipt](reports/evidence/p12-10h-grok-synthetic-100-receipt-20260804.md) 已通过；[CPAR curl E2E preflight](reports/evidence/p12-10h-grok-cpar-e2e-preflight-20260804.md) 认证 `/v1/models` 通过但因无 Grok route 在推理前停止，100 次实际发送 0 次 | BLOCKED |
