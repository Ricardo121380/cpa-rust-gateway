...
| 当前任务 | `P13-07A` 已完成本地实现与 review：新增 side-effect-free Provider-scoped deterministic selector 和安全 decision projection，但尚未接入 serving scheduler/现有 Route Explain。下一片为 `P13-07B` composition + Route Explain integration；继续保留 max_attempts、首败、Config Version 与无跨 Provider fallback 边界，不启动 P13-08/11/12。 |
...
| P13-07 | Cost-aware、Fill-first、Least-loaded 路由：在 Provider scope 内依据额度、并发、健康和 capability 选择；保留 max_attempts、首败边界和 Config Version 回滚 | P13-05、P13-06 | P13-07A deterministic Provider-scoped selector 已本地通过；P13-07B 待接入现有 scheduler/Route Explain，后续再做 bounded staging canary；不跨 Provider fallback | IN_PROGRESS |
...
| 状态 | `LOCAL_PASS_PENDING_PHASE_GATE`；P13-07A 本地实现与 review 完成，P13 umbrella 仍 `IN_PROGRESS`；下一片为 P13-07B composition/scheduler/Route Explain integration |
