...
| 当前任务 | `P13-07` 收口边界 review：P13-07C 已完成本地实现与独立复核，状态为 `LOCAL_PASS_PENDING_PHASE_GATE`。当前 serving composition 对 versioned cost 仍明确投影为 `Unknown`；在 P13-07 phase closeout 或 staging canary 前，下一步必须先冻结 P13-05 price catalog 到 request-time routing cost signal 的无歧义语义，且不得增加每请求 Store I/O、猜测 token 数或把 unknown 当零。不启动 P13-08/11/12。 |
...
| P13-07 | Cost-aware、Fill-first、Least-loaded 路由：在 Provider scope 内依据额度、并发、健康和 capability 选择；保留 max_attempts、首败边界和 Config Version 回滚 | P13-05、P13-06 | P13-07A deterministic Provider-scoped selector、P13-07B 只读 scheduler/Route Explain composition 与 P13-07C same-scheduler serving lease revalidation 已本地通过并完成 review；versioned cost 当前仍为 `Unknown`，须先冻结 P13-05 catalog→request routing cost signal，再做 P13-07 phase closeout或 bounded staging canary；不跨 Provider fallback | IN_PROGRESS |
