...
| 当前任务 | `P13-15B + P13-16A production acceptance`：Build/Codex exact Endpoint/Credential-scoped discovery source 已本地通过；真实上游当前观测为 Build `grok-4.6`/`grok-4.5` 与 visible API-supported Codex `gpt-5.6-terra`/`gpt-5.6-luna`/`gpt-5.5`/`gpt-5.4-mini`，不是套餐白名单。启动 catch-up、每分钟 bounded refresh、encrypted CAS 与 atomic runtime replacement 已覆盖当前生产活动的 Build/Codex OAuth，本地测试/strict Clippy 通过，等待 Oracle 部署和 post-refresh continuity。P13-15C/D 自动持久化/route materialization 与其余渠道仍未完成，不能把当前列表误称为全渠道动态透传。P13-12 后端与生产 `supergrok` authoritative backfill 已完成，仍等待 Claude Code 前端同步与 formal Gate。 |
...
| P13-16 | 已导入 OAuth runtime 自动续期：active exact Provider/channel Credential 在 expiry 前由 CPAR 后台 refresh、encrypted CAS 持久化并原子替换后续 lease；API key/SSO cookie 不伪刷新；注册、首次授权、revoked grant 交互 reauth 与 replenishment 仍在 Autoreg/operator 边界 | P6/P12 refresh transaction、P13-06 pool、P13-11 egress；`CR-P13-RUNTIME-OAUTH-REFRESH-001` | A 已实现当前生产活动的 Grok Build/Codex startup catch-up + periodic worker、Build provider-only claim、Codex exact binding、in-flight lease pinning和本地回归；Oracle restart/post-expiry evidence待完成。Claude/Kiro 在对应延期渠道启用前须各自 compose executor，之后统一 formal Gate | LOCAL_PASS_PENDING_PHASE_GATE |
