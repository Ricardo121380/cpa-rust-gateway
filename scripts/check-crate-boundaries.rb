...
allowed = {
  ...
  "gateway-control" => %w[gateway-access gateway-auth gateway-catalog gateway-core gateway-observability gateway-router gateway-store gateway-upstream serde_json zeroize],
  "gateway-core" => %w[serde serde_json],
  "gateway-http-actix" => %w[actix-web futures-util gateway-auth gateway-control gateway-core gateway-observability gateway-protocol gateway-router gateway-store gateway-stream gateway-upstream protocol-anthropic protocol-openai-responses provider-openai-compatible serde serde_json subtle tokio url zeroize],
  ...
  "provider-grok" => %w[flate2 gateway-catalog gateway-continuity gateway-core gateway-provider gateway-router gateway-store gateway-upstream getrandom hmac protocol-openai-responses rusqlite serde serde_json sha2 time tokio url zeroize],
