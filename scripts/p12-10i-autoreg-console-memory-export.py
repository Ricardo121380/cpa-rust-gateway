...
def _read_direct_file(path: Path, *, require_root: bool, maximum: int) -> bytes:
...
def _parse_session(raw: bytes, token: str, expected_email: str) -> None:
    ...
    for line in raw.splitlines():
        ...
        if not isinstance(document, dict) or set(document) - ALLOWED_SESSION_KEYS:
            fail("invalid_session_document")
        if document.get("email") != expected_email or not isinstance(document.get("cookies"), list):
            continue
        ...
        for cookie in document["cookies"]:
            ...
            if isinstance(name, str) and isinstance(value, str):
                cookies[name] = value
        if cookies.get("sso") == token and cookies.get("sso-rw") == token:
            matches += 1
    if matches != 1:
...
def _read_task(
    ...
    *,
    ...
    _direct_path(database, directory=False, require_root=require_root)
    try:
    ...
    if status != "completed" or target != completed or failed != 0 or not isinstance(email, str) or not email:
    ...
    token = _parse_sso(
        _read_direct_file(sso_path, require_root=require_root, maximum=MAX_TOKEN_BYTES)
    )
    session_path = directory / "keys" / "auth-sessions.jsonl"
    session_raw = _read_direct_file(
        session_path, require_root=require_root, maximum=MAX_SESSION_BYTES
    )
    _parse_session(session_raw, token, email)
...
def main() -> int:
    ...
    token, directory = _read_task(source_root, arguments.database, arguments.task_id, max_age_hours=arguments.max_age_hours)
    # The source identity is intentionally task-scoped and contains no email or token.
    ...
    print("source_export=PASS provider=grok_console source_count=1 emitted_count=1", file=sys.stderr, flush=True)
    return 0
